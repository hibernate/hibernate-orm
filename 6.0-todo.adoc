...
public interface DatabaseInformation {
    ...
    // 		Would be a good idea to "shadow extend", or at least learn from it.
    //		Best option is to reuse it ("shadow extend", etc) and have the
    //		existing code that uses it be able to continue to work as before.
