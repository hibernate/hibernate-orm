...
// 		buildEnv(defaultJdk, 'edb')
		...
		jdkJavassistBuildEnv(defaultJdk, '17'),
		jdkJavassistBuildEnv(defaultJdk, '18'),
		jdkJavassistBuildEnv(defaultJdk, '19'),
	];
		...
		executions.put(buildEnv.tag, {
			runBuildOnNode(buildEnv.node) {
				...
				withEnv(["JAVA_HOME=${tool buildEnv.buildJdkTool}", "PATH+JAVA=${tool buildEnv.buildJdkTool}/bin", "TEST_JAVA_HOME=${tool buildEnv.testJdkTool}"]) {
					...
					if ( buildEnv.getBytecodeProvider() != null ) {
						state[buildEnv.tag]['additionalOptions'] += " -Dhibernate.bytecode.provider=${buildEnv.getBytecodeProvider()}";
					}
...
BuildEnvironment buildEnv(String version, String dbName) {
	return new BuildEnvironment( version, version, dbName, null, NODE_PATTERN_BASE, null );
}

BuildEnvironment buildEnv(String version, String dbName, String node) {
	return new BuildEnvironment( version, version, dbName, null, node, null );
}

BuildEnvironment buildEnv(String version, String dbName, String node, String notificationRecipients) {
	return new BuildEnvironment( version, version, dbName, null, node, notificationRecipients );
}

BuildEnvironment jdkBuildEnv(String version, String testVersion) {
	return new BuildEnvironment( version, testVersion, "h2", null, NODE_PATTERN_BASE, null );
}

BuildEnvironment jdkBuildEnv(String version, String testVersion, String notificationRecipients) {
	return new BuildEnvironment( version, testVersion, "h2", null, NODE_PATTERN_BASE, notificationRecipients );
}

BuildEnvironment jdkJavassistBuildEnv(String version, String testVersion) {
	return new BuildEnvironment( version, testVersion, "h2", "javassist", NODE_PATTERN_BASE, null );
}
...
public class BuildEnvironment {
	...
	public BuildEnvironment(String version, String testVersion, String dbName, String bytecodeProvider, String node, String notificationRecipients) {
	...
	String getTag() { return "jdk_${testVersion}_${dbName}" + (bytecodeProvider == null ? '' : "_${bytecodeProvider}") }
	String getBytecodeProvider() { bytecodeProvider }
	String getNode() { node }
