...
if [ "$RDBMS" == "derby" ]; then
  ...
  goal="-Pdb=oracle -Dhibernate.connection.url=jdbc:oracle:thin:@localhost:1521:ORCLCDB -Dhibernate.connection.username=\"sys as sysdba\" -Dhibernate.connection.password=Oradoc_db1"
elif [ "$RDBMS" == "db2" ]; then
...
eval "./gradlew check ${goal} -Plog-test-progress=true --stacktrace"