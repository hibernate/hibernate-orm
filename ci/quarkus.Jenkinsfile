...
@Field final String ORM_JDK_VERSION = '25'
...
def manuallyTriggered = currentBuild.getBuildCauses().toString().contains( 'UserIdCause' )

// allow to bypass the check if the build is triggered manually:
if (!manuallyTriggered && !ENABLE_QUARKUS_BUILDS) {
    print "INFO: Build skipped because we are not yet testing a next LTS version of Quarkus."
    currentBuild.result = 'NOT_BUILT'
    return
}
...
def configurations = [
        ...
        // Skipped because we only care about one project, which we added to Data3
        //new BuildConfiguration( name: "Data4", nativeProfile: true, projects: "hibernate-orm-rest-data-panache"),
        new BuildConfiguration( name: "Data5", nativeProfile: true, projects: "jpa-postgresql, jpa-postgresql-withxml, hibernate-reactive-postgresql, hibernate-orm-tenancy/schema, hibernate-orm-tenancy/schema-mariadb"),
        // Ignoring, because they don't bring much here: hibernate-search-orm-elasticsearch-tenancy, hibernate-search-orm-opensearch, hibernate-search-standalone-elasticsearch, hibernate-search-standalone-opensearch
        new BuildConfiguration( name: "Data6", nativeProfile: true, projects: "hibernate-search-orm-elasticsearch, hibernate-search-orm-elasticsearch-outbox-polling"),
        new BuildConfiguration( name: "Data7", nativeProfile: true, projects: "hibernate-reactive-db2, hibernate-reactive-mariadb, hibernate-reactive-mssql, hibernate-reactive-mysql, hibernate-reactive-mysql-agroal-flyway, hibernate-reactive-panache, hibernate-reactive-panache-kotlin, hibernate-reactive-oracle")
]
