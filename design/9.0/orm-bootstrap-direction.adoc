...
This means `ManagedResources` acts as the historical source bundle, while this PoC's `PreparedMappingSources`
...
PersistenceUnitDescriptor / HibernatePersistenceConfiguration
    ...
    -> BindingCoordinator / MappingResolutionPipeline
----
...
* `MappingSources`: source contributions after entry-point source discovery, including scanner results, not yet categorized

== PreparedMappingSources

`PreparedMappingSources` should be the normalized source model produced by source collection.
...
stored as separate `PreparedMappingSources` buckets.  Extra query imports are
...
references, class names, package names, XML bindings, converter descriptors, and query imports.  `PreparedMappingSources`
should be source-model oriented and normalized for categorization.

== XML Preprocessing

Move XML preprocessing behind `PreparedMappingSources`.  The PoC now uses direct
consumption:

[source]
----
PreparedMappingSources -> XmlPreProcessingResult
----
...
[source]
----
PreparedMappingSources + MetadataBuildingContext -> CategorizedDomainModel
----
...
=== Make PreparedMappingSources the Normalized Source Model

PreparedMappingSources should be the output of source collection.
...
=== Move XML Preprocessing Behind PreparedMappingSources

Today XML preprocessing takes ManagedResources. In the PoC, make XML preprocessing consume PreparedMappingSources directly.
...
=== Categorize From PreparedMappingSources
...
PreparedMappingSources *is not* a clone of ManagedResources.
...
PreparedMappingSources should be normalized:
...
`MappingResolutionPipeline`.  Keep this section only as historical context for how the
