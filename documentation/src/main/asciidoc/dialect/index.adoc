...
This guide catalogs available Dialects. Authors and maintainers of external
Dialect implementations should use the link:{doc-dialect-provider-url}[Dialect Provider Guide].
The link:{doc-spi-url}[SPI and Service Provider Guide] defines the general
provider-role and compatibility model used by that guide.
