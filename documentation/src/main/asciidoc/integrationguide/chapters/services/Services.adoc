...
[[services-factory-owned]]
==== Factory-owned runtime components

Runtime components which require a `SessionFactory` are owned directly by the
factory instead of being stored in a child service registry.  Extension
strategies or factories are discovered as Java services while the factory's
construction plan is prepared.

===== `EventListenerRegistry`

`org.hibernate.event.service.spi.EventListenerRegistry` is owned by the
factory's `EventEngine`.
It manages all of Hibernate's event listeners.
...
`org.hibernate.stat.spi.StatisticsImplementor` is the SPI portion of the
Statistics API; the collector portion, if you will.  A custom
`org.hibernate.stat.spi.StatisticsFactory` may be registered as a Java service.

===== `NativeQueryInterpreter`

`org.hibernate.engine.query.spi.NativeQueryInterpreter` is the factory-owned
strategy Hibernate uses for interpreting native queries.  An implementation
may be registered as a Java service.

===== `CacheImplementor`

`org.hibernate.cache.spi.CacheImplementor` provides the factory's access to the
second-level cache.  A custom `org.hibernate.cache.spi.CacheFactory` may be
registered as a Java service.
