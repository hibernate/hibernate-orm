...
| Using link:{doc-javadoc-url}org/hibernate/jpa/HibernatePersistenceConfiguration.html[`HibernatePersistenceConfiguration`]
// or the older link:{doc-javadoc-url}org/hibernate/cfg/Configuration.html[`Configuration`] class
to construct a `SessionFactory`
...
| By letting the container take care of the bootstrap process and of injecting the `EntityManagerFactory`
...
Such container environments typically also feature facilities to automatically manage the lifecycle of an `EntityManager` and its association with container-managed transactions.
...
| Informix                  | `com.informix.jdbc.IfxDriver:{version}`
...
| {data}[Jakarta Data] stateless and stateful repository support |
`jakarta.data:jakarta.data-api` +
and `jakarta.data:jakarta.data.stateful-api`
...
| https://github.com/mongodb/mongo-hibernate/[MongoDB] dialect | `org.mongodb:mongodb-hibernate`
| https://github.com/GoogleCloudPlatform/google-cloud-spanner-hibernate[Google Spanner] dialect | `com.google.cloud:google-cloud-spanner-hibernate-dialect`
|===
...
In some container environments, for example, in any EE container, the `<class>` elements are unnecessary, as we explain <<entity-discovery,below>>.

The `<default-to-one-fetch-type>` element specifies that many-to-one and one-to-one associations default to <<proxies-and-lazy-fetching,lazy fetching>>.
As <<lazy-problem,explained later>>, we strongly recommend this setting in all new code.
...
EntityManagerFactory entityManagerFactory =
        Persistence.createEntityManagerFactory("org.hibernate.example",
                Map.of(Persistence.ConnectionProperties.JDBC_PASSWORD, password));
----

[[configuration-api]]
=== Programmatic configuration using JPA API

The `PersistenceConfiguration` class allows full programmatic control over creation of the `EntityManagerFactory`.
...
The `defaultToOneFetchType()` method specifies that many-to-one and one-to-one associations default to <<proxies-and-lazy-fetching,lazy fetching>>.
As <<lazy-problem,explained later>>, we strongly recommend this setting in all new code.

The specification gives JPA implementors like Hibernate explicit permission to extend this class, and so Hibernate offers the link:{doc-javadoc-url}org/hibernate/jpa/HibernatePersistenceConfiguration.html[`HibernatePersistenceConfiguration`], which lets us obtain a `SessionFactory` without any need for a cast.footnote:[Alternatively, the venerable class link:{doc-javadoc-url}org/hibernate/cfg/Configuration.html[`Configuration`] offers similar functionality.]
...
SessionFactory sessionFactory =
        new HibernatePersistenceConfiguration("Bookshop")
            ...
            .defaultToOneFetchType(FetchType.LAZY)
            // register the entity classes
            .managedClasses(Book.class, Author.class)
            // PostgreSQL
...
.*Read more: Advanced configuration options*
[%collapsible]
====
****
...
The inner interfaces of the `Persistence` class -- which, for convenience, are inherited by `PersistenceConfiguration` -- declare `static final` constants holding the names of all configuration properties defined by the specification itself.
For example, `Persistence.ConnectionProperties.JDBC_URL` holds the property name `"jakarta.persistence.jdbc.driver"`.
...
To see the generated SQL as it's sent to the database, we have two options.

One way is to set the property `hibernate.show_sql` to `true`, so that Hibernate logs SQL directly to the console.
...
Note that `hibernate.globally_quoted_identifiers` is a synonym for `<delimited-identifiers/>` in the `orm.xml` mapping file.
...
where the configured Dialect reports that the exact Java class is supported by the driver.
JDBC drivers are required to directly support `java.time` types since Java 8 (JDBC 4.2).

When the Dialect reports that a requested type is unsupported, Hibernate falls back to the corresponding
legacy JDBC representation and logs the known driver/database limitation.  This capability is resolved
separately for each Java Time class, with `OffsetTime` and `OffsetDateTime` treated as a matched pair.
