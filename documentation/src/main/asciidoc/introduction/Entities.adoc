...
====
Hibernate _does_ allow a unidirectional one-to-many association, that is, a `@OneToMany` with no matching `@ManyToOne` on the other side.
...
By default, this association maps to a separate <<join-table-mappings,association join table>>.
In practice, this mapping is quite unnatural and should be avoided almost all of the time.
====
