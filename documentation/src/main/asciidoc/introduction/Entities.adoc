...
[TIP]
====
The requirement for a default constructor is relaxed when the <<bytecode-enhancer,bytecode enhancer>> is used.
====
...
As a result, Hibernate cannot add a primary key to the table.
...
Set<String> topics;
----
Now the generated DDL looks a lot better:
[source,sql]
----
create table Book_topics (
    Book_isbn varchar(255) not null,
