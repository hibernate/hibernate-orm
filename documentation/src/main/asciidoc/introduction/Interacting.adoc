...
Similarly, `StatelessSession` extends `EntityAgent`.
...
An instance of `EntityManager` is a _stateful session_.
It mediates the interaction between your program and the database via operations on a _persistence context_.
In this chapter, we're going to focus on `EntityManager` first.
We'll <<bypass-persistence-context,come back>> to `EntityAgent` and _stateless sessions_ toward the end of the chapter.
What you need to know for now is that a stateless session doesn't have a persistence context.

Still, we should note for the record that some people prefer to use stateless sessions everywhere.
...
We saw how to obtain an `EntityManagerFactory` in <<configuration-jpa>> and <<configuration-api>>.
It's quite unsurprising that we may use this object to create an `EntityManager` or `EntityAgent`:
...
When we're finished with the `EntityManager` or `EntityAgent`, we must explicitly clean it up:
...
[CAUTION]
Even though an `EntityAgent` is a "stateless" session, the `EntityAgent` itself still holds state which must be cleaned up by calling `close()`.
...
JPA 4 _session creation options_ let us provide some information controlling the behavior of the `EntityManager` or `EntityAgent` in a type safe way.

[source,java]
----
EntityManager entityManager =
        entityManagerFactory.createEntityManager(new FetchBatchSize(20),
                                                 ReadOnlyMode.READ_ONLY,
                                                 CacheStoreMode.BYPASS);
----

Search for implementations of `EntityManager.CreationOption` or `EntityAgent.CreationOption` to find the option types provided by the specification and by Hibernate.
A number of useful options are declared by link:{doc-javadoc-url}org/hibernate/SessionCreationOption.html[`SessionCreationOption`].

.*Read more: Injecting the EntityManager or EntityAgent*
[%collapsible]
====
****
If you're writing code for some sort of container environment, you'll probably obtain the `EntityManager` or `EntityAgent` by some sort of dependency injection.
...
[source,java]
----
@PersistenceAgent EntityAgent entityAgent;
----
...
[source,java]
----
@Inject EntityAgent entityAgent;
----
****
====
...
entityManagerFactory.runInTransaction(entityManager -> {
    // do the work in a stateful session
    ...
});
----
...
In case we're using an `EntityAgent`, or if we would like direct access to the `Session` or `StatelessSession` without a type cast, `runInTransaction()` lets us specify the class of session we require:

[source,java]
----
entityManagerFactory.runInTransaction(EntityAgent.class, entityAgent -> {
    // do the work in a stateless session
    ...
});
----
[source,java]
----
entityManagerFactory.runInTransaction(Session.class, session -> {
    // do the work in a Hibernate session
    ...
});
----
...
| `get(Class,Object)`
| Obtain a persistent object given its type and its id, throwing `EntityNotFoundException` if there is no such record
| `find(Class,Object)`,
| Obtain a persistent object given its type and its id, returning `null` if there is no such record
...
returning `null` if there is no such record
...
| `get(Class,Object,FindOption...)`
| Obtain a persistent object given its type and its id, using the specified options, throwing if there is no such record
| `get(EntityGraph,Object,FindOption...)`
| Obtain a persistent object given its id and an `EntityGraph` specifying its type and associations which should be eagerly fetched, using the specified options, throwing if there is no such record
| `find(Class,Object,FindOption...)`
| Obtain a persistent object given its type and its id, using the specified options, returning `null` there is no such record
| `find(EntityGraph,Object,FindOption...)`
| Obtain a persistent object given its id and an `EntityGraph` specifying its type and associations which should be eagerly fetched, using the specified options, returning `null` there is no such record
...
Finally, the following methods are capable of efficiently loading multiple entity instances in parallel:

[%breakable,cols="50,~"]
|===
| Method name and parameters | Effect

| `getMultiple(Class,List<Object>,FindOption...)`
| Obtain a list of persistent objects given their type and their ids, using the specified options, throwing if there is no such record
| `getMultiple(EntityGraph,List<Object>,FindOption...)`
| Obtain a list of persistent objects given their ids and an `EntityGraph` specifying their type and associations which should be eagerly fetched, using the specified options, throwing if there is no such record
| `findMultiple(Class,List<Object>,FindOption...)`
| Obtain a list of persistent objects given their type and their ids, using the specified options, returning `null` there is no such record
| `findMultiple(EntityGraph,List<Object>,FindOption...)`
| Obtain a list of persistent objects given their ids and an `EntityGraph` specifying their type and associations which should be eagerly fetched, using the specified options, returning `null` there is no such record
|===
...
[source,java]
----
List<Book> books = entityManger.findMultiple(Book.class, bookIds);
----

link:{doc-javadoc-url}org/hibernate/FindMultipleOption.html[`FindMultipleOption`] declares a number of options influencing the behavior of `findMultiple()` and `getMultiple()`.

[[load-by-natural-id]]
=== Lookup by natural id
As discussed <<natural-id-attributes,earlier>>, Hibernate offers the ability to map a natural id and perform retrieval operations using the natural id.
This is accomplished using the option link:{doc-javadoc-url}org/hibernate/KeyType.html#NATURAL[`KeyType.NATURAL`]:

[source,java]
----
var bookKey = new BookKey(isbn, printing);
var book = entityManager.find(Book.class, bookKey, KeyType.NATURAL);
----

In this example, `BookKey` is the link:{doc-javadoc-url}org/hibernate/annotations/NaturalIdClass.html[`@NaturalIdClass`] for `Book`.
If an entity with a composite natural id lacks a natural id class, a `Map` may be used instead:

[source,java]
----
var bookKey = Map.of(Book_.ISBN, isbn, Book_.PRINTING, printing);
var book = entityManager.find(Book.class, bookKey, KeyType.NATURAL);
----
...
1. Hibernate only does this for an entity which is currently associated with a persistence context.
...
- All associations should be set `fetch=LAZY` to avoid fetching extra data when it's not needed.
  As we mentioned <<lazy-problem,earlier>>, this setting is not the default for `@ManyToOne` associations, and must be specified explicitly unless the default is changed using `<default-to-one-fetch-type>` or `defaultToOneFetchType()`.
...
[source,java]
----
Book book = entityManager.find(Book.class, bookId);  // fetch just the Book, leaving authors unfetched
...
Instances of `FetchOption` influence how a node of a graph is fetched.

[source,java]
----
var graph = entityManager.createEntityGraph(Book.class);
graph.addAttributeNode(Book_.publisher)
...
[NOTE]
====
In the code examples above, The classes `Book_` and `Author_` are generated by <<metamodel-generator,Hibernate Processor>>, as we saw <<mapped-by-metamodel,earlier>>.
...
.*Read more: Fetch graphs and load graphs*
[%collapsible]
====
****
...
The annotation-based API used to be so verbose that it just not wasn't worth using.
However, in JPA 4 this API has been much improved via introduction of the `@Fetch` annotation.
So perhaps you'll find a use for it.
You'll find more information in the Javadoc for `@NamedEntityGraph`.
====
...
3. finally, the modification is made persistent by merging the detached instance in a second transaction, with a new persistence context, by calling `entityManager.merge(e)`.
...
TIP: In some cases, `merge()` is much less efficient than the <<bypass-persistence-context,`upsert()`>> operation of `EntityAgent`.
...
|===
| Hibernate link:{doc-javadoc-url}org/hibernate/FlushMode.html[`FlushMode`] | JPA `FlushModeType` | Interpretation

| `MANUAL` | `EXPLICIT` | Never flush automatically
...
The custom option link:{doc-javadoc-url}org/hibernate/ReadOnlyMode.html[`ReadOnlyMode`] may be used to control read-only mode:

[source,java]
----
// load a single entity in read-only mode
var book = entityManager.find(Book.class, isbn, ReadOnlyMode.READ_ONLY);
----
[source,java]
----
// load all query results in read-only mode
var books =
...
[source,java]
----
// load every entity in read-only mode
entityManager.addOption(ReadOnlyMode.READ_ONLY);
----

It's not necessary to dirty-check an entity instance in read-only mode.
Indeed, read-only mode is a way to make a stateful session work a bit more like a stateless session, bypassing some of the overhead associated with the use of a persistence context.

[[bypass-persistence-context]]
=== Operations which bypass the persistence context

When we use an `EntityAgent`, we're always working with detached objects.
Thus, the programming model is a bit different to what we've just seen.
The methods for retrieving entities and querying the database are the same -- indeed, they're all declared by a common supertype named `EntityHandler` -- but these operations always return detached objects, and so lazy fetching is an explicit operation.
And the operations used to update the database are completely different.

.Important methods of the `EntityAgent`
[%autowidth.stretch]
|===
| Method name and parameters | Effect
...
NOTE: The operations of a `EntityAgent` have no corresponding ``CascadeType``s, and so these operations never cascade to associated entity instances.

NOTE: There's no `flush()` operation, and so `update()` is always explicit.

Notice that all the operations listed above result in an immediate interaction with the database.
...
Lifecycle callbacks allow the application program to respond to events occurring deep within Hibernate.

- The annotations `@PrePersist`, `@PreRemove`, `@PreMerge`, `@PostPersist`, `@PostRemove`, `@PostMerge`, and `@PostLoad` allow an entity to respond to persistence lifecycle operations and maintain its transient internal state.
  These events only occur in a stateful session.

- The annotations `@PreInsert`, `@PreUpdate`, `@PreUpsert`, `@PreDelete`, `@PostInsert`,`@PostUpdate`, `@PostUpsert`, and `@PostDelete` allow the application program to be notified of operations executed against the database.
  These events occur in both stateful and stateless sessions.
...
There are two distinct ways to attach listeners to an entity:

- The `@EntityListeners` annotation specifies one or more listeners for a given entity class:
+
[source,java]
----
@Entity
...
- On the other hand, the newer `@EntityListener` annotation lets a listener be attached to one or more entities without the need to annotate the entities themselves:
+
[source,java]
----
@EntityListener
