...
In this chapter, we're not going to talk much about `EntityAgent`.
...
.*Read more: Injecting the `EntityManager`*
[%collapsible]
====
****
...
JPA 4 _session creation options_ let us provide some information controlling the behavior of the `EntityManager` in a type safe way.
...
Search for implementations of `EntityManager.CreationOption` to find the option types provided by the specification and by Hibernate.
