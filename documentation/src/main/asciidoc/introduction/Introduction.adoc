...
But without the extended mapping annotations, and without the use of Hibernate-specific option objects, you miss out on much of the power of Hibernate.
====
...
But first, we want to make sure you can at least run Hibernate.
...
- bootstrap Hibernate and create a `EntityManagerFactory` or `SessionFactory` at the beginning of your test suite (we've already seen how to do that), and
- create a new `EntityManager` or `Session` inside each `@Test` method, using `inTransaction()`, for example.
