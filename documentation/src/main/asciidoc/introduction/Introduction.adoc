...
- <<metamodel-generator,Hibernate Processor>>, which is responsible for generating the _static metamodel_ and implementations of Jakarta Data repositories.
...
But JPA 4 changes the tradeoff significantly.

[IMPORTANT]
As of Jakarta Persistence 4.0, we feel that there is no longer much advantage to be had from the use of `Session` and `SessionFactory`.
...
In this Guide, we call each instance of `EntityManager` a "session".
...
In Hibernate, every `EntityAgent` is also an instance of the much older interface <<stateless-sessions,`StatelessSession`>>, and so we sometimes call an instance of `EntityAgent` a "stateless session".
...
In this case, we're using the `runInTransaction()` method of `EntityManagerFactor`.
...
[source,java]
----
configuration.property(Persistence.SchemaManagementProperties.SCHEMAGEN_LOAD_SCRIPT_SOURCE,
