...
[[case-convention]]
.*Read more: Hibernate SQL case convention*
[%collapsible]
====
****
...
====
However, we can add a discriminator column if we like, and in that case the generated SQL for polymorphic queries is slightly simpler.
====
...
create table BooksAuthors (
    ...
    primary key (Book_isbn, authors_ssn)
)
====
...
Here, there should be a `UNIQUE` constraint or `PRIMARY KEY` on one of the columns of the association table.
...
====
[source,sql]
create table AuthorPerson (
    author_ssn varchar(255) not null unique,
    ssn varchar(255) not null,
    primary key (ssn)
)
====

Here, there should be a `UNIQUE` constraint or `PRIMARY KEY` on _both_ columns of the association table.

Finally, `@JoinTable` may also be used with a <<unidirectional-one-to-many,unidirectional `@OneToMany` association>>.

[source,java]
----
@OneToMany // no matching @ManyToOne
@JoinTable(name="PublishedBooks")
Set<Book> books;
----
...
Here there is a `UNIQUE` or `PRIMARY KEY` constraint on the foreign key of the `Book` table in the join table `PublishedBooks`.
...
@Table(name="Books")
@SecondaryTable(name="Editions",
        foreignKey = @ForeignKey(name="BooksToEditionsById"))
class Book {
    @Id @GeneratedValue
...
alter table if exists Editions
   add constraint BooksToEditionsById
   foreign key (bookId)
   references Books
====

Note that we have used the `@ForeignKey` annotation to assign a name to the foreign key constraint.
...
alter table if exists Items
   add constraint ItemsToBooksBySsn
...
[[lob-semantics]]
.*Read more: Semantics of the @Lob annotation*
[%collapsible]
====
****
...
In Hibernate, an attribute annotated `@Lob` is written to JDBC using the `setClob()` or `setBlob()` method of `PreparedStatement`, and is read from JDBC using the `getClob()` or `getBlob()` method of `ResultSet`.
...
This is easy: just annotate the record type, or the attribute which holds a reference to it, with the new `@Struct` annotation:
...
Both `@UniqueConstraint.columnNames` and <<indexes,`@Index.columnList`>> identify mapped columns.
Except for explicit collection-role markers, Hibernate resolves each reference in this order:

. The logical column name, before physical naming.
. The physical column name, if no logical name matches.
. An entity attribute path, if neither column lookup matches and the declaration belongs to an entity's primary or secondary table.

For example, `@Column(name="person_name") String name` may be referenced as `person_name`,
as its physically transformed name, or as `name` when that spelling does not match a column.
Column matches always take precedence over attribute matches.
Use full embeddable paths such as `address.city`; Hibernate does not guess the enclosing embeddable from `city`.

For collection and association join tables, use explicit `\{element}` and `\{key}` markers:

- `\{element}` selects the element or map value; `\{key}` selects a map key.
- `\{element}.address.city` and `\{key}.code` traverse embeddable attributes.
- `\{key}` never means the owner's foreign key or a list index. Reference owner foreign-key columns by column name.

These markers bypass column lookup. To select a literal column named `\{element}.city`, quote the whole identifier:
...
Every attribute or role reference must resolve to one actual column on the declaring table.
A single-column association may identify its local foreign-key column, but paths into associated entities are rejected.
Multi-column values, formulas, and references to other tables are also rejected.
Unknown references fail when Hibernate builds the mappings; naming the constraint or index explicitly does not bypass validation.

The `@CheckConstraint` annotation adds a check constraint to the table.

[source,java]
----
@Entity
@Table(check = @CheckConstraint(name="ValidISBN", constraint="length(isbn)=13"))
class Book { ... }
----

This annotation is commonly used at the field level:

[source,java]
----
@Id
@Column(check = @CheckConstraint(constraint="length(isbn)=13"))
String isbn;
----
