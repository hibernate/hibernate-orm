...
We already mentioned that <<unidirectional-one-to-many,unidirectional `@OneToMany` associations should be avoided>>.
For completeness, we note that `@JoinTable` may also be used with this sort of mapping.
