...
Hibernate comes with an {generator}[annotation processor] which does much more than just this.
...
[[metamodel-generator]]
.*Read more: The changing role of Hibernate Processor*
[%collapsible]
====
****
...
----
/**
 * Static metamodel for {@link org.example.Book}
 **/
@StaticMetamodel(Book.class)
public abstract class Book_ {

	/**
	 * Static metamodel type for {@link org.example.Book}
	 **/
    ...
    /**
     * Static metamodel for attribute {@link org.example.Book#authors}
     **/
...
[TIP]
====
Actually, Hibernate Processor doesn't require that such annotations be applied to the entity class itself.
====
...
Automatic generation of _finder methods_ and _query methods_ is a relatively new feature of Hibernate Processor--originally introduced as an experiment--which ultimately grew into a whole new way to use Hibernate and, eventually, a whole new Jakarta specification.

[IMPORTANT]
====
The rest of this chapter is optional.
If you're happy with the approach taken by Jakarta Data, you're welcome to skip it completely, and move on to <<tuning-and-performance>>.

The functionality we’re about to describe was developed before Jakarta Data took on its current shape, and directly triggered the apocalypse which lead to the final form of the specification. Therefore, there’s massive overlap between the functionality described in this chapter, and the functionality available via the Jakarta Data annotations.

For information about the Jakarta Data-centric version of this functionality, please refer to link:{doc-data-repositories-url}[Introducing Hibernate Data Repositories].
====
...
Back in <<organizing-persistence>>, we showed you how to organize persistence logic using a Jakarta Data repository.
...
// On the other hand, Jakarta Data can't do _everything_ described below.
//
...
interface Queries {
    @HQL("where title like :title and type = :type")
    List<Book> findBooksByTitleAndType(EntityAgent agent, String title, String type);
}
----
...
- `EntityAgent`,
- `Session` or `StatelessSession`, or
...
.Generated Code
----
public class _Queries implements Queries {

    private final @Nonnull EntityManager entityManager;

    public _Queries(@Nonnull EntityManager entityManager) {
...
The generated class `_Queries` now implements the `Queries` interface, and the generated query method implements our abstract method directly.

Of course, the protocol for calling the query method has to change:

[source,java]
----
Queries queries = new _Queries(entityManager);
...
- placing `jakarta.inject` on the build path will cause an `@Inject` annotation to be added to the constructor of `_Queries`, and
...
Even better, a parameter may be of type `Range<T>`, where `P` is the type of the matching field.
...
| `Order<? super E>` | Specifies an entity attribute to order by, if `R` is the entity type returned by the query | Order.asc(Book_.title)
| `List<Order? super E>` +
(or varargs) | Specifies entity attributes to order by, if `R` is the entity type returned by the query | List.of(Order.asc(Book_.title), Order.asc(Book_.isbn))
