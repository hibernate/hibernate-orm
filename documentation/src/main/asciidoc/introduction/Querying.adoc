...
Here we want to see how to execute a query via the `EntityAgent` or `EntityManager` API.
...
- _mutation statements_ modify data and return the number of modified rows.

Selection queries usually start with the keyword `select` or `from`, whereas mutation statements begin with the keyword `insert`, `update`, or `delete`.

.Executing HQL
[%breakable,cols="10,36,14,~"]
|===
| Kind | `EntityHandler` method | Interface | Execution method

| Selection | `createQuery(String,Class)` | `TypedQuery` | `getResultList()`, `getSingleResult()`, or `getSingleResultOrNull()`
| Mutation | `createStatement(String)` | `Statement` | `execute()`
|===

For example, to execute a selection query written in HQL, we might write:

[source,java]
----
List<Book> matchingBooks =
        entityManager.createQuery("from Book where title like :titlePattern", Book.class)
                     .setParameter("titlePattern", titleSearchPattern)
                     .getResultList();
----
...
We've used `getResultList()` because we're expecting our query to return multiple results.
If we're expecting a query to return a single result, we can use `getSingleResult()`.

[source,java]
----
Book book =
        entityManager.createQuery("from Book where isbn = :isbn", Book.class)
...
To execute a mutation statement, we use `execute()`, which returns the number of entities affected by the `insert`, `update`, or `delete`.

[source,java]
----
int deletedBooks =
        entityManager.createStatement("delete from Book where title like :titlePattern")
                     .setParameter("titlePattern", titleSearchPattern)
                     .execute();
----

The `setParameter()` methods specify arguments to query parameters.
...
----
List<Book> matchingBooks =
        entityManager.createQuery("from Book where title like ?1", Book.class)
...
====
_Never_ concatenate user input with HQL and pass the concatenated string to `createQuery()` or `createStatement()`.
...
- by passing the `Type` as a third argument to `setParameter()`, or
- by packaging the argument and its `Type` in a `TypedParameterValue`.

For example, we may pass a <<static-metamodel,static metamodel>> reference to `setParameter()`.

[source,java]
----
entityManager.createQuery("from Person where address = :address")
...
The `TypedQuery` and `Statement` interfaces provide a number of operations for controlling execution of a query or statement.

[source,java]
----
var recentPublications =
        entityAgent.createQuery("from Book where year(book) = year(local date)")
                   .ofType(Book.class)
                   .setTimeout(Timeout.ms(60_000))
                   .setQueryFlushMode(QueryFlushMode.NO_FLUSH)
                   .getResultList();
----

Even better JPA 4 provides for extension via query options and statement options
Hibernate defines several useful options in link:{doc-javadoc-url}/org/hibernate/query/QueryOption.html[QueryOption].

[source,java]
----
import org.hibernate.ReadOnlyMode;
import static org.hibernate.query.QueryOption.*;
...
var recentPublications =
        entityAgent.createQuery("from Book where year(book) = year(local date)")
                   .ofType(Book.class)
                   .addOption(new ResultSetCache("recent-publications"))
                   .addOption(new JdbcFetchSize(100))
                   .addOption(new Comment("Retrieve recent publications"))
                   .addOption(ReadOnlyMode.READ_ONLY)
                   .getResultList();
----
...
But if there are many entities associated with the persistence context, then this can be an expensive operation.

To disable this behavior, set the query flush mode to `NO_FLUSH`:

[source,java]
----
Book bookOrNull =
        entityManager.createQuery("from Book where isbn = ?1", Book.class)
                     .setParameter(1, isbn)
                     .setQueryFlushMode(QueryFlushMode.NO_FLUSH)
                     .getSingleResult();
----
...
[CAUTION]
====
Setting the flush mode to `NO_FLUSH` might cause the query to return stale results.
====
...
But in Hibernate, we can get something much better, a link:{doc-javadoc-url}org/hibernate/query/criteria/HibernateCriteriaBuilder.html[`HibernateCriteriaBuilder`]:
...
The `HibernateCriteriaBuilder` extends `CriteriaBuilder` and adds many operations available in HQL that JPQL doesn't have, including things like:
...
We're ready to create a simple criteria query.
...
if (titlePattern != null) {
    where = builder.and(where, book.get(Book_.title).like(titlePattern));
}
if (namePattern != null) {
    Join<Book,Author> author = book.join(Book_.author);
    where = builder.and(where, author.get(Author_.name).like(namePattern));
}
...
Note that we've explicitly written in the static types of some objects here.
We wouldn't do that in real code.
Readability of criteria queries benefits quite a lot from the use of `var`.

.*Read more: why is the Criteria API so complicated?*
[%collapsible]
====
****
Unlike <<restrictions-and-ordering,restrictions>> in Hibernate or Jakarta Data, Criteria queries can have multiple root entities, and may join the same association multiple times.
That means that the Criteria API must reify each query root as an object and each join as an object.

A query root is an instance of `Root`:

[source,java]
Root<Book> book = query.from(Book.class);

A join is an instance of `Join`:

[source,java]
SetJoin<Book,Author> author = book.join(Book_.authors);

Now, to obtain an expression which can be used to restrict or order the query results, we usually need to resolve a reference to a persistent attribute _in the context_ of a given root or join:

[source,java]
TextExpression authorName = author.get(Author_.name)
TextExpression bookTitle = book.get(Book_.title);

Finally, we can express a restriction or ordering criterion in terms of expressions.

[source,java]
Predicate nameLikePattern = author.get(Author_.name).like(namePattern);
Order byTitle = book.get(Book_.title).asc();

If all this feels a bit overpowered for what you're trying to do, well, we feel ya.
We've addressed this objection both <<restrictions-and-ordering,in Hibernate>> and in Jakarta Data.
The restrictions API in Jakarta Data really is a pleasure to use.
Please try it out!
****
====
...
====
In the query above, we didn't bother treating `titlePattern` and `namePattern` as parameters.
...
| Selection | `createQuery(CriteriaQuery)` | `TypedQuery` | `getResultList()`, `getSingleResult()`, or `getSingleResultOrNull()`
| Mutation  |`createStatement(CriteriaStatement)` | `Statement` | `execute()`
|===
...
delete.where(book.get(Book_.publicationDate).extract(LocalDateField.YEAR).lt(2000));
entityManager.createStatement(delete).execute();
----
...
var query =
        // transform HQL to criteria
        builder.createQuery(Book.class,
                "from Book where year(publicationDate) > 2000");

// modify the resulting criteria query
var root = (Root<Book>) query.getRootList().get(0);
query.where(root.get(Book_.title).like("Hibernate%"))
     .orderBy(root.get(Book_.title).asc(), root.get(Book_.isbn).desc());

// execute the modified query
List<Book> matchingBooks = entityManager.createQuery(query).getResultList();
----
This is starting to get a bit messy.
For problems like this, we can often use <<restrictions-and-ordering,`Restriction`>> instead.

[[criteria-definition]]
.*Read more: A more comfortable way to write criteria queries*
[%collapsible]
====
****
...
CriteriaQuery<Book> query =
        new CriteriaDefinition(entityManagerFactory, Book.class) {{
            select(book);
            if (titlePattern != null) {
                restrict(book.get(Book_.title).like(titlePattern));
            }
            if (namePattern != null) {
                var author = book.join(Book_.author);
                restrict(author.get(Author_.name).like(namePattern));
            }
...
As we said <<orm-or-sql,right up front>>, Hibernate's generated SQL is meant to be used in conjunction with handwritten SQL, and native SQL queries are one of the facilities we provide to make that easy.
...
| Selection |  `createNativeQuery(String,Class)`, `createNativeQuery(String,ResultSetMapping)` | `TypedQuery` | `getResultList()`, `getSingleResult()`, or `getSingleResultOrNull()`
| Mutation | `createNativeStatement(String)` | `Statement` | `execute()`
| Stored procedure | `createStoredProcedureQuery(String)`, `createStoredProcedureQuery(String,Class...)` | `StoredProcedureQuery` | Various options (see Javadoc)
|===

For the most simple cases, Hibernate can infer the shape of the result set from the query result type we provide:
...
String title =
        entityAgent.createNativeQuery("select title from Books where isbn = ?1", String.class)
                   .setParameter(1, isbn)
                   .getSingleResult();
----
...
So for more complicated cases, we need to provide a JPA 4 `ResultSetMapping`.
There are two ways to do this:

- use the `@SqlResultSetMapping` annotation to define a named mapping, and pass the name or static metamodel reference to `createNativeQuery()`, or
- use the new `ResultSetMapping` framework to express the result set mapping in programmatic code.

The first option gets fairly messy, so we don't want to hurt your eyes by showing you an example of it.
The second option is more elegant:
...
...
var authorMapping =
         // mapping for the Author entity
         entity(Author.class,
				// mapping for a fields of Author
                field(Author_.ssn, "auth_ssn"),
				field(Author_.deceased, "auth_dead"),
				// mapping for an embedded object
                embedded(Author_.name,
						 // mappings for fields of Name embeddable
                         field(Name_.first, "auth_first_name"),
                         field(Name_.last, "auth_last_name")));
var authors =
        session.createNativeQuery(
                        """
                        select ssn as auth_ssn,
                               dead as auth_dead,
                               fname as auth_first_name,
                               lname as auth_last_name
                        from authors
                        """,
                        authorMapping)
               .getResultList();
...
Thanks to the use of the static metamodel, this code is very type safe.
...
// ----
// List<Book> books =
//         entityAgent.createNativeQuery("select * from Books", Book.class)
//                    .getResultList();
// ----
//
// There's two ways to ensure the persistence context is flushed before this query is executed.
//
// Either, we could simply force a flush by calling `flush()` or by setting the flush mode to `FLUSH`:
//
// [source,java]
// ----
// List<Book> books =
//         entityAgent.createNativeQuery("select * from Books", Book.class)
//                    .setQueryFlushMode(FLUSH)
//                    .getResultList();
// ----
//
...
// [TIP]
// ====
// We can call stored procedures using `createStoredProcedureQuery()`.
// ====
...
Since Hibernate 7, there's a new option, a very ergonomic API for programmatically adding restrictions or ordering to an existing query before executing it.
...
List<Book> matchingBooks = selection.createQuery(entityAgent).getResultList();
----
...
----
List<Book> books =
        entityAgent.createQuery("from Book where title like ?1 order by title", Book.class)
...
List<Book> books =
        entityAgent.createQuery("from Book where title like ?1 order by title fetch first ?2 rows only", Book.class)
                   .setParameter(1, titlePattern)
...
.Methods for query limits and pagination
...
KeyedResultList<Book> first =
        entityAgent.createQuery(QUERY, Book.class)
...
----
var results =
        entityAgent.createQuery("select isbn, title from Book", Object[].class)
                   .getResultList();
...
var results =
        entityAgent.createQuery("select isbn, title from Book", IsbnTitle.class)
                   .getResultList();
...
var resultList = entityAgent.createQuery(query).getResultList();
...
The `@NamedQuery` and `@NamedStatement` annotations let us define a HQL query or statement that's compiled and checked as part of the bootstrap process.
This means we find out about errors in our queries earlier, instead of waiting until the query is actually executed.
We can place the annotation on any class, even on an entity class.
...
Alternatively, the `@NamedQuery` annotation can be placed on a package or module descriptor.
If we declare a named query at the package or module level, we must call:

[source,java]
----
configuration.addPackage("org.hibernate.example")
// or
configuration.addModule("org.hibernate-example")
----
...
Hibernate Processor can do that for us, with the help of the link:{doc-javadoc-url}org/hibernate/annotations/processing/CheckHQL.html[`@CheckHQL`] annotation, and that's a really good reason to use `@NamedQuery` and `@NamedStatement`.
====

The `@NamedNativeQuery` and `@NamedNativeStatement`  annotations let us do something similar with native SQL queries.
Like `@NamedQuery`, they may be placed on a package or module descriptor.
But there's much less advantage to using `@NamedNativeQuery`, because there's very little Hibernate can do to validate the correctness of a query written in the native SQL dialect of your database.

Whether it's written in HQL or SQL, we execute our named query like this:
...
[%breakable,cols="10,12,36,14,~"]
|===
| Kind | Annotations | `EntityHandler` method | Interface | Execution method

| Selection | `@NamedQuery`, `@NamedNativeQuery` | `createQuery(TypedQueryReference)`, `createNamedQuery(String,Class)` | `TypedQuery` | `getResultList()`, `getSingleResult()`, `getSingleResultOrNull()`
| Mutation | `@NamedStatement`, `@NamedNativeStatement` | `createStatement(Statemen tReference)`, `createNamedStatement(String)` | `Statement` | `execute()`
|===
