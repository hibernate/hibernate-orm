...
| Selection |  `createNativeQuery(String,Class)` | `TypedQuery` | `getResultList()`, `getSingleResult()`, or `getSingleResultOrNull()`
| Mutation | `createNativeStatement(String)` | `Statement` | `execute()`
| Stored procedure | `createStoredProcedureQuery(String)` | `StoredProcedureQuery` | `execute()`
|===
