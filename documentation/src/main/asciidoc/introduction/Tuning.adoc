...
----
List<Book> books =
        entityAgent.createuery("from Book order by isbn")
            .ofType(Book.class)
            .getResultList();
...
| link:{doc-javadoc-url}/org/hibernate/BatchSize.html[`new BatchSize(10)`] | `new BatchSize(10)`
// | `@Fetch(batchSize=10)`, `@BatchSize(10)`
| `hibernate.use_subselect_fetch`
| `true` to enable subselect fetching
| link:{doc-javadoc-url}/org/hibernate/SessionCreationOption.SubselectFetchMode.html[`SubselectFetchMode.ENABLED`] | link:{doc-javadoc-url}/org/hibernate/FetchMethod.html#BULK_SELECT[`FetchMethod.BULK_SELECT`]
// | `@Fetch(SUBSELECT)`
|===
...
var entityManager =
        ...
        entityManagerFactory.createEntityManager(new BatchSize(5));
----
[source,java]
----
import org.hibernate.SessionCreationOption.SubselectFetchMode;
...
var entityManager =
        // create a session with subselect fetching enabled
        entityManagerFactory.createEntityManager(SubselectFetchMode.ENABLED);
----
