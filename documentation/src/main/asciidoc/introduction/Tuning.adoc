...
| link:{doc-javadoc-url}/org/hibernate/SessionCreationOption.BulkSelect.html[`BulkSelect.ENABLED`] | link:{doc-javadoc-url}/org/hibernate/FetchMethod.html#BULK_SELECT[`FetchMethod.BULK_SELECT`]
// | `@Fetch(SUBSELECT)`
|===
...
[source,java]
----
import org.hibernate.SessionCreationOption.BulkSelect;
...
var entityManager =
        // create a session with subselect fetching enabled
        entityManagerFactory.createEntityManager(BulkSelect.ENABLED);
----
...
Note that link:{doc-javadoc-url}/org/hibernate/FetchMethod.html#BULK_SELECT[`FetchMethod.BULK_SELECT`] has the same effect as `FetchMethod.SELECT`, except after execution of a HQL or criteria query.
But after query execution, `BULK_SELECT` is able to much more efficiently fetch associations.
