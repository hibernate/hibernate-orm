...
[%autowidth.stretch]
|===
| Configuration property name | Purpose | Alternative option

| link:{doc-javadoc-url}/org/hibernate/cfg/JdbcSettings.html#STATEMENT_FETCH_SIZE[`hibernate.jdbc.fetch_size`] | The default JDBC fetch size  | link:{doc-javadoc-url}/org/hibernate/query/QueryOption.JdbcFetchSize.html[JdbcFetchSize]
|===
...
| link:{doc-javadoc-url}/org/hibernate/cfg/BatchSettings.html#STATEMENT_BATCH_SIZE[`hibernate.jdbc.batch_size`] | Maximum batch size for SQL statement batching | link:{doc-javadoc-url}/org/hibernate/SessionCreationOption.JdbcBatchSize.html[JdbcBatchSize]
|===
...
// [%breakable,cols="32,~,14,14"]
[%breakable]
|===
| Configuration property name | Property value | Session option | Fetch option

| `hibernate.default_batch_fetch_size`
| A sensible batch size `>1` to enable batch fetching
| link:{doc-javadoc-url}/org/hibernate/SessionCreationOption.FetchBatchSize.html[`FetchBatchSize`] | `BatchSize`
// | `@Fetch(batchSize=10)`, `@BatchSize(10)`
| `hibernate.use_subselect_fetch`
| `true` to enable subselect fetching
| link:{doc-javadoc-url}/org/hibernate/SessionCreationOption.PreferredFetchMethod.html[`PreferredFetchMethod.BY_SUBQUERY`] | link:{doc-javadoc-url}/org/hibernate/FetchMethod.html#BY_SUBQUERY[`FetchMethod.BY_SUBQUERY`]
// | `@Fetch(SUBSELECT)`
|===

A global setting is a blunt instrument, and typically we need more granular control over fetching.
One alternative is to enable batch or subselect fetching in a given session, using a session creation option defined by Hibernate:

[source,java]
----
import org.hibernate.SessionCreationOption.FetchBatchSize;
...
var entityManager =
        // create a session with batch fetching enabled
        entityManagerFactory.createEntityManager(new FetchBatchSize(5));
----
[source,java]
----
...
...
var entityManager =
        // create a session with subselect fetching enabled
        entityManagerFactory.createEntityManager(PreferredFetchMethod.BY_SUBQUERY);
----
...
But this approach is probably still not granular enough.

Since JPA 4, the best way to make use of batch or subselect fetching is via an <<entity-graph,entity graph>>.
JPA provides a standard `FetchOption` for batch fetching:
...
graph.addAttributeNode(Book_.authors)
     .addOption(new BatchFetch(5));
Book book = entityManager.find(graph, bookId);
----

Hibernate defines a custom `FetchOption` which allows more direct choice of association fetching strategy:
...
Note that link:{doc-javadoc-url}/org/hibernate/FetchMethod.html#BY_SUBQUERY[`FetchMethod.BY_SUBQUERY`] has the same effect as `FetchMethod.BY_ID`, except after execution of a HQL or criteria query.
But after query execution, `BY_SUBQUERY` is able to much more efficiently fetch associations.


// [%unbreakable]
// [TIP]
// ====
// We may request subselect fetching even more selectively by annotating a collection or association with the link:{doc-javadoc-url}/org/hibernate/annotations/Fetch.html[`@Fetch`] annotation.
// [source,java]
// ----
...
List<Book> booksWithJoinFetchedAuthors =
        entityAgent.createQuery(query).getResultList();
----
...
This cache is utilized when the entity is retrieved using one of the operations of `Session` which performs <<load-by-natural-id,lookup by natural id>>.
...
At worst, interaction with the cache may be controlled by specifying explicit cache modes:
...
A Hibernate link:{doc-javadoc-url}/org/hibernate/CacheMode.html[`CacheMode`] packages a `CacheRetrieveMode` with a `CacheStoreMode`.
...
The `REFRESH_SESSION` mode specifies that entity state returned by a query should overwrite state held in the persistence context, just as if `refresh()` had been called.

[source,java]
----
List<Publisher> refreshed =
        entityManager.createQuery("from Publisher", Publisher.class)
            .addOption(CacheMode.REFRESH_SESSION)
            .getResultList();
----

Apart from `REFRESH_SESSION`, there's no particular reason to prefer Hibernate's `CacheMode` over the JPA equivalents.
...
The JPA-standard `Cache` interface allows programmatic eviction of cached entities.

[source,java]
----
entityManagerFactory.getCache()
        .evict(Book.class, bookId);
----

Unfortunately, as we've already observed above, JPA provides a quite poor approximation to Hibernate's very granular and flexible second-level cache architecture.
The link:{doc-javadoc-url}org/hibernate/Cache.html[`Cache`] interface allows much more control over programmatic eviction of cached items.

[source,java]
----
entityManagerFactory.getCache()
        .unwrap(Cache.class)
        .evictEntityData(Book.class, bookId);
----
...
An arguably underappreciated feature of Hibernate is the ability to program with <<bypass-persistence-context,stateless sessions>>, which provide a command-oriented, more bare-metal approach to interacting with the database.
This capability has only recently been formalized in the JPA specification via the addition of the `EntityAgent` interface.
...
Use of a stateless session alleviates the need to call `clear()` or `detach()` to perform first-level cache management.
...
Therefore, the `get()`, `find()`, `lock()`, and `refresh()` methods accept a link:{doc-javadoc-url}/org/hibernate/LockMode.html[`LockMode`] as an option.
Here's the simplest way to execute a `select ... for update` in Hibernate:

[source,java]
Book book = session.get(Book.class, isbn, LockMode.PESSIMISTIC_WRITE);
...
The statistics are exposed by the link:{doc-javadoc-url}org/hibernate/stat/Statistics.html[`Statistics`] object exposed by `SessionFactory`:
...
Column terms follow the <<constraint-column-references,same resolution rules as unique constraints>>: logical column name, then physical column name, then an entity attribute path.
Explicit `\{key}` and `\{element}` references on collection/join tables instead select the collection role directly.
An optional `ASC` or `DESC` applies to the resolved column.
