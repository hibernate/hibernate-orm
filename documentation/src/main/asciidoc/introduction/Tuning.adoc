...
| link:{doc-javadoc-url}/org/hibernate/SessionCreationOption.FetchBatchSize.html[`new FetchBatchSize(10)`] | `new BatchSize(10)`
...
Use of a stateless session alleviates the need to call:

- `clear()` or `detach()` to perform first-level cache management, and
- `setCacheStoreMode()` and `setCacheRetrieveMode()` to bypass interaction with the second-level cache.
