...
| Grouping and tuple instantiation | Fencing | `( ... )` and `( ..., ..., ... )`
| Absolute value and norm | Fencing | `\| ... \|` and `\|\| ... \|\|`
| Case lists |  | `case ... end`
| Member reference | Binary infix | `a.b`
| Function application | Postfix | `f(x,y)`
| Indexing | Postfix | `a[i]`
| Exponentiation | Binary infix | `^`
...
The basic SQL arithmetic operators, `+`,`-`,`*`, and `/` are joined by the remainder operator `%` and the exponentiation operator `^`.
...
The absolute value operator is a synonym for the function `abs()`:

[source, hql]
----
select |p.x-q.x|, |p.y-q.y| from Point p, Point q
----
...
:vector-doc: {doc-user-guide-url}#vector-module

link:{vector-doc}[Hibernate Vector] is a new add-on package for Hibernate which supplies support for vector data types.
It adds the following functions and operators to HQL:

[[vector-module-functions-overview]]
[cols="25,^15,~,^15"]
|===
| Function | Operator | Purpose | JPA standard

||`x + y`| Vector addition | ✖
||`x - y`| Vector subtraction | ✖
||`x * y`| Elementwise multiplication | ✖
| `norm(x)` | `\|\|x\|\|` or `\|\|x\|\|2` | https://en.wikipedia.org/wiki/Euclidean_space#Euclidean_norm[Euclidean norm] | ✖
| `euclidean_distance(x,y)` or `l2_distance(x,y)` | `\|\|x-y\|\|` or `\|\|x-y\|\|2` | https://en.wikipedia.org/wiki/Euclidean_distance[Euclidean metric] | ✖
| `taxicab_distance(x,y)` or `l1_distance(x,y)` | `\|\|x-y\|\|1` | https://en.wikipedia.org/wiki/Taxicab_geometry["Manhattan" or "taxicab" metric] | ✖
| `cosine_distance(x,y)` | | https://en.wikipedia.org/wiki/Cosine_similarity[Cosine distance] | ✖
| `dot(x,y)` | | https://en.wikipedia.org/wiki/Dot_product[Euclidean scalar or "dot" product] | ✖
| `dim(x)` | | The dimension of a vector | ✖
|===
****
