...
- `HibernatePersistenceConfiguration` for configuration and bootstrap.
...
protected void setUp() {
    ...
    sessionFactory =
            new HibernatePersistenceConfiguration( "hibernate-tutorial-annotations" )  <1> <2>
                    .managedClass(Event.class)        <3>
                    .createEntityManagerFactory();    <4>
}
----
<1> The `setUp()` method first creates a Hibernate-specific persistence configuration.
<2> Here we put all configuration information in `hibernate.properties`, so there's not much interesting to see.
<3> Here we have only one entity class to register.
<4> The final step in the bootstrap process is to build a `SessionFactory` for the configured services and validated domain model.
