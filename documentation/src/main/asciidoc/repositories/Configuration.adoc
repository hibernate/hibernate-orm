...
| `jakarta.data:jakarta.data-stateful` | Jakarta Data Stateful Repositories API
...
* In Gradle, you'll need to use `annotationProcessor`.
+
[source,groovy,subs="attributes+"]
----
annotationProcessor 'org.hibernate.orm:hibernate-processor:{fullVersion}'
----
* In Maven, you will need to configure the `maven-compiler-plugin`.
+
...
[source,java]
----
Library library = new _Library(statelessSession);
----
...
- Jakarta Bean Validation constraint annotations,
...
- security annotations from the `jakarta.annotation.security` package: `@DenyAll`, `@PermitAll`, and `@RolesAllowed`.
