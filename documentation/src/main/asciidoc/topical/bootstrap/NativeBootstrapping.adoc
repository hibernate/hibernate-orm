...
`StandardServiceRegistryBuilder` javadoc for full details.  Some specific methods of interest:
...
`MetadataSources` has many other methods as well; explore its API and javadoc for more information.  Also,
...
over the `Metadata` building process.  See its javadoc for full details.
...
`Metadata#getSessionFactoryBuilder`.  See the `SessionFactoryBuilder` javadoc for details of the control it allows
