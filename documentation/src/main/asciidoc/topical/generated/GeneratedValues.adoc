...
Let's also add an annotation for tracking the username who created or last modified the entity. A generator annotation
type can define arbitrary attributes which can be accessed from within the `initialize()` method of the generation
implementation. For the `@ModifiedBy` annotation we leverage that to make the generation timing configurable by the user.
...
public @interface ModifiedBy {
    GenerationTiming when() default GenerationTiming.ALWAYS;
}
...
public class ModifiedByValueGeneration
    ...
    public void initialize(ModifiedBy annotation, Class<?> propertyType) {
        generationTiming = annotation.when();
    }
...
public class Document {
    ...
    @ModifiedBy(when = GenerationTiming.INSERT)
