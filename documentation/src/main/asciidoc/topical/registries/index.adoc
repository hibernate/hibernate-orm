...
Components which require a +SessionFactory+ are owned directly by the factory
instead of being stored in a child service registry.  Replaceable strategies or
factories are discovered as Java services while preparing factory construction.


==== EventListenerRegistry

+org.hibernate.event.service.spi.EventListenerRegistry+ is owned by the
factory's +EventEngine+.  It manages and exposes all of Hibernate's event
listeners.  A major use-case for +Integrators+ is to alter the listener
registry.
...
collector portion, if you will.  Its factory may be registered as a Java
service.
...
Or we can tell it to load settings from properties file:

[source,java]
----
StandardServiceRegistry serviceRegistry = new StandardServiceRegistryBuilder()
        .loadProperties( "com/acme/hibernate.properties" )
        .build();
----
