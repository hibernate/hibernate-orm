...
+
This defines a global setting, which can then be controlled per parameter via `org.hibernate.query.procedure.spi.ParameterRegistration#enablePassingNulls(boolean)`
+
