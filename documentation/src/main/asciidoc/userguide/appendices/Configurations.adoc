...
|`hibernate.jdbc.use_string_for_clob` |`true` or `false` (default value) |

By default, the `Dialect#useInputStreamToInsertBlob()` option is used to save Clob object types.

`org.hibernate.dialect.PostgreSQL81Dialect#useInputStreamToInsertBlob()` does not use Clob binary streaming by default,
and that's where the `hibernate.jdbc.use_string_for_clob` property can be used
to save the Clob using the `org.hibernate.type.descriptor.sql.ClobTypeDescriptor#STRING_BINDING`, so, instead of saving an OID,
the actual `String` value is saved in the `text` column.
