...
Applications can also extend (add new `BasicType` registrations) or override (replace an existing `BasicType` registration) using a
`TypeContributor` during bootstrap.
...
[source, java,, indent=0]
----
include::{sourcedir}/basic/bitset/BitSetTypeContributor.java[tags=basic-custom-type-register-BasicType-example]
----
====
