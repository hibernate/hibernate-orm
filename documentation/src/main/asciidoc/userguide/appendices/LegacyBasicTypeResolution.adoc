...
`MutabilityPlan` aspects within the `BasicType` contract.  To influence this mapping, users needed to specify the
'BasicType' or `UserType` (which gets wrapped in a `BasicType`) to use instead

That said, both `BasicType` and `UserType` continue to be fully supported.  See <<basic-legacy>> for details.

Hibernate's legacy strategy for resolving a basic type is based on finding the implementation
of `org.hibernate.type.BasicType` to use.  Internally, a registry of basic types is used.  The
`BasicType` references are registered under String keys.  The following tables enumerate the
different built-in `BasicType` implementations, and their registration keys.
