...
`hibernate.cache.check_second_level_cache_polymorphic_proxies`::
    If `true`, forces Hibernate to check the second-level cache for polymorphic entities prior to creating a proxy.
    This is enabled by default, but can be disable as it has a performance impact to check the second-level cache.
