...
then we can annotate the association with the `orphan-removal` attribute and dissociating the child will trigger a delete statement on the actual child table row as well.
