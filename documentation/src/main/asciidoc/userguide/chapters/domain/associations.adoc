...
To preserve synchronicity between both sides, it's good practice to provide helper methods for adding or removing associations.
...
If for a bidirectional `@OneToMany` association performs better when removing or changing elements order,
the `@ManyToMany` relationship cannot benefit from such an optimization, unless it is replaced by two bidirectional `@OneToMany` associations.
...
The link table has an associated entity, which controls the relationship for both sides that need to be joined.
...
Both the `Person` and the `Address` have a` mappedBy` `@OneToMany` side, while the `PersonAddress` own both the `person` and the `address` associations.
The helper methods are still required by the bidirectional associations.

[NOTE]
====
The aforementioned example uses a Hibernate specific mapping for the link entity, since JPA doesn't allow building a composite identifier directly out of multiple `@ManyToOne` associations.
...
There is only one delete statement executed because, this time, the association is controlled by the `@ManyToOne` side,
which only has to monitor the state of the underlying `FOREIGN KEY` relationship.
