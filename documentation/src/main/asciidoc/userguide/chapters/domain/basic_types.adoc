...
`BasicValueConverter`:: Describes the conversions that need to occur when reading from or writing to the database.
      It can be influenced `AttributeConverter`.  Some Java types (enums, e.g.) have an implicit conversion
...
extending the Hibernate type system.  Fpr more information, see <<basic-type-contributor>>.


See <<basic-bitset-java-type>> examples of most of these approaches, using the mapping of `BitSet`.
