...
By default, Hibernate uses the https://docs.oracle.com/javase/8/docs/api/java/sql/PreparedStatement.html#setTimestamp-int-java.sql.Timestamp-[`PreparedStatement.setTimestamp(int parameterIndex, java.sql.Timestamp)`] or
...
When the time zone is not specified, the JDBC driver uses the underlying JVM default time zone, which might not be suitable if the application is used from all across the globe.
...
With this configuration property in place, Hibernate calls the https://docs.oracle.com/javase/8/docs/api/java/sql/PreparedStatement.html#setTimestamp-int-java.sql.Timestamp-java.util.Calendar-[`PreparedStatement.setTimestamp(int parameterIndex, java.sql.Timestamp, Calendar cal)`] or
...
In the following example, the `java.util.Period` is mapped to a `VARCHAR` database column.
...
A mutable `Object` allows you to modify its internal structure, and Hibernate dirty checking mechanism propagates the change to the database:
...
When persisting a `Product` entity, Hibernate quotes all identifiers as in the following example:
...
When the `Person` entity is persisted, Hibernate fetches the calculated `fullName` column from the database,
...
Hibernate fetches the calculated `fullName` column from the database after the entity is modified.
...
When the `Person` entity is persisted, Hibernate populates the `createdBy` column with the currently logged user.
...
The same goes when the `Person` entity is updated.
Hibernate populates the `updatedBy` column with the currently logged user.
...
When the `Event` entity is persisted, Hibernate populates the underlying `timestamp` column with the current JVM timestamp value:
...
When the `Bid` entity is persisted, Hibernate populates the underlying `updated_on` column with the current JVM timestamp value:
...
When updating the `Bid` entity, Hibernate modifies the `updated_on` column with the current JVM timestamp value:
...
When loading the `Account` entity, Hibernate calculates the `interest` property using the configured `@Formula`:
...
When executing an `Account` entity query, Hibernate filters out all records that are not active.
...
When fetching the `debitAccounts` or the `creditAccounts` collections, Hibernate applies the `@Where` clause filtering criteria to the associated child entities.
...
When fetching the `currentWeekReaders` collection, Hibernate finds one entry:
...
By default, without explicitly enabling the filter, Hibernate fetches all `Account` entities.
If the filter is enabled and the filter parameter value is provided,
then Hibernate applies the filtering criteria to the associated `Account` entities.
...
This way, when fetching the `accounts` collections, Hibernate applies the `@Filter` clause filtering criteria to the associated collection entries.
...
This way, when fetching the `accounts` collections, Hibernate applies the `@FilterJoinTable` clause filtering criteria to the associated collection entries.
...
Hibernate applies the right table aliases to the filter predicates:
