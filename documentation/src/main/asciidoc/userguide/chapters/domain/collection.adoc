...
A unidirectional map controls exposes a parent-child association from the parent-side only.
...
[source,java]
----
include::{sourcedir}/collection/UnidirectionalMap.java[]
----

[source,sql]
----
include::{sourcedir}/collection/UnidirectionalMap.sql[]
----
====
...
Like most bidirectional associations, the relationship can be owned by the child-side, while the parent can propagate entity state transitions to the child entity too.
In the following example, you can see that `@MapKeyEnumerated` was used for associating the `Phone` type enum as a map key.

.Bidirectional Map
====
[source,java]
----
include::{sourcedir}/collection/BidirectionalMap.java[]
----

[source,sql]
----
include::{sourcedir}/collection/BidirectionalMap.sql[]
----
====
...
When it comes to arrays, there is quite a difference between Java arrays and relational database array type (e.g. VARRAY, ARRAY).
First, now all database systems implement the SQL-92 standard ARRAY type, and, for this reason, Hibernate doesn't support native database array types.
Second, Java arrays are relevant for basic types only, since storing Embeddable or Entity types should always be done using the Java Collection API.
...
[source,java]
----
include::{sourcedir}/collection/BaseTypeBinaryArray.java[]
----

[source,sql]
----
include::{sourcedir}/collection/BaseTypeBinaryArray.sql[]
----
====
...
Collections not marked as such require a custom Hibernate `Type`, and all there values must be stored into a single column in the database table.
...
.Comma delimited backed collection
====
[source,java]
----
include::{sourcedir}/collection/CommaDelimitedStringCollection.java[]
----
====

The developer can use the collection like any other collection we've discussed so far, and Hibernate will take care of the transforming part.
...
[source,java]
----
include::{sourcedir}/collection/CommaDelimitedStringCollectionLifecycle.java[]
----

[source,sql]
----
include::{sourcedir}/collection/CommaDelimitedStringCollectionLifecycle.sql[]
----
====
