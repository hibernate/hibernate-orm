...
Hibernate maps a `List` as a persistent list by default.
When no explicit `jakarta.persistence.OrderColumn` is specified, Hibernate uses an implicit order column.

[[NOTE]]
====
Implicit list semantics only affect owned collection mappings.
