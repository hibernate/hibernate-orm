...
Keep in mind that the value of the expression `nullable` of the annotation `@OrderColumn` is conditionally ignored
during the binding process as it is determined for the index column automatically.  If the index column belongs to
the join table it is then force set to be not null, otherwise the value specified is retained.
