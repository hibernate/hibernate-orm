...
`@Bag` can also be placed on a class, `package-info.java`, or `module-info.java` to apply BAG semantics
to all `List` attributes within that scope.  Conversely, `@List` can be used at the same levels
to force LIST semantics (i.e., an ordered, indexed list).
