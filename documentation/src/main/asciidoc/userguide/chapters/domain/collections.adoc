...
In the example above, once the parent entity is persisted, the child entities are persisted as well.
...
The mapping is similar with the <<collections-bidirectional-bag>> example, just that the parent side is annotated with either `@OrderBy` or `@OrderColumn`.
...
When using the `@OrderColumn` annotation, the `order_id` column is embedded in the child table:
...
When inserting two `Phone` records, Hibernate starts the List index from 100 this time.
