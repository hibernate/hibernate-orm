...
By default, the ordering of a list is not maintained.
Use `@OrderColumn` to explicitly persist element positions, or declare `@DefaultListSemantics(LIST)`
on a package or module to use an implicit order column for eligible list attributes.
...
For ordinary `List` mappings, Hibernate determines the classification in this order:

. `@Bag` means `BAG`.
. Collection-identifier metadata such as `@CollectionId` means `ID_BAG`.
. Explicit list-index metadata such as `@OrderColumn` or `@ListIndexBase` means `LIST`.
. `@OrderBy` or `@SQLOrder` means `BAG`.
. Unowned to-many associations without explicit list-index metadata mean `BAG`.
. Otherwise, consult `@DefaultListSemantics`, falling back to the existing configured default, or `BAG`.

Existing validation of incompatible annotations still applies. For example, `@Bag` cannot be combined
with `@OrderColumn` or `@ListIndexBase`. Custom collection types retain their own classification.

The package annotation takes precedence over the module annotation, and both override the configured default.
A package annotation applies only to that exact package, not to subpackages.
Inherited attributes use their declaring type's package and module; attributes of embeddables use the
embeddable's package and module, even when embedded in an entity in another package.
Use `@DefaultListSemantics(BAG)` to select bag semantics in a scope whose surrounding default is `LIST`.

The `hibernate.mapping.default_list_semantics` setting is deprecated in favor of this annotation and is
scheduled for removal in ORM 9.0. Explicit use of the setting logs a deprecation warning, even when an
annotation overrides it. Until removal, the setting remains a fallback for scopes without the annotation.
The programmatic `MetadataBuilder.applyImplicitListSemantics()` default also remains supported.

[NOTE]
====
Default `LIST` semantics only affect owned collection mappings.
...
explicitly. Both sides of the relationship must be kept in sync for element positions to be updated correctly.
...
As discussed in <<collection-list>>, `@DefaultListSemantics(BAG)` on a package or module selects
bag semantics for eligible lists. An attribute-level `@Bag` overrides a scoped `LIST` default.
...
Ordering may be defined in the following ways:

`@OrderBy`:: the collection is ordered upon retrieval using element state
`@SQLOrder`:: the collection is ordered upon retrieval using a SQL expression
`@OrderColumn`:: the collection uses a dedicated column to persist element positions

Without explicit list-index metadata, `@OrderBy` and `@SQLOrder` retain bag classification
and do not consult `@DefaultListSemantics`.
For eligible lists without explicit ordering metadata, `@DefaultListSemantics(LIST)` supplies an implicit order column.
