...
Collections of embeddable types are specifically valued collections (as embeddable types are value types). For more information and examples, check out <<chapters/domain/collections.adoc#collections-elemental,Collections of value types>>.

[[embeddable-mapkey]]
==== Embeddable type as a Map key

Embeddable types can also be used as `Map` keys.
This topic is converted in detail in <<chapters/domain/collections.adoc#collections-map-mapping,Map - key>>.
...
[[embeddable-mapping-collections]]
