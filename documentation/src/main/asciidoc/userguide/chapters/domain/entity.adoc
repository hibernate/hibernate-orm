...
then the two `Book` object references would not be equal since their references are different.
...
By default, when it needs to use a proxy instead of the actual Pojo, Hibernate uses a Bytecode manipulation library like
...
When loading the `Book` entity proxy, Hibernate proxies the `Identifiable` interface instead as illustrated by the following example:
...
Both the `Cuisine` entity and the `Country` embeddable types are instantiated as Java dynamic proxies,
