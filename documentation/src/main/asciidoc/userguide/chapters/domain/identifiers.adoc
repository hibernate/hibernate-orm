...
Explicit identifier-generator names are globally scoped to the persistence unit.
Two different generator declarations may not use the same explicit name.
For an unnamed generator declaration, the JPA entity name is used as the implicit
generator name. Thus, different entities may each declare an unnamed generator
...
1.  using `org.hibernate.id.uuid.UuidGenerator`, which can be configured using the `org.hibernate.annotations.UuidGenerator` annotation. It builds on top of the <<identifiers-generators-IdGeneratorType,@IdGeneratorType>> support.
2.  using `@jakarta.persistence.GeneratedValue(strategy = GenerationType.UUID)`, on a String or UUID typed attribute, or also
`@jakarta.persistence.GeneratedValue(strategy = GenerationType.AUTO)` on a UUID typed attribute.
...
.UUID generation with explicit UUID Version 7 generation strategy
====
[source,java]
----
