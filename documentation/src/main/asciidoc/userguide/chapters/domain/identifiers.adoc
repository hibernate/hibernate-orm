...
If the identifier type is UUID, Hibernate uses an <<identifiers-generators-uuid, UUID identifier>>.

If the identifier type is numerical (e.g. `Long`, `Integer`), then Hibernate uses the `IdGeneratorStrategyInterpreter` to resolve the identifier generator strategy.
...
`FallbackInterpreter`::
	...
	If the underlying database supports sequences, then a SEQUENCE generator is used. Otherwise, a TABLE generator is used instead.
