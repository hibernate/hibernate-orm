...
TODO: Add composite @Id on @ManyToOne like it's the case in the @ManyToMany section
...
[NOTE]
====
For discussion of generated values for non-identifier attributes, see link:#generated[TO DO]
====
...
This is set to true by default, however if applications set this to false the resolutions discussed here will be very different.
