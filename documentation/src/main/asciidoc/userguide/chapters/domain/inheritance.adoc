...
`null`:: If the underlying discriminator column is null, the `null` discriminator mapping is used.
...
and Hibernate will fetch only the entities that are either mapped with
