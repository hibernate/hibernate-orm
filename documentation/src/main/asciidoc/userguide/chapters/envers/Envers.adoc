...
Hibernate generates the following tables using the `hibernate.hbm2ddl.auto` schema tool:
...
all attributes, except for the entity identifier, will be `null`.
...
Once you configured the `ValidityAuditStrategy`, the following schema is automatically generated:
...
Envers inserts a new record in the `REVCHANGES` table with the previous entity class name:
...
For the following entities, Hibernate generates the following database schema:
