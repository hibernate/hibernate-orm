...
The JPA specification allows you to define a default entity listener which is applied for every entity in that particular system.
...
When persisting a `Person` or `Book` entity, the `createdOn` is set by the `onPersist` method of the `DefaultEntityListener`.
...
When updating a `Person` or `Book` entity, the `updatedOn` is set by the `onUpdate` method of the `DefaultEntityListener`.
...
the `createdOn` is not set by the `onPersist` method of the `DefaultEntityListener`
