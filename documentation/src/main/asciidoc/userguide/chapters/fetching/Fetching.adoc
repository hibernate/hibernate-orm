...
Since Hibernate 7.3, a new configuration setting controls which syntax the `LegacyGraphParser` uses:
...
When `@NamedEntityGraph` is placed on a package or module, the `root` attribute **must** be specified.
If the annotation is placed on a package or module and the `root` is omitted:
...
This annotation works in conjunction with the `LegacyGraphParser` and respects the syntax
