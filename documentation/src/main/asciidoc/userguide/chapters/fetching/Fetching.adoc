...
[IMPORTANT]
====
If you forget to JOIN FETCH all EAGER associations, Hibernate issues a secondary select for each and every one of those
...
SELECT::
	The association is fetched lazily using a secondary select for each individual entity,
...
Hibernate avoids the N+1 query issue by generating a single SQL statement to initialize all `employees` collections
...
Hibernate avoids the secondary query by issuing an OUTER JOIN for the `employees` collection.
