...
[TIP]
====
The order in which SQL statements are executed is given by the `ActionQueue2` and not by the order in which entity state operations have been previously defined.
====

The `ActionQueue2` executes all operations in the following order:
