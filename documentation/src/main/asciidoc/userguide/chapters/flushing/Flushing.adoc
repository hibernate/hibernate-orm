...
Because the JPA doesn't impose a strict rule on delaying flushing, when executing a native SQL query, the persistence context is flushed.
...
Even if we remove the first entity and then persist a new one, Hibernate executes the `DELETE` statement after the `INSERT`.
