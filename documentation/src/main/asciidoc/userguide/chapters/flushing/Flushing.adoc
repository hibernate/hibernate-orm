...
[TIP]
====
The order in which SQL statements are executed is given by the `ActionQueueLegacy` and not by the order in which entity state operations have been previously defined.
====

The `ActionQueueLegacy` executes all operations in the following order:
...
[NOTE]
====
There is also a new, currently experimental, implementation of `ActionQueue` based on graph planning named, appropriately, `GraphBasedActionQueue`.  To enable usage of this, specify:

[source,java]
----
hibernate.flush.queue.impl=graph
----
====
