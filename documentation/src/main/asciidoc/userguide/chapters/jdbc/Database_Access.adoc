...
[NOTE]
====
If the Java EE appliction server or JTA transcation manager supports switching from one EJB to another while the transaction gets propagated from the outer EJB to the inner one,
...
If the current `Session` was crated using the
