...
resolved by using a custom `FilterParamResolver`. The custom resolver can be
created as a bean, and in this case it will be retrieved from the context,
or it will be instantiated anew by the filter. The resolver must implement
the interface `FilterParamResolver`.
