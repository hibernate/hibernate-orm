...
[IMPORTANT]
====
Filters apply to entity queries, but not to direct fetching, unless otherwise configured using the `appliedOnFind` flag
...
On the other hand, the `minimumAmount` filter is taken into consideration, because its `appliedOnFind` flag is set to `true`.
