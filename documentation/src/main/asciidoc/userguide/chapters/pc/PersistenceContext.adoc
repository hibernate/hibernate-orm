...
Hibernate's `org.hibernate.annotations.CascadeType.PERSIST` is equivalent to JPA's `javax.persistence.CascadeType.PERSIST`. Both annotations are used
when we are interested in propagating the persist operation.
