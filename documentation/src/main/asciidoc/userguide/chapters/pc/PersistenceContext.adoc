...
When interested in cascading the operation in an association between entities, we can use the `@Cascade`. To do that, all that's need to be done is
...
[source, JAVA, indent=0]
----
include::{sourcedir-caching}/CascadeTest.java[tags=cascade-persist-example]
----
====
