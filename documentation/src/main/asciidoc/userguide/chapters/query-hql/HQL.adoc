...
[source, JAVA, indent=0]
----
include::{sourcedir}/Person.java[tags=jpql-api-named-query-example, indent=0]
...
We see that the query is defining a root entity reference to the `org.hibernate.userguide.hql.Person` object model type.
Additionally, it declares an alias of `p` to that `org.hibernate.userguide.hql.Person` reference, which is the identification variable.
...
[source, JAVA, indent=0]
----
include::{sourcedir}/Phone.java[tags=hql-collection-qualification-example, indent=0]
