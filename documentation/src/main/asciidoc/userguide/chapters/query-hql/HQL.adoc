...
* <<chapters/flushing/Flushing.adoc#flushing,Flushing>>
* <<chapters/fetching/Fetching.adoc#fetching,Fetching>>
...
<<chapters/query-criteria/Criteria.adoc#criteria,Criteria>> for more information.
...
references, see link:#hql-collection-valued-associations[Collection member references].
...
link:#hql-collection-valued-associations[Collection member references]. In the case of
...
See link:#hql-collection-expressions[Collection-related expressions] for additional
...
See link:#hql-from-clause[The `FROM` clause].

=== Path expressions

Again, see link:#hql-from-clause[The `FROM` clause].
...
link:#hql-entity-type-exp[Entity type].
...
See link:#hql-exp-functions[Scalar functions] for details on the `concat()`
function
...
See also link:#hql-collection-qualification[Special case - qualified path expressions] as there is a
...
link:#hql-expressions[Expressions] are all valid select expressions,
except where otherwise noted. See the section
link:#hql-api[Query API] for information on handling the results
...
link:#hql-api[Query API]) here we are wrapping the values in a
...
attributes.
* entity type expressions. See link:#hql-entity-type-exp[Entity type]
...
link:#group_by_illustration[???] example, we retrieved order
...
Flushing is covered in detail in <<chapters/flushing/Flushing.adoc#flushing,Flushing>>. Locking is covered
in detail in <<chapters/locking/Locking.adoc#locking,Locking>>. The concept of read-only state is
covered in <<chapters/pc/PersistenceContext.adoc#pc,Persistence Contexts>>.
...
* [method]+Query#list+ - executes the select query and returns back the list of
results.
* [method]+Query#uniqueResult+ - executes the select query and returns the single
...
the EntityGraph discussions in <<chapters/fetching/Fetching.adoc#fetching,Fetching>>.
