...
[source, java, indent=0]
----
include::{example-dir-hql}/HQLInsertAndUpdateTest.java[tags=hql-update-example]
----
====
...
[source, sql, indent=0]
----
include::{example-dir-hql}/HQLInsertAndUpdateTest.java[tags=hql-insert-example]
----
...
[NOTE]
====
In a literal with an exponent, the `R` is case-insensitive.
...
The first argument is an expression to a JSON document. The second argument is a JSON path as a string expression.

WARNING: Some databases might also allow extracting non-scalar values. Beware that this behavior is not portable.

NOTE: It is recommended to use the dot notation for JSON paths instead of the bracket notation,
