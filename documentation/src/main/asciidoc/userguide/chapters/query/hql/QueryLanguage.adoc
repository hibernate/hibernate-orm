...
- A custom `Dialect` may register additional native functions by overriding `initializeFunctionRegistry()`. <<hql-user-defined-functions-impl, Implementation of this method.>>
...
[[hql-user-defined-functions-impl]]
...
Finally, when `countItemsGreaterVal` function is registered we can use it in our JPQL/HQL queries.
