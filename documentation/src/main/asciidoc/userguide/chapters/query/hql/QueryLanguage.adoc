...
Finally, functions that evaluate the id, version, or natural id of an entity and the key-column of
a to-one foreign-key:
...
| `fk()` | The value of the key-column of a `@ManyToOne` (or logical `@ManyToOne`) attribute.  Mainly useful with <<associations-not-found,`@NotFound` mappings>>
|===
