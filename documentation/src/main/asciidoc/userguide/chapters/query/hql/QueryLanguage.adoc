...
| `timestampadd()` | Add an interval to a datetime | `timestampadd(field, interval, datetime)` | &cross;
| `timestampdiff()` | Extract the interval between two datetimes | `timestampdiff(field, from, to)` | &cross;
|===
...
| `repeat()` | Concatenate a string with itself multiple times | `replace(string, times)` | &cross; / &cross;
