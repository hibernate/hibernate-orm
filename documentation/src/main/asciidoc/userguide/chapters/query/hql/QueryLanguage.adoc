...
==== Aggregate functions and `FILTER`

It's common to have aggregate functions like `count()`, `sum()`, and `max()` in a select list.
The aggregate functions are listed <<hql-aggregate-functions,below>>.

An aggregate function may even have a filter, a sort of mini-`WHERE`-clause applying to just one item of the select list:

[[hql-aggregate-functions-simple-filter-example]]
.Using FILTER with
====
[source, JAVA, indent=0]
----
...
Aggregate functions often appear in queries with a `GROUP BY` clause.
For information on grouping see <<hql-group-by,above>>.
...
All aggregate functions support the inclusion of a _filter clause_.
