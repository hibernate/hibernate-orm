...
association uses a Proxy that will be initialized when being navigated for the first time.
...
To join multiple entities, you need to use a `SqlResultSetMapping` for each entity the SQL query fetches.
...
For this example, the following entities are used:
...
Hibernate resolves the `{h-domain}` placeholder according to the values of the default catalog and schema:
