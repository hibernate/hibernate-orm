...
If the `hibernate.hbm2ddl.auto` configuration is set to `create`, Hibernate generates the following database schema:
...
Hibernate executes the script file after the schema is automatically generated.
...
When the `SessionFactory` is bootstrapped, Hibernate executes the `database-object`, therefore creating the `sp_count_books` function.
...
In the mapping above, both the `name` and `clientId` table columns use a `DEFAULT` value.
...
This way, when omitting the `name` and the `clientId` attribute, the database sets them according to their default values.
