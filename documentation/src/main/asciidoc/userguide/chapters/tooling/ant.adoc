...
[[ant-enhance-enableClientEnhancement]]
===== `*enableClientEnhancement*` =====
This attribute defaults to `false`. Set it to `true` to rewrite client accesses to persistent fields through Hibernate's generated readers and writers.
...
the effective `enableClientEnhancement` option are all `false`, the enhancement task is not executed.
