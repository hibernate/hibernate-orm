...
enableClientEnhancement:: Whether to rewrite client field accesses through Hibernate's generated readers and writers. Defaults to `false`. Targets must be selected for managed enhancement or already be compatibly enhanced. The deprecated `enableExtendedEnhancement` name remains an alias; an explicit new value takes precedence, including `false`. See <<BytecodeEnhancement-client>>.
