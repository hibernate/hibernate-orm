...
<<chapters/bootstrap/Bootstrap.adoc#bootstrap-jpa-compliant,JTA>>
...
TransactionManager, UserTransaction, etc. See <<chapters/jndi/JNDI.adoc#jndi,JNDI>> for
...
Let's take a look at using the Transaction API in the various
environments.
...
are shown in <<chapters/batch/Batching.adoc#batch,Batching>>. Keeping a Session open for the duration
