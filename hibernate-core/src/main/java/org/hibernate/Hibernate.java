 ...
 * <p>
 * Operations like {@link #isInitialized(Object)} and {@link #initialize(Object)} are of general purpose. But
 * {@link #createDetachedProxy(SessionFactory, Class, Object)} and
 * {@link CollectionInterface#createDetachedProxy(SessionFactory, Class, Object)} are intended for use by generic code
 * that must materialize an "amputated" graph of Hibernate entities. (For example, a library which deserializes entities
 * from JSON.)
 * <p>
 * Graphs of Hibernate entities obtained from a {@link Session} are usually in an amputated form, with associations and
 * collections replaced by proxies and lazy collections. (That is, by instances of the internal types
 * {@link HibernateProxy} and {@link PersistentCollection}.) These objects are fully serializable using Java
 * serialization, but can cause discomfort when working with custom serialization libraries. Therefore, this class
 * defines operations that may be used to write code that completely removes the amputated leaves of the graph (the
 * proxies) during serialization, and rematerializes and reattaches them during deserialization. It's possible, in
 * principle, to use these operations, together with reflection, or with the Hibernate metamodel, to write such generic
 * code for any given serialization library, but the details depend on what facilities the library itself offers for the
 * program to intervene in the process of serialization and of deserialization.
 *
 * @author Gavin King
 */
public final class Hibernate {
	...
	/**
	 * Force initialization of a proxy or persistent collection. In the case of a many-valued association, only the
	 * collection itself is initialized. It is not guaranteed that the associated entities held within the collection
	 * will be initialized.
	 *
	 * @param proxy a persistable object, proxy, persistent collection or {@code null}
	 * @throws HibernateException if the proxy cannot be initialized at this time, for example, if the {@code Session}
	 * was closed
	 */
	...
	/**
	 * Obtain the {@linkplain Collection#size() size} of a persistent collection, without fetching its state from the
	 * database.
	 *
	...
	/**
	 * Determine if the given persistent collection {@linkplain Collection#contains(Object) contains} the given element,
	...
	/**
	 * Obtain the value associated with the given key by the given persistent map, without fetching the state of the map
	 * from the database.
	 *
	...
	/**
	 * Obtain the element of the given persistent list with the given index, without fetching the state of the list from
	 * the database.
	 *
	...
	/**
	 * Get the true, underlying class of a proxied persistent class. This operation will initialize a proxy by side
	 * effect.
	 *
	...
	/**
	 * Check if the property is initialized. If the named property does not exist or is not persistent, this method
	...
	/**
	 * If the given object is not a proxy, return it. But, if it is a proxy, ensure that the proxy is initialized, and
	 * return a direct reference to its proxied entity object.
	 *
	 * @param proxy an object which might be a proxy for an entity
	 * @return a reference that is never proxied
	 * @throws LazyInitializationException if this operation is called on an uninitialized proxy that is not associated
	...
	/**
	 * If the given object is not a proxy, cast it to the given type, and return it. But, if it is a proxy, ensure that
	 * the proxy is initialized, and return a direct reference to its proxied entity object, after casting to the given
	 * type.
	 *
	 * @param proxy an object which might be a proxy for an entity
	...
	/**
	 * Obtain a detached, uninitialized reference (a proxy) for a persistent entity with the given identifier. The
	 * returned proxy is not associated with any session, and cannot be initialized by calling
	 * {@link #initialize(Object)}. It can be used to represent a reference to the entity when working with a detached
	 * object graph.
	 *
	...
	public static final class CollectionInterface<C> {
		...
		/**
		 * Obtain a detached, uninitialized persistent collection of the type represented by this object. The returned
		 * wrapper object is not associated with any session, and cannot be initialized by calling
		 * {@link #initialize(Object)}. It can be used to represent an uninitialized collection when working with a
		 * detached object graph.
		 *
	...
	/**
	 * Obtain an instance of {@link CollectionInterface} representing persistent bags of a given element type.
	 *
	...
	/**
	 * Obtain an instance of {@link CollectionInterface} representing persistent sets of a given element type.
	 *
	 * @param <U> the element type
	 * @since 6.0
	 */
	...
	/**
	 * Obtain an instance of {@link CollectionInterface} representing persistent lists of a given element type.
	 *
	 * @param <U> the element type
	 * @since 6.0
	 */
	...
	/**
	 * Obtain an instance of {@link CollectionInterface} representing persistent maps of a given key and value types.
	 *
	...
	/**
	 * Obtain an instance of {@link CollectionInterface} representing sorted persistent sets of a given element type.
	 *
	 * @param <U> the element type
	 * @since 6.0
	 */
	...
	/**
	 * Obtain an instance of {@link CollectionInterface} representing sorted persistent maps of a given key and value
	 * types.
	 *
	 * @param <U> the key type
	 * @param <V> the value type
	...
	/**
	 * Obtain an instance of {@link CollectionInterface} representing persistent collections of the given type.
	 *
