...
public class BulkOperationCleanupAction implements Executable, Serializable {
	...
	public BulkOperationCleanupAction(SharedSessionContractImplementor session, List<EntityDescriptor> affectedEntities) {
		...
		for ( EntityDescriptor persister : affectedEntities ) {
			...
			// see if the entity is cached and if so register a clean-up
			final EntityDataAccess entityDataAccess = factory.getCache()
					.getEntityRegionAccess( rootEntityDescriptor.getHierarchy() );
			if ( entityDataAccess != null ) {
				entityCleanups.add( new EntityCleanup( entityDataAccess ) );
			}

			if ( rootEntityDescriptor.getHierarchy().getNaturalIdDescriptor() != null ) {
				final NaturalIdDataAccess access = factory.getCache()
						.getNaturalIdRegionAccess( rootEntityDescriptor.getHierarchy() );
				if ( access != null ) {
					naturalIdCleanups.add( new NaturalIdCleanup( access ) );
				}
			}

			final Set<String> roles = factory.getTypeConfiguration().getCollectionRolesByEntityParticipant(
			...
			for ( String role : roles ) {
				...
				if ( access != null ) {
					collectionCleanups.add( new CollectionCleanup( access ) );
				}
			}
		}
