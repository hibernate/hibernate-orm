		...
		if ( id instanceof DelayedPostInsertIdentifier ) {
			...
			if ( entry == null ) {
				if ( LOG.isDebugEnabled() ) {
					LOG.debugf(
							"The Persistence Context does not contain any entry for the [%s] entity instance. " +
							"This may occur if an entity is created and then deleted in the same JPA EntityManager or Hibernate Session",
							instance
					);
				}
				// If an Entity is created and then deleted in the same Transaction,
				// when Action#postDelete() calls this method the persistence context does not contain anymore an entry.
				return null;
			}
