...
public class SessionFactoryOptionsBuilder implements SessionFactoryOptions {
	...
	private static Interceptor determineInterceptor(
			Map<String, Object> configurationSettings,
			StrategySelector strategySelector) {
		final Object setting = configurationSettings.get( INTERCEPTOR );
		if ( setting instanceof Interceptor instance ) {
			return instance;
		}
		// class-name strings are handled by determineInterceptorStrategy
		return null;
	}
