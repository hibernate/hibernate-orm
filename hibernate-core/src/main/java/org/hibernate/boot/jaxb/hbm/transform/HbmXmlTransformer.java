...
import org.hibernate.boot.jaxb.mapping.spi.JaxbHbmAnyDiscriminatorValueMappingImpl;
...
import org.hibernate.boot.jaxb.mapping.spi.JaxbToOneAttribute;
...
public class HbmXmlTransformer {
	...
	private void transferEntities() {
		...
		for ( JaxbHbmRootEntityType hbmClass : hbmXmlMapping.getClazz() ) {
			processRootEntity( hbmClass );
		}

		for ( JaxbHbmDiscriminatorSubclassEntityType hbmSubclass : hbmXmlMapping.getSubclass() ) {
			processDiscriminatedSubclass( hbmSubclass );
		}

		for ( JaxbHbmJoinedSubclassEntityType hbmSubclass : hbmXmlMapping.getJoinedSubclass() ) {
			processJoinedSubclass( hbmSubclass );
		}

		for ( JaxbHbmUnionSubclassEntityType hbmSubclass : hbmXmlMapping.getUnionSubclass() ) {
			processUnionSubclass( hbmSubclass );
		}

	}

	private void processRootEntity(JaxbHbmRootEntityType hbmClass) {
		final JaxbEntityImpl jaxbEntity = new JaxbEntityImpl();
		// hbm mappings are always complete
		jaxbEntity.setMetadataComplete( true );
		ormRoot.getEntities().add( jaxbEntity );

		// determine if it is a typed or dynamic class
		if ( StringHelper.isEmpty( hbmClass.getName() ) ) {
			// no class name == dynamic
			if ( StringHelper.isEmpty( hbmClass.getEntityName() ) ) {
				throw new MappingException( "Assumed dynamic entity did not define entity-name", origin );
			}
			jaxbEntity.setName( hbmClass.getEntityName() );
		}
		else {
			jaxbEntity.setClazz( hbmClass.getName() );
			jaxbEntity.setName( hbmClass.getEntityName() );
		}

		transferRootEntity( hbmClass, jaxbEntity );
	}

	private void processDiscriminatedSubclass(JaxbHbmDiscriminatorSubclassEntityType hbmSubclass) {
		final JaxbEntityImpl jaxbEntity = new JaxbEntityImpl();
		ormRoot.getEntities().add( jaxbEntity );

		if ( StringHelper.isEmpty( hbmSubclass.getName() ) ) {
			// no class name == dynamic
			if ( StringHelper.isEmpty( hbmSubclass.getEntityName() ) ) {
				throw new MappingException( "Assumed dynamic entity did not define entity-name", origin );
			}
			jaxbEntity.setName( hbmSubclass.getEntityName() );
		}
		else {
			jaxbEntity.setClazz( hbmSubclass.getName() );
			jaxbEntity.setName( hbmSubclass.getEntityName() );
		}

		transferDiscriminatorSubclass( hbmSubclass, jaxbEntity );
	}

	private void processJoinedSubclass(JaxbHbmJoinedSubclassEntityType hbmSubclass) {
		final JaxbEntityImpl jaxbEntity = new JaxbEntityImpl();
		ormRoot.getEntities().add( jaxbEntity );
		...
		transferJoinedSubclass( hbmSubclass, jaxbEntity );
	}

	private void processUnionSubclass(JaxbHbmUnionSubclassEntityType hbmSubclass) {
		final JaxbEntityImpl jaxbEntity = new JaxbEntityImpl();
		ormRoot.getEntities().add( jaxbEntity );
		...
		transferUnionSubclass( hbmSubclass, jaxbEntity );
	}
	...
	private void transferRootEntity(JaxbHbmRootEntityType hbmClass, JaxbEntity entity) {
		...
		for ( JaxbHbmNamedNativeQueryType hbmQuery : hbmClass.getSqlQuery() ) {
			entity.getNamedNativeQueries().add( transformNamedNativeQuery(
					hbmQuery,
	...
	private void transferBaseEntityInformation(JaxbHbmEntityBaseDefinition hbmClass, JaxbEntity jaxbEntity) {
		// hbm mappings are always complete
		jaxbEntity.setMetadataComplete( true );

		transfer( hbmClass::getEntityName, jaxbEntity::setName );
		transfer( hbmClass::getName, jaxbEntity::setClazz );

		if ( hbmClass instanceof Discriminatable ) {
			final Discriminatable discriminatable = (Discriminatable) hbmClass;
			transfer( discriminatable::getDiscriminatorValue, jaxbEntity::setDiscriminatorValue );
		}
		...
		transfer( hbmClass::isLazy, jaxbEntity::setLazy );
		transfer( hbmClass::getProxy, jaxbEntity::setProxy );

		transfer( hbmClass::getBatchSize, jaxbEntity::setBatchSize );

		transfer( hbmClass::isDynamicInsert, jaxbEntity::setDynamicInsert );
		transfer( hbmClass::isDynamicUpdate, jaxbEntity::setDynamicUpdate );
		transfer( hbmClass::isSelectBeforeUpdate, jaxbEntity::setSelectBeforeUpdate );

		transferToolingHints( hbmClass );
		transferResultSetMappings( jaxbEntity.getName(), hbmClass );
	}
	...
	private void transferJoinedSubclass(JaxbHbmJoinedSubclassEntityType hbmSubclass, JaxbEntity subclassEntity) {
		...
		if ( !hbmSubclass.getJoinedSubclass().isEmpty() ) {
			for ( JaxbHbmJoinedSubclassEntityType nestedHbmSubclass : hbmSubclass.getJoinedSubclass() ) {
				processJoinedSubclass( nestedHbmSubclass );
			}
		}
	}
	...
	private void transferAttributes(List hbmAttributeMappings, JaxbAttributesContainer attributes) {
		for ( Object hbmAttributeMapping : hbmAttributeMappings ) {
			if ( hbmAttributeMapping instanceof JaxbHbmBasicAttributeType ) {
				...
				final JaxbHbmCompositeAttributeType hbmComposite = (JaxbHbmCompositeAttributeType) hbmAttributeMapping;
				ormRoot.getEmbeddables().add( convertEmbeddable( hbmComposite ) );
				attributes.getEmbeddedAttributes().add( transformEmbedded( hbmComposite ) );
			}
				...
				attributes.getAnyMappingAttributes().add( transformAnyMappingAttribute( any ) );
...
//		}
	...
	private JaxbAnyMappingImpl transformAnyMappingAttribute(JaxbHbmAnyAssociationType source) {
		...
		source.getMetaValue().forEach( (sourceMapping) -> {
			final JaxbHbmAnyDiscriminatorValueMappingImpl mapping = new JaxbHbmAnyDiscriminatorValueMappingImpl();
	...
	private JaxbElementCollectionImpl transformElementCollection(final PluralAttributeInfo source) {
		...
		if ( source.getElement() != null ) {
			...
			jaxbEmbeddable.setClazz( source.getCompositeElement().getClazz() );
	...
	private void transferIdentifier(JaxbHbmRootEntityType source, JaxbEntity target) {
		if ( source.getId() != null ) {
			...
			if ( isAggregate ) {
				...
				final JaxbEmbeddableImpl jaxbEmbeddable = new JaxbEmbeddableImpl();
				jaxbEmbeddable.setClazz( hbmCompositeId.getClazz() );
				jaxbEmbeddable.setAttributes( new JaxbEmbeddableAttributesContainerImpl() );
				for ( Object hbmCompositeAttribute : hbmCompositeId.getKeyPropertyOrKeyManyToOne() ) {
					if ( hbmCompositeAttribute instanceof JaxbHbmCompositeKeyBasicAttributeType ) {
						...
						jaxbEmbeddable.getAttributes().getBasicAttributes().add( basic );
					}
					else {
						...
						jaxbEmbeddable.getAttributes().getManyToOneAttributes().add( manyToOne );
					}
				}
				ormRoot.getEmbeddables().add( jaxbEmbeddable );
			}
...
//		}
	...
	private void transferNaturalIdentifiers(JaxbHbmRootEntityType source, JaxbEntity target) {
		...
		transferAttributes(
				...
				new JaxbAttributesContainerImpl() {
					@Override
	...
	private void transferUnionSubclass(JaxbHbmUnionSubclassEntityType hbmSubclass, JaxbEntity subclassEntity) {
		...
		if ( !hbmSubclass.getUnionSubclass().isEmpty() ) {
			for ( JaxbHbmUnionSubclassEntityType nestedHbmSubclass : hbmSubclass.getUnionSubclass() ) {
				processUnionSubclass( nestedHbmSubclass );
			}
		}
	}
		...
		if ( isNotEmpty( s ) ) {
			...
			for ( String hbmCascade : split ) {
				...
				if (hbmCascade.contains( "save-update" ) ) {
					cascadeType.setCascadeSaveUpdate( new JaxbEmptyTypeImpl() );
				}
				...
				if (hbmCascade.contains( "delete" ) ) {
					cascadeType.setCascadeDelete( new JaxbEmptyTypeImpl() );
				}
			}
		}
		return cascadeType;
