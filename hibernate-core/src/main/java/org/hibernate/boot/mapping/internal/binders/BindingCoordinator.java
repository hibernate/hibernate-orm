...
public class BindingCoordinator {
	...
	private void coordinateBinding() {
		...
		PrimaryKeyNaming.finish( bindingState );
	}
	...
	private void applyFallbackJdbcTypeContributions() {
		final var typeConfiguration = bindingState.getTypeConfiguration();
		...
		if ( bindingState.getMetadataBuildingContext().getBuildingPlan().getWrapperArrayHandling()
			...
			typeConfiguration.getBasicTypeRegistry().addTypeReferenceRegistrationKey(
		...
		addFallbackIfNecessary( SqlTypes.INET, SqlTypes.VARBINARY );
		addFallbackIfNecessary( SqlTypes.GEOMETRY, SqlTypes.VARBINARY );
		addFallbackIfNecessary( SqlTypes.POINT, SqlTypes.VARBINARY );
		addFallbackIfNecessary( SqlTypes.GEOGRAPHY, SqlTypes.GEOMETRY );

		addFallbackIfNecessary( SqlTypes.MATERIALIZED_BLOB, SqlTypes.BLOB );
		addFallbackIfNecessary( SqlTypes.MATERIALIZED_CLOB, SqlTypes.CLOB );
		addFallbackIfNecessary( SqlTypes.MATERIALIZED_NCLOB, SqlTypes.NCLOB );
	}

	private void adaptToPreferredSqlTypeCode(
		...
		final var basicType = new NamedBasicTypeImpl<>(
				...
				typeConfiguration.getJdbcTypeRegistry().getDescriptor( preferredSqlTypeCode ),
				name
		);
	...
	private void addFallbackIfNecessary(int typeCode, int fallbackTypeCode) {
	...
	private MappedSuperclass registerMappedSuperclassShell(ClassDetails type) {
		if ( !type.isRealClass() ) {
			return null;
		}
		...
		final MappedSuperclass superMappedSuperclass = locateSuperMappedSuperclass( type.getSuperClass() );
	...
	private MappedSuperclass locateSuperMappedSuperclass(ClassDetails type) {
		for ( ClassDetails candidate = type;
				candidate != null && candidate != ClassDetails.OBJECT_CLASS_DETAILS;
				candidate = candidate.getSuperClass() ) {
			if ( candidate.hasDirectAnnotationUsage( jakarta.persistence.MappedSuperclass.class ) ) {
				return registerMappedSuperclassShell( candidate );
			}
			if ( candidate.hasDirectAnnotationUsage( jakarta.persistence.Entity.class ) ) {
				return null;
			}
		}
		return null;
	...
	private void runAssociationIdentifierPhase(List<ManagedTypeBinder> binders, boolean strict) {
		...
		do {
			...
			for ( ManagedTypeBinder binder : binders ) {
				if ( binder instanceof TypeBindingPhase.AssociationIdentifiers associationIdentifiers ) {
					processedAny |= associationIdentifiers.bindAssociationIdentifiers( strict );
				}
			}
		}
		while ( processedAny );
