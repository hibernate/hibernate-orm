...
public class EntityTypeBinder extends IdentifiableTypeBinder
	...
	public void bindTypeSkeleton() {
		...
		if ( !entityName.equals( importName ) ) {
			getBindingState().addImport( entityName, entityName );
		}
	}
	...
	public boolean bindAssociationIdentifiers(boolean strict) {
		return new AssociationIdentifierBinder( this, strict ).bindAssociationIdentifiers();
	}
	...
	private static void applyResolvedGenericType(org.hibernate.mapping.Value value, TypeDetails resolvedType) {
		final String typeName = resolvedType.getName();
	...
	private void processJpaEventCallbacks(
		...
		if ( prePersistMethod != null && appliesToTarget( style, prePersistMethod, methodArgumentType ) ) {
		...
		if ( postPersistMethod != null && appliesToTarget( style, postPersistMethod, methodArgumentType ) ) {
		...
		if ( preInsertMethod != null && appliesToTarget( style, preInsertMethod, methodArgumentType ) ) {
		...
		if ( postInsertMethod != null && appliesToTarget( style, postInsertMethod, methodArgumentType ) ) {
		...
		if ( preUpdateMethod != null && appliesToTarget( style, preUpdateMethod, methodArgumentType ) ) {
		...
		if ( postUpdateMethod != null && appliesToTarget( style, postUpdateMethod, methodArgumentType ) ) {
		...
		if ( preUpsertMethod != null && appliesToTarget( style, preUpsertMethod, methodArgumentType ) ) {
		...
		if ( postUpsertMethod != null && appliesToTarget( style, postUpsertMethod, methodArgumentType ) ) {
		...
		if ( preRemoveMethod != null && appliesToTarget( style, preRemoveMethod, methodArgumentType ) ) {
		...
		if ( postRemoveMethod != null && appliesToTarget( style, postRemoveMethod, methodArgumentType ) ) {
		...
		if ( preDeleteMethod != null && appliesToTarget( style, preDeleteMethod, methodArgumentType ) ) {
		...
		if ( postDeleteMethod != null && appliesToTarget( style, postDeleteMethod, methodArgumentType ) ) {
		...
		if ( preMergeMethod != null && appliesToTarget( style, preMergeMethod, methodArgumentType ) ) {
		...
		if ( postLoadMethod != null && appliesToTarget( style, postLoadMethod, methodArgumentType ) ) {
	...
	private static boolean appliesToTarget(
			JpaEventListenerStyle style,
		...
		return callbackMethod.getArgumentTypes().get( 0 ).toJavaClass().isAssignableFrom( methodArgumentType );
	}
	...
	private static void bindCacheable(
		...
		final Cache cacheAnn = managedType.getClassDetails().getDirectAnnotationUsage( Cache.class );
		if ( cacheAnn != null && managedType.getHierarchy().getRoot() != managedType ) {
			final String className = typeBinding.getClassName() == null
					? managedType.getClassDetails().getName()
					: typeBinding.getClassName();
		...
		if ( cacheableAnn != null
				...
				&& cacheAnn == null ) {
			typeBinding.setCached( false );
			return;
		}
		if ( managedType.getHierarchy().getRoot() != managedType && cacheableAnn == null ) {
			typeBinding.setCached( typeBinding.getSuperclass().isCached() );
			return;
		}
	...
	private void bindDiscriminatorValue(
		...
		if ( ann == null ) {
			...
			else if ( resolvedJavaType == Integer.class || resolvedJavaType == int.class ) {
				typeBinding.setDiscriminatorValue( String.valueOf( managedType.getJpaEntityName().hashCode() ) );
			}
			else {
				throw new MappingException(
						"Entity '" + managedType.getJpaEntityName() + "' must explicitly specify its '@DiscriminatorValue'"
				);
			}
		}
		else {
	...
	private static void bindDiscriminator(
		...
		final DiscriminatorOptions optionsAnn = managedType.getClassDetails()
				.getDirectAnnotationUsage( DiscriminatorOptions.class );
		if ( optionsAnn != null ) {
			typeBinding.setForceDiscriminator( optionsAnn.force() );
			typeBinding.setDiscriminatorInsertable( optionsAnn.insert() );
		}
	}
	...
	private void processSoftDelete(
		...
		final SoftDelete softDeleteConfig = getTypeBinding() instanceof RootClass
				? SoftDeleteBindingHelper.resolveEntitySoftDelete( classDetails, getBindingState() )
				: null;
	...
	private void processCacheRegions(
		...
		if ( cacheRegion != null ) {
			...
			if ( cacheRegion.getAccessType() != null ) {
	...
	private void processSqlRestriction(IdentifiableTypeMetadata type) {
		...
		if ( !( binding instanceof RootClass rootClass ) ) {
			throw new AnnotationException( "Entity class '" + type.getClassDetails().getName()
	...
	private SQLRestriction findSqlRestriction(IdentifiableTypeMetadata type) {
		...
		while ( superType != null
			...
			final SQLRestriction superRestriction = getOverridableAnnotation(
					superType.getClassDetails(),
					SQLRestriction.class,
					dialect,
					modelsContext
			);
			if ( superRestriction != null ) {
				return superRestriction;
			}
			superType = superType.getSuperType();
		}
		return null;
