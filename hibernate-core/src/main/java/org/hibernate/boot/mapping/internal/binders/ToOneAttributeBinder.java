...
class ToOneAttributeBinder {
	...
	static Value bindToOneValue(
		...
		if ( source.oneToOne() != null && source.hasPrimaryKeyJoinColumnSpecification() ) {
			return bindOwningPrimaryKeyOneToOne(
					source,
					ownerType,
					ownerBinding,
					ownerClassName,
	...
	private static OneToOne bindOwningPrimaryKeyOneToOne(
		...
		value.setTypeUsingReflection( ownerClassName, propertyName, bindingState.getClassLoaderService() );
		...
		final boolean optional = source.optional() && allowsNull( member );
	...
	private static ManyToOne bindToOne(
		...
		final List<Column> referencedPropertyColumns = propertyRef == null
				...
				? List.of()
				: referencedPropertyColumns( target, propertyRef.value() );
		final MapsId mapsId = member.getDirectAnnotationUsage( MapsId.class );
		value.setPropertyName( source.implicitNamingPath().getFullPath() );
		value.setReferencedEntityName( target.entityName() );
		...
		final boolean optional = mapsId == null && source.optional() && allowsNull( member );
		property.setOptional( optional );
		property.setCascade( source.cascades( bindingState ), source.orphanRemoval() );
		...
		boolean unconstrainedSharedIdentifierOneToOne = false;
		...
		if ( mapsId == null ) {
			...
			bindJoinColumns(
					...
					referencedPropertyColumns,
					logicalOneToOne && !hasFormula( valueJoinColumns ),
			...
			unconstrainedSharedIdentifierOneToOne = isUnconstrainedSharedIdentifierOneToOne(
					value,
					logicalOneToOne,
					optional,
					sharedIdentifierColumns,
			...
			property.setOptional(
					joinTable == null
							? optional && ( value.isNullable() || unconstrainedSharedIdentifierOneToOne )
							: optional
			);
			if ( unconstrainedSharedIdentifierOneToOne ) {
	...
	private static boolean hasFormula(List<JoinColumnOrFormulaSource> joinColumns) {
		for ( JoinColumnOrFormulaSource joinColumn : joinColumns ) {
			if ( joinColumn.formula() != null ) {
				return true;
			}
		}
		return false;
	...
	private static OneToOne bindInverseOneToOne(
			...
			BindingContext bindingContext) {
		final TargetEntityBinding target = resolveTargetEntityBinding( source, bindingState, bindingContext );
		if ( source.hasExplicitJoinColumnOverride() ) {
			final String concreteOwnerName = ownerBinding == null
					? ownerType.getClassDetails().getClassName()
					: ownerBinding.getClassName();
			throw new AnnotationException(
					"Association '" + propertyName + "' in entity '" + concreteOwnerName
	...
	private static void applyNotFound(ToOneSource source, ManyToOne value) {
		...
		else if ( source.isLogicalOneToOne()
				&& source.optional()
				&& ( source.member().getDirectAnnotationUsage( PrimaryKeyJoinColumn.class ) != null
					|| source.member().getDirectAnnotationUsage( PrimaryKeyJoinColumns.class ) != null ) ) {
			value.setNotFoundAction( NotFoundAction.IGNORE );
		}
	}

	private static void applyLazy(
			ToOneSource source,
		...
		if ( source.notFound() != null ) {
			if ( fetchType == FetchType.LAZY ) {
				BOOT_LOGGER.ignoreNotFoundWithFetchTypeLazy( ownerBinding.getEntityName(), source.propertyName() );
			}
			value.setLazy( false );
			value.setUnwrapProxy( true );
		}
		else {
			final boolean eager = fetchType != FetchType.LAZY;
			value.setLazy( !eager );
			value.setUnwrapProxy( eager );
			value.setUnwrapProxyImplicit( true );
		}
	}
	...
	private static void bindJoinColumns(
			...
			List<Column> sharedIdentifierColumns,
			Database database,
			List<Column> referencedPropertyColumns,
		...
		if ( ( referenceToPrimaryKey || !referencedPropertyColumns.isEmpty() )
				&& !joinColumnAnns.isEmpty()
				&& joinColumnAnns.size() != targetColumns.size() ) {
		...
		if ( !value.hasFormula() && value.getColumns().size() == targetColumns.size() ) {
			final SelectableOrderResolution selectableOrderResolution = hasExplicitReferencedColumnNames( orderedJoinColumns )
					? SelectableOrderResolver.tryResolveByReferencedNames(
							value.getColumns(),
							targetColumns,
			...
			if ( selectableOrderResolution != null ) {
				value.setForeignKeyColumnMappings( selectableOrderResolution.foreignKeyColumnMappings() );
			}
		}
	}
	...
	private static boolean hasExplicitReferencedColumnNames(List<JoinColumnOrFormulaSource> joinColumns) {
