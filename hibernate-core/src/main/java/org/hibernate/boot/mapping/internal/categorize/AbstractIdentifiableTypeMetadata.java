...
public abstract class AbstractIdentifiableTypeMetadata
	...
	private AccessType determineAccessType() {
		...
		final AccessType localDefaultAccessType = determineLocalDefaultAccessType();
		if ( localDefaultAccessType != null ) {
			return localDefaultAccessType;
		}

		return hierarchy.getDefaultAccessType();
	}

	private AccessType determineLocalDefaultAccessType() {
		final ClassDetails classDetails = getClassDetails();
		for ( var field : classDetails.getFields() ) {
			if ( CategorizationHelper.isIdAccessTypeIndicator( field ) ) {
				return AccessType.FIELD;
			}
		}
		for ( var method : classDetails.getMethods() ) {
			if ( CategorizationHelper.isIdAccessTypeIndicator( method ) ) {
				return AccessType.PROPERTY;
			}
		}
		for ( var field : classDetails.getFields() ) {
			if ( CategorizationHelper.isDefaultAccessTypeIndicator( field ) ) {
				return AccessType.FIELD;
			}
		}
		for ( var method : classDetails.getMethods() ) {
			if ( CategorizationHelper.isDefaultAccessTypeIndicator( method ) ) {
				return AccessType.PROPERTY;
			}
		}
		return null;
	...
	protected List<JpaEventListener> collectHierarchyEventListeners(JpaEventListener localCallback) {
		...
		if ( localCallback != null ) {
			combined.replaceAll( listener -> listener.withoutCallbacksOverriddenBy( localCallback ) );
			combined.removeIf( listener -> !listener.hasCallbackMethods() );
		}
	...
	private void applyEventListeners(ClassDetails listenerSource, Consumer<JpaEventListener> consumer) {
		...
		for ( Class<?> listenerClass : entityListenerClasses ) {
			JpaEventListener.listenersForTarget(
	...
	private ClassDetails packageInfoDetails(ClassDetails classDetails) {
