...
public class CategorizationHelper {
	...
	public static boolean isIdAccessTypeIndicator(MemberDetails memberDetails) {
	...
	public static boolean isDefaultAccessTypeIndicator(MemberDetails memberDetails) {
		...
		return isIdAccessTypeIndicator( memberDetails )
	...
	static AttributeNature determineAttributeNature(
		...
		if ( !plural && any == null ) {
			...
			if ( backingMember.hasDirectAnnotationUsage( Temporal.class )
					...
					|| ( !explicitlyEmbedded
							&& !aggregateArray
							&& !isAggregateEmbeddableJdbcType( backingMember, memberType, aggregateJdbcTypeCode )
