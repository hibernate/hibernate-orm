...
public class GlobalRegistrationsImpl implements GlobalRegistrations {
	...
	private void collectNamedQueries(List<JaxbNamedHqlQueryImpl> jaxbNamedQueries, XmlDocumentContext xmlDocumentContext) {
		...
		for ( JaxbNamedHqlQueryImpl jaxbNamedQuery : jaxbNamedQueries ) {
			...
			collectNamedQueryRegistration( name, NamedQueryRegistration.Kind.HQL, true, queryAnnotation, null );
		}
	}
	...
	private void collectNamedNativeQueries(
		...
		for ( JaxbNamedNativeQueryImpl jaxbNamedQuery : namedNativeQueries ) {
			...
			collectNamedQueryRegistration( name, NamedQueryRegistration.Kind.NATIVE, true, queryAnnotation, null );
		}
	}
	...
	private void collectStoredProcedureQueries(
		...
		for ( JaxbNamedStoredProcedureQueryImpl jaxbQuery : namedProcedureQueries ) {
			...
			collectNamedQueryRegistration( name, NamedQueryRegistration.Kind.CALLABLE, true, queryAnnotation, null );
		}
	}
	...
	private void collectNamedQueryRegistration(
		...
		collectNamedQueryRegistration( name, kind, isJpa, configuration, location, true );
	}

	private void collectNamedQueryRegistrationIfAbsent(
			String name,
			NamedQueryRegistration.Kind kind,
			boolean isJpa,
			Annotation configuration,
			AnnotationTarget location) {
		collectNamedQueryRegistration( name, kind, isJpa, configuration, location, false );
	}
	...
	private void collectNamedQueryRegistration(
		...
		final var registration = new NamedQueryRegistration( name, kind, isJpa, configuration, location );
		if ( replaceExisting ) {
	...
	private void collectNamedEntityGraphRegistration(
		...
		if ( existing != null ) {
			if ( Objects.equals( existing.entityName(), entityName ) ) {
	...
	private static boolean isPackageInfo(ClassDetails classDetails) {
		return classDetails.getName().endsWith( ".package-info" );
	}

	private String parsedGraphEntityName(
			ClassDetails classDetails,
			org.hibernate.annotations.NamedEntityGraph graph,
			boolean packageInfo) {
		if ( !packageInfo ) {
			return jpaEntityName( classDetails );
		}
		final Class<?> root = graph.root();
		if ( void.class.equals( root ) ) {
			return null;
		}
		return jpaEntityName( classDetailsRegistry.resolveClassDetails( root.getName() ) );
	}
	...
	public void collectSequenceGenerators(List<JaxbSequenceGeneratorImpl> sequenceGenerators) {
		...
		sequenceGenerators.forEach( (generator) -> {
			final var annotationUsage = JpaAnnotations.SEQUENCE_GENERATOR.createUsage( modelsContext );
			if ( isNotEmpty( generator.getName() ) ) {
				annotationUsage.name( generator.getName() );
			}
			if ( isNotEmpty( generator.getSequenceName() ) ) {
				annotationUsage.sequenceName( generator.getSequenceName() );
			}
			if ( isNotEmpty( generator.getCatalog() ) ) {
				annotationUsage.catalog( generator.getCatalog() );
			}
			if ( isNotEmpty( generator.getSchema() ) ) {
				annotationUsage.schema( generator.getSchema() );
			}
			if ( generator.getInitialValue() != null ) {
				annotationUsage.initialValue( generator.getInitialValue() );
			}
			if ( generator.getAllocationSize() != null ) {
				annotationUsage.allocationSize( generator.getAllocationSize() );
			}
			if ( isNotEmpty( generator.getOptions() ) ) {
	...
	public void collectTableGenerators(List<JaxbTableGeneratorImpl> tableGenerators) {
		...
		tableGenerators.forEach( (generator) -> {
			final var annotationUsage = JpaAnnotations.TABLE_GENERATOR.createUsage( modelsContext );
			if ( isNotEmpty( generator.getName() ) ) {
				annotationUsage.name( generator.getName() );
			}
			if ( isNotEmpty( generator.getTable() ) ) {
				annotationUsage.table( generator.getTable() );
			}
			if ( isNotEmpty( generator.getCatalog() ) ) {
				annotationUsage.catalog( generator.getCatalog() );
			}
			...
			if ( isNotEmpty( generator.getPkColumnName() ) ) {
				annotationUsage.pkColumnName( generator.getPkColumnName() );
			}
			if ( isNotEmpty( generator.getValueColumnName() ) ) {
				annotationUsage.valueColumnName( generator.getValueColumnName() );
			}
			if ( isNotEmpty( generator.getPkColumnValue() ) ) {
				annotationUsage.pkColumnValue( generator.getPkColumnValue() );
			}
			if ( generator.getInitialValue() != null ) {
				annotationUsage.initialValue( generator.getInitialValue() );
			}
			...
			annotationUsage.uniqueConstraints( XmlAnnotationHelper.collectUniqueConstraints(
					generator.getUniqueConstraints(),
					modelsContext
			) );
			annotationUsage.indexes( XmlAnnotationHelper.collectIndexes(
					generator.getIndexes(),
					modelsContext
			) );
