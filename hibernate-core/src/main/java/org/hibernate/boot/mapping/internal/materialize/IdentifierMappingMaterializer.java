...
import org.hibernate.boot.mapping.internal.context.EntityIdentifierHandoff;
...
public class IdentifierMappingMaterializer {
	...
	public IdentifierBinding materializeBasicIdentifier(
		...
		applyIdentifierPropertyValueGenerator( idProperty, idAttributeMember );
		...
		deferAttributeBinders( idAttributeMember, typeBinding, idProperty );
	...
	public IdentifierBinding materializeAggregatedIdentifier(
		...
		deferAttributeBinders( aggregatedKeyMapping.getAttribute().getMember(), typeBinding, idProperty );
	...
	public IdentifierBinding materializeScalarIdClassIdentifier(
		...
		for ( AttributeMetadata idAttribute : orderedIdAttributes( idMapping, binding ) ) {
			...
			final BasicValue idClassValue = createBasicIdClassValue( table, member, idClassMember, typeBinding );
			applyGeneratedValue( idClassValue, member, typeBinding );
	...
	public IdentifierBinding materializeNonAggregatedIdentifier(
		...
		for ( AttributeMetadata idAttribute : orderedIdAttributes( idMapping, binding ) ) {
			...
			if ( idAttribute.getNature() == AttributeNature.BASIC
				...
				final BasicValue idClassValue = hasIdClass
						? createBasicIdClassValue( table, member, idClassMember, typeBinding )
						: basicValue;
				if ( hasIdClass ) {
					applyGeneratedValue( idClassValue, member, typeBinding );
				}
	...
	public IdentifierBinding materializeEntityIdentifierBinding(
		...
		final IdentifierBinding identifierBinding = new IdentifierBinding(
				entityType,
				rootClass,
				keyMapping,
				identifierValue,
		...
		state.addEntityIdentifierHandoff( new EntityIdentifierHandoff( view, identifierBinding ) );
	...
	private AttributeMetadata idAttribute(NonAggregatedKeyMapping idMapping, String attributeName) {
		...
		throw new IllegalStateException( "Could not resolve identifier attribute '" + attributeName + "'" );
	}
	...
	private BasicValue createBasicIdValue(Table table, MemberDetails member, TypeDetails type) {
		final BasicValue basicValue = BasicValue.unregistered( state.getMetadataBuildingContext(), table );
	...
	private static void validateIdentifierSelectable(BasicValue basicValue, MemberDetails member) {
		if ( basicValue.hasFormula() ) {
	...
	private BasicValue createBasicIdClassValue(
			Table table,
	...
	private Column bindIdColumn(
		...
		applyIdentifierColumnDefault( member, column );
	...
	private void applyIdentifierColumnDefault(MemberDetails member, Column column) {
		final ColumnDefault columnDefault = getOverridableAnnotation(
				member,
				ColumnDefault.class,
				state.getMetadataBuildingContext().getMetadataCollector().getDatabase().getDialect(),
				state.getMetadataBuildingContext().getModelsContext()
		);
		if ( columnDefault != null ) {
			column.setDefaultValue( columnDefault.value() );
		}
	}

	private void deferAttributeBinders(MemberDetails member, RootClass typeBinding, Property property) {
		state.addAttributeCustomMapping( CustomMappingBinder.attributeBinding( member, typeBinding, property, state, context ) );
	}
