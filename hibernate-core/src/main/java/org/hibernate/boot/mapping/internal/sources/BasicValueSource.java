...
public record BasicValueSource(
	...
	public static BasicValueSource mapKey(MemberDetails member, BindingContext bindingContext) {
		return mapKey( member, member.getMapKeyType(), bindingContext );
	}

	public static BasicValueSource mapKey(MemberDetails member, TypeDetails mapKeyType, BindingContext bindingContext) {
			...
			for ( Convert conversion : member.getRepeatedAnnotationUsages( Convert.class, modelsContext ) ) {
				if ( roleName.equals( conversion.attributeName() )
						|| ( "value".equals( roleName )
								&& ( conversion.attributeName() == null || conversion.attributeName().isEmpty() ) ) ) {
					return conversion;
				}
			}
		}
