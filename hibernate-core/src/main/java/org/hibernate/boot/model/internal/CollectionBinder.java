...
//				comment,
	...
	protected void bindOneToManySecondPass(Map<String, PersistentClass> persistentClasses) {
		...
		if ( foreignJoinColumns.hasMappedBy() ) {
			Value foreignKeyValue = associatedClass.getRecursiveProperty( foreignJoinColumns.getMappedBy() ).getValue();
			collection.setCollectionTable( foreignKeyValue.getTable() );
			if ( foreignKeyValue instanceof Any ) {
				collection.setMappedByAnyDiscriminatorDescriptor( ( (Any) foreignKeyValue ).getDiscriminatorDescriptor() );
			}
		}
		else {
