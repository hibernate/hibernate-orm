...
//		annotationToXml.put( AccessType.class, "access" );
	...
	private void initAnnotations() {
		if ( annotations == null ) {
			...
			if ( className != null && propertyName == null ) {
				//is a class
				final JaxbManagedType jaxbManagedTypeOverride = xmlContext.getManagedTypeOverride( className );
				...
				addIfNotNull( annotationList, getEntity( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getMappedSuperclass( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getEmbeddable( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getTable( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getSecondaryTables( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getPrimaryKeyJoinColumns( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getIdClass( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getCacheable( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getCaching( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getInheritance( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getDiscriminatorValue( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getDiscriminatorColumn( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getSequenceGenerator( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getTableGenerator( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getNamedQueries( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getNamedNativeQueries( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getNamedStoredProcedureQueries( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getNamedEntityGraphs( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getSqlResultSetMappings( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getExcludeDefaultListeners( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getExcludeSuperclassListeners( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getAccessType( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getAttributeOverrides( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getAssociationOverrides( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getEntityListeners( jaxbManagedTypeOverride, defaults ) );
				addIfNotNull( annotationList, getConverts( jaxbManagedTypeOverride, defaults ) );

				this.annotations = annotationList.toArray( new Annotation[0] );
				for ( Annotation ann : this.annotations ) {
					annotationsMap.put( ann.annotationType(), ann );
				}
				checkForOrphanProperties( jaxbManagedTypeOverride );
			}
				...
				preCalculateElementsForProperty( jaxbManagedTypeOverride, entityListenerOverride );
	...
	private void checkForOrphanProperties(JaxbManagedType root) {
		...
		if ( container != null ) {
			...
			if ( container instanceof JaxbAttributesContainerImpl ) {
				JaxbAttributesContainerImpl jaxbAttributes = (JaxbAttributesContainerImpl) container;
	...
	private void getJoinTable(
			...
			XMLContext.Default defaults) {
		addIfNotNull( annotationList, buildJoinTable( jaxbAssociationAttribute.getJoinTable(), defaults ) );
	}
	...
	private Cache getCaching(JaxbManagedType root, XMLContext.Default defaults) {
		if ( root instanceof JaxbEntityImpl ) {
			final JaxbCachingImpl caching = ( (JaxbEntityImpl) root ).getCaching();
			if ( caching != null ) {
	...
	private Cacheable getCacheable(JaxbManagedType root, XMLContext.Default defaults) {
		if ( root instanceof JaxbEntityImpl ) {
			Boolean attValue = ( (JaxbEntityImpl) root ).isCacheable();
			if ( attValue != null ) {
	...
	private void preCalculateElementsForProperty(JaxbManagedType jaxbManagedType, JaxbEntityListenerImpl entityListener) {
		elementsForProperty = new PropertyMappingElementCollector( propertyName );
		JaxbAttributesContainer attributes = jaxbManagedType == null ? null : jaxbManagedType.getAttributes();
		//put entity.attributes elements
		if ( attributes != null ) {
		...
		if ( jaxbManagedType instanceof JaxbLifecycleCallbackContainer ) {
			elementsForProperty.collectLifecycleCallbacksIfMatching( (JaxbLifecycleCallbackContainer) jaxbManagedType );
		}
		if ( entityListener != null ) {
			elementsForProperty.collectLifecycleCallbacksIfMatching( entityListener );
		}
		elementsForProperty.collectTenantIdIfMatching( jaxbManagedType );
	}
	...
	private DiscriminatorColumn getDiscriminatorColumn(JaxbManagedType root, XMLContext.Default defaults) {
		JaxbDiscriminatorColumnImpl element = root instanceof JaxbEntityImpl ? ( (JaxbEntityImpl) root ).getDiscriminatorColumn() : null;
		if ( element != null ) {
	...
	private SecondaryTables getSecondaryTables(JaxbManagedType root, XMLContext.Default defaults) {
		final List<JaxbSecondaryTableImpl> elements = root instanceof JaxbEntityImpl
				? ( (JaxbEntityImpl) root ).getSecondaryTables()
