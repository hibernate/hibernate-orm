...
/**
 * SecondPass implementing delayed resolution of id-generators using strict JPA
 * resolution - based mainly on global resolution of generator names, along with
 * support for UUID and String member types with AUTO.  We also account for legacy
 * (un-configurable) named generators ({@code increment}, {@code uuid.hex}, etc.).
 *
 * @implNote For unnamed generators, this strategy will register a global registration
 * using the entity's name and later look it up by that name.  This more strictly
 * follows the JPA specification where all generator names should be considered global
 * and resolved globally.
 *
 * @see BetterIdGeneratorResolverSecondPass
 *
