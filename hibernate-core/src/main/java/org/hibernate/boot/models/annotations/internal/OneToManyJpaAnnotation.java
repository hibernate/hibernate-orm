...
public class OneToManyJpaAnnotation implements OneToMany, AttributeMarker.Fetchable, AttributeMarker.Cascadeable {
	...
	public OneToManyJpaAnnotation(OneToMany annotation, SourceModelBuildingContext modelContext) {
		this.targetEntity = extractJdkValue( annotation, JpaAnnotations.ONE_TO_MANY, "targetEntity", modelContext );
		this.cascade = extractJdkValue( annotation, JpaAnnotations.ONE_TO_MANY, "cascade", modelContext );
		this.fetch = extractJdkValue( annotation, JpaAnnotations.ONE_TO_MANY, "fetch", modelContext );
		this.mappedBy = extractJdkValue( annotation, JpaAnnotations.ONE_TO_MANY, "mappedBy", modelContext );
		this.orphanRemoval = extractJdkValue( annotation, JpaAnnotations.ONE_TO_MANY, "orphanRemoval", modelContext );
	}
