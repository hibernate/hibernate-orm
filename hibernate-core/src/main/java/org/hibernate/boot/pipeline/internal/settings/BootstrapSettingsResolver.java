...
public class BootstrapSettingsResolver {
	...
	private static void normalizeValidationMode(
			...
			PersistenceUnitDescriptor persistenceUnitDescriptor,
			Map<?, ?> integrationSettingsCopy) {
		final var jakartaIntegrationMode = remove( integrationSettingsCopy, JAKARTA_VALIDATION_MODE );
		final var legacyIntegrationMode = remove( integrationSettingsCopy, JPA_VALIDATION_MODE );
		if ( jakartaIntegrationMode != null ) {
			configurationValues.put( JAKARTA_VALIDATION_MODE, jakartaIntegrationMode );
			configurationValues.remove( JPA_VALIDATION_MODE );
			return;
		}
		if ( legacyIntegrationMode != null ) {
			DEPRECATION_LOGGER.deprecatedSetting( JPA_VALIDATION_MODE, JAKARTA_VALIDATION_MODE );
			configurationValues.put( JAKARTA_VALIDATION_MODE, legacyIntegrationMode );
			configurationValues.put( JPA_VALIDATION_MODE, legacyIntegrationMode );
			return;
		}
		...
		final var configuredMode = configurationValues.get( JAKARTA_VALIDATION_MODE );
		if ( configuredMode != null ) {
			configurationValues.remove( JPA_VALIDATION_MODE );
			return;
		}
		final var legacyConfiguredMode = configurationValues.remove( JPA_VALIDATION_MODE );
		if ( legacyConfiguredMode != null ) {
			DEPRECATION_LOGGER.deprecatedSetting( JPA_VALIDATION_MODE, JAKARTA_VALIDATION_MODE );
			configurationValues.put( JAKARTA_VALIDATION_MODE, legacyConfiguredMode );
			configurationValues.put( JPA_VALIDATION_MODE, legacyConfiguredMode );
		}
	}
	...
	private static void normalizeSharedCacheMode(
			...
			PersistenceUnitDescriptor persistenceUnitDescriptor,
			Map<?, ?> integrationSettingsCopy) {
		final var jakartaIntegrationMode = remove( integrationSettingsCopy, JAKARTA_SHARED_CACHE_MODE );
		final var legacyIntegrationMode = remove( integrationSettingsCopy, JPA_SHARED_CACHE_MODE );
		if ( jakartaIntegrationMode != null ) {
			configurationValues.put( JAKARTA_SHARED_CACHE_MODE, jakartaIntegrationMode );
			configurationValues.remove( JPA_SHARED_CACHE_MODE );
			return;
		}
		if ( legacyIntegrationMode != null ) {
			DEPRECATION_LOGGER.deprecatedSetting( JPA_SHARED_CACHE_MODE, JAKARTA_SHARED_CACHE_MODE );
			configurationValues.put( JAKARTA_SHARED_CACHE_MODE, legacyIntegrationMode );
			configurationValues.put( JPA_SHARED_CACHE_MODE, legacyIntegrationMode );
			return;
		}
		...
		final var configuredMode = configurationValues.get( JAKARTA_SHARED_CACHE_MODE );
		if ( configuredMode != null ) {
			configurationValues.remove( JPA_SHARED_CACHE_MODE );
			return;
		}
		final var legacyConfiguredMode = configurationValues.remove( JPA_SHARED_CACHE_MODE );
		if ( legacyConfiguredMode != null ) {
			DEPRECATION_LOGGER.deprecatedSetting( JPA_SHARED_CACHE_MODE, JAKARTA_SHARED_CACHE_MODE );
			configurationValues.put( JAKARTA_SHARED_CACHE_MODE, legacyConfiguredMode );
			configurationValues.put( JPA_SHARED_CACHE_MODE, legacyConfiguredMode );
		}
	}
