...
public class CacheEntryHelper {
	...
	static Serializable[] disassemble(
			@Nonnull final Object[] row,
			@Nonnull final Type[] types,
			@Nullable final boolean[] nonCacheable,
			@Nonnull final SharedSessionContractImplementor session,
			@Nonnull final Object owner) {
		return disassemble( row, types, nonCacheable, session, owner, null );
	}
			...
			if ( nonCacheable!=null && nonCacheable[i] ) {
				...
				if ( persister != null
					...
					if ( Hibernate.isPropertyInitialized( owner, propertyName ) ) {
						final Object resolved = persister.getPropertyValue( owner, i );
