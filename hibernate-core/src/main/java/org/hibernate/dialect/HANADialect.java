...
public class HANADialect extends Dialect {
	...
	public void contributeTypes(TypeContributions typeContributions, ServiceRegistry serviceRegistry) {
		...
		jdbcTypeRegistry.addDescriptor( Types.CLOB, new HANADialect.HANAClobJdbcType( maxLobPrefetchSize, useUnicodeStringTypes ) );
		jdbcTypeRegistry.addDescriptor( Types.NCLOB, new HANADialect.HANANClobJdbcType( maxLobPrefetchSize ) );
		jdbcTypeRegistry.addDescriptor( Types.BLOB, new HANADialect.HANABlobType( maxLobPrefetchSize ) );
			...
			return new BasicBinder<>( javaType, this ) {
				...
				protected void doBind(PreparedStatement st, X value, int index, WrapperOptions options) throws SQLException {
					...
					if ( value instanceof BlobImplementer ) {
						try ( InputStream is = new HANADialect.CloseSuppressingInputStream( binaryStream.getInputStream() ) ) {
		...
		public <X> ValueExtractor<X> getExtractor(JavaType<X> javaType) {
			return new BasicExtractor<>( javaType, this ) {
				...
				protected X doExtract(ResultSet rs, int paramIndex, WrapperOptions options) throws SQLException {
					...
					if ( rsBlob == null || rsBlob.length() < HANADialect.HANAStreamBlobType.this.maxLobPrefetchSize ) {
						return javaType.wrap( rsBlob, options );
					}
					Blob blob = new HANADialect.MaterializedBlob( DataHelper.extractBytes( rsBlob.getBinaryStream() ) );
	...
	private static class HANAClobJdbcType extends ClobJdbcType {
			...
			return new BasicBinder<>( javaType, this ) {
				...
				protected void doBind(final PreparedStatement st, final X value, final int index, final WrapperOptions options) throws SQLException {
					...
					if ( value instanceof ClobImplementer ) {
						try ( Reader r = new HANADialect.CloseSuppressingReader( characterStream.asReader() ) ) {
		...
		public <X> ValueExtractor<X> getExtractor(JavaType<X> javaType) {
			return new BasicExtractor<>( javaType, this ) {
				...
				protected X doExtract(ResultSet rs, int paramIndex, WrapperOptions options) throws SQLException {
					Clob rsClob;
					if ( HANADialect.HANAClobJdbcType.this.useUnicodeStringTypes ) {
					...
					if ( rsClob == null || rsClob.length() < HANADialect.HANAClobJdbcType.this.maxLobPrefetchSize ) {
						return javaType.wrap( rsClob, options );
					}
					Clob clob = new HANADialect.MaterializedNClob( DataHelper.extractString( rsClob ) );
		...
		public <X> ValueExtractor<X> getExtractor(JavaType<X> javaType) {
			return new BasicExtractor<>( javaType, this ) {
				...
				protected X doExtract(ResultSet rs, int paramIndex, WrapperOptions options) throws SQLException {
					...
					if ( rsNClob == null || rsNClob.length() < HANADialect.HANANClobJdbcType.this.maxLobPrefetchSize ) {
						return javaType.wrap( rsNClob, options );
					}
					NClob nClob = new HANADialect.MaterializedNClob( DataHelper.extractString( rsNClob ) );
	...
	public static class HANABlobType implements JdbcType {
		...
		public static final JdbcType INSTANCE = new HANADialect.HANABlobType( HANAServerConfiguration.MAX_LOB_PREFETCH_SIZE_DEFAULT_VALUE );

		final int maxLobPrefetchSize;

		final HANADialect.HANAStreamBlobType hanaStreamBlobTypeDescriptor;

		public HANABlobType(int maxLobPrefetchSize) {
			this.maxLobPrefetchSize = maxLobPrefetchSize;
			this.hanaStreamBlobTypeDescriptor = new HANADialect.HANAStreamBlobType( maxLobPrefetchSize );
		}
		...
		public <X> ValueExtractor<X> getExtractor(final JavaType<X> javaType) {
			return new BasicExtractor<>( javaType, this ) {
				...
				protected X doExtract(ResultSet rs, int paramIndex, WrapperOptions options) throws SQLException {
					Blob rsBlob = rs.getBlob( paramIndex );
					if ( rsBlob == null || rsBlob.length() < HANADialect.HANABlobType.this.maxLobPrefetchSize ) {
						return javaType.wrap( rsBlob, options );
					}
					Blob blob = new HANADialect.MaterializedBlob( DataHelper.extractBytes( rsBlob.getBinaryStream() ) );
					return javaType.wrap( blob, options );
			...
			return new BasicBinder<>( javaType, this ) {
				...
				protected void doBind(PreparedStatement st, X value, int index, WrapperOptions options) throws SQLException {
					...
					if ( value instanceof byte[] ) {
						...
						descriptor = HANADialect.HANABlobType.this.hanaStreamBlobTypeDescriptor;
					}
