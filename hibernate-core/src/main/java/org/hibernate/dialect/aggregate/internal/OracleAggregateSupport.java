...
//				return "(" + customWriteExpression + ") format json";
	...
	public List<AggregateAuxiliaryObject> aggregateAuxiliaryObjects(AggregateAuxiliaryObjectRequest request) {
		final int typeCode = request.aggregateColumn().sqlTypeCode();
		if ( typeCode == STRUCT_ARRAY || typeCode == STRUCT_TABLE ) {
			final var element = request.aggregateColumn().arrayElement();
			assert element != null;
			return List.of( new AggregateUserDefinedArrayType(
					request.aggregateColumn().sqlTypeName(),
					typeCode == STRUCT_ARRAY ? ARRAY : TABLE,
					typeCode == STRUCT_ARRAY ? element.arrayLength() : 0,
					element.typeName(),
					element.sqlTypeCode(),
					element.ddlTypeCode()
			) );
		}
		return super.aggregateAuxiliaryObjects( request );
	}
