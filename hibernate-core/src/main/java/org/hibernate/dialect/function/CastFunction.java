 ...
 * @author Gavin King
 */
public class CastFunction implements SqmFunctionTemplate {
	/**
	 * Singleton access
	 */
	...
	public String render(Type columnType, List args, SessionFactoryImplementor factory) throws QueryException {
		...
		final int jdbcTypeCode = factory.getMetamodel().getTypeConfiguration().resolveCastTargetType( type ).getColumnDescriptor().getSqlTypeDescriptor().getSqlType();
