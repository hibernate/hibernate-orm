...
///
/// A resolver may combine three sources of database behavior:
///
/// - Hard-coded dialect knowledge, when behavior is fixed for the database
///   and version represented by the dialect. Such facts require no JDBC access.
/// - Database-specific queries, when JDBC access is permitted, to discover
///   settings which affect concurrency, such as SQL Server's
///   `READ_COMMITTED_SNAPSHOT` setting.
/// - The connection's configured isolation level, obtained using
///   [Connection#getTransactionIsolation()]. Bootstrap exposes this observation
///   through [JdbcMetadata#getTransactionIsolation()] when JDBC metadata is
///   available, so the resolver need not read it again. A resolver may also
///   inspect the supplied connection directly when JDBC access is permitted.
///
/// These sources are complementary, not a required sequence of attempts.
/// A JDBC isolation value must be interpreted using dialect knowledge and any
/// relevant database settings; it does not by itself distinguish all locking
/// and snapshot behavior. The database's default isolation level is not a
/// substitute for the configured isolation of the factory's connections.
///
