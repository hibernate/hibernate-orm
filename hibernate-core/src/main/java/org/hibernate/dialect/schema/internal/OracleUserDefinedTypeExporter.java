...
/// @author Yoobin Yoon
public final class OracleUserDefinedTypeExporter implements Exporter<UserDefinedType> {
	private final Dialect dialect;
	private final StandardUserDefinedTypeExporter standardExporter;
	private final ExistenceCheckPlacement dropIfExistsPlacement;

	public OracleUserDefinedTypeExporter(Dialect dialect, UserDefinedTypeDdlSupport ddlSupport) {
		this.dialect = dialect;
		this.standardExporter = new StandardUserDefinedTypeExporter( dialect, ddlSupport );
		this.dropIfExistsPlacement = ddlSupport.dropIfExistsPlacement();
	}
	...
	public String[] getSqlCreateStrings(
		...
		return userDefinedType instanceof UserDefinedArrayType arrayType
				? getSqlCreateStrings( arrayType, metadata, context )
				: standardExporter.getSqlCreateStrings( userDefinedType, metadata, context );
	}
	...
	public String[] getSqlDropStrings(
			...
			SqlStringGenerationContext context) {
		return userDefinedType instanceof UserDefinedArrayType arrayType
				? getSqlDropStrings( arrayType, metadata, context )
				: standardExporter.getSqlDropStrings( userDefinedType, metadata, context );
	}

	private String[] getSqlDropStrings(
			UserDefinedArrayType userDefinedType,
			Metadata metadata,
			SqlStringGenerationContext context) {
		final QualifiedName typeName = new QualifiedNameParser.NameParts(
	...
	private String buildDropTypeSqlString(String arrayTypeName) {
		if ( dropIfExistsPlacement == ExistenceCheckPlacement.BEFORE_NAME ) {
			return "drop type if exists " + arrayTypeName + " force";
		}
		else if ( dropIfExistsPlacement == ExistenceCheckPlacement.AFTER_NAME ) {
			return "drop type " + arrayTypeName + " if exists force";
		}
		else {
	...
	private String createOrReplaceConcatFunction(String arrayTypeName) {
