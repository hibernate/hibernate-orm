...
import static org.hibernate.sql.ast.spi.query.from.SqlAstJoinType.INNER;
import static org.hibernate.sql.ast.spi.query.from.SqlAstJoinType.LEFT;
import static org.hibernate.sql.ast.spi.query.from.SqlAstJoinType.RIGHT;
import static org.hibernate.sql.spi.SqlAppender.COMMA_SEPARATOR;
import static org.hibernate.sql.spi.SqlAppender.NO_SEPARATOR;
...
public final class FullJoinEmulation {
	...
	public boolean renderSelectClauseIfNeeded(SelectClause selectClause) {
		if ( renderingFullJoinEmulationBranch
			...
			try {
				...
				translator.renderSelectItems( selectClause );
	...
	private FullJoinEmulationPlan createFullJoinEmulationPlan(QuerySpec querySpec, List<FullJoinEmulationInfo> fullJoins) {
		return translator.getDialect().getSetOperationSupport().supports( EXCEPT_ALL )
	...
	private void renderFullJoinEmulationBranch(QuerySpec querySpec, SqlAstJoinType[] joinSides, Consumer<QuerySpec> renderQuerySpec) {
		...
		try {
			if ( translator.getDialect().getSetOperationSupport().supports( DUPLICATE_SELECT_ITEMS ) ) {
	...
	private void renderFullJoinEmulationSortExpression(
		...
		final boolean supportsNullPrecedence = renderNullPrecedence
				&& translator.getDialect().getNullOrderingSupport().supports( NULLS_FIRST_LAST );
	...
	private void renderFullJoinEmulationAdditionalSelectItems(SelectClause selectClause) {
		final boolean forceSelectAliases =
				!translator.getDialect()
						.getSetOperationSupport().supports( DUPLICATE_SELECT_ITEMS );
		String separator =
	...
	private Nulls resolveRenderedNullPrecedence(SortSpecification sortSpecification) {
		...
		if ( isDefaultOrdering( nullPrecedence, sortOrder,
				translator.getDialect().getNullOrderingSupport().getDefaultOrdering() ) ) {
			return null;
		}
