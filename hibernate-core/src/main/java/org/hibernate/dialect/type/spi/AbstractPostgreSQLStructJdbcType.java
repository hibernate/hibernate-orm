...
/// Provider base for serializing and deserializing an embeddable aggregate
/// using the PostgreSQL component format.
///
/// Subclasses define driver binding and write-expression behavior. Use the
/// runtime-model constructor when resolving a mapped aggregate so Hibernate
/// applies the boot-model attribute ordering internally.
///
/// @author Christian Beikov
/// @author Steve Ebersole
/// @since 8.0
@SPI({ USE, IMPLEMENT })
public abstract class AbstractPostgreSQLStructJdbcType implements StructuredJdbcType {
	...
	protected AbstractPostgreSQLStructJdbcType() {
		this( null, null, (int[]) null );
	}

	/// Create a mapped descriptor and resolve the database declaration's
	/// attribute order from the supplied runtime-model context.
	@SPI(IMPLEMENT)
	protected AbstractPostgreSQLStructJdbcType(
		...
		this(
				...
				creationContext.getBootModel()
						.getDatabase()
						.getDefaultNamespace()
						.locateUserDefinedType( Identifier.toIdentifier( typeName ) )
						.getOrderMapping()
		);
	}

	private AbstractPostgreSQLStructJdbcType(
			EmbeddableMappingType embeddableMappingType,
			String typeName,
			int[] orderMapping) {
