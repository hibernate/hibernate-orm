 ...
 * Implementation Warning: Hibernate needs to instantiate a high amount of instances of this class, therefore we need to
 * take care of its impact on memory consumption.
 *
 * @author Gavin King
...
public interface EntityEntry {
	...
	/**
	 * Handle updating the internal state of the entry after actually performing the database update. Specifically we
	 * update the snapshot information and escalate the lock mode
	 *
	 * @param entity The entity instance
	 * @param updatedState The state calculated after the update (becomes the new {@link #getLoadedState() loaded
	 * state}.
	...
	/**
	 * After actually inserting a row, record the fact that the instance exists on the database (needed for
	 * identity-column key generation)
	 */
	 ...
	 * possibly be dirty. This can only be the case if it is in a modifiable state (not read-only/deleted) and it either
	 * has mutable properties or field-interception is not telling us it is dirty. Clear as mud? :/ A name like
	 * canPossiblyBeDirty might be better
	 *
	 * @param entity The entity to test
	 * @return {@code true} indicates that the entity could possibly be dirty and that dirty check should happen;
