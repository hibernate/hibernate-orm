...
public abstract class AbstractSaveEventListener extends AbstractReassociateEventListener {
	...
	private static boolean shouldDelayIdentityInserts(
		...
		return !partOfTransaction || !indicatesImmediateIdentityInsert( flushMode );

	}

	private static boolean isPartOfTransaction(EventSource source) {
		return source.isTransactionInProgress() && source.isJoinedToTransaction();
	}

	private static boolean indicatesImmediateIdentityInsert(FlushMode flushMode) {
		return flushMode == ALWAYS || flushMode == AUTO;
	}
