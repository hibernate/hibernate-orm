...
public class DotNode extends FromReferenceNode implements DisplayableNode, SelectExpression, TableReferenceNode {
	...
	private void dereferenceEntity(
		...
		if ( joinIsNeeded ) {
			dereferenceEntityJoin( classAlias, entityType, useThetaJoin, implicitJoin, parent );
		}
		else {
	...
	private void dereferenceEntityJoin(String classAlias, EntityType propertyType, boolean useThetaJoin, boolean impliedJoin, AST parent)
			throws SemanticException {
		...
		if ( !useFoundFromElement ) {
			...
			if ( joinColumns.length == 0 && lhsFromElement instanceof EntityQuerySpace ) {
				...
				joinSequence = getSessionFactoryHelper()
						.createJoinSequence( useThetaJoin, propertyType, tableAlias, joinType, polyJoinColumns );
			}
			else {
				...
				joinSequence = getSessionFactoryHelper()
						.createJoinSequence( useThetaJoin, propertyType, tableAlias, joinType, joinColumns );
			}
