...
public class BatchingBatcher extends AbstractBatcher {
	...
	public void addToBatch(Expectation expectation) throws SQLException, HibernateException {
        if (!expectation.canBeBatched()) throw new HibernateException("attempting to batch an operation which cannot be batched");
		PreparedStatement batchUpdate = getStatement();
		batchUpdate.addBatch();
		expectations[ batchSize++ ] = expectation;
        if (batchSize == getFactory().getSettings().getJdbcBatchSize()) doExecuteBatch(batchUpdate);
	}

	@Override
    protected void doExecuteBatch(PreparedStatement ps) throws SQLException, HibernateException {
        if (batchSize == 0) LOG.noBatchedStatementsToExecute();
		else {
            LOG.executingBatchSize(batchSize);

			try {
				checkRowCounts( ps.executeBatch(), ps );
			}
			catch (RuntimeException re) {
                LOG.error(LOG.unableToExecuteBatch(), re);
				throw re;
			}
			finally {
				batchSize = 0;
	...
	private void checkRowCounts(int[] rowCounts, PreparedStatement ps) throws SQLException, HibernateException {
        ...
        if (numberOfRowCounts != batchSize) LOG.jdbcDriverDidNotReturnExpectedNumberOfRowCounts();
