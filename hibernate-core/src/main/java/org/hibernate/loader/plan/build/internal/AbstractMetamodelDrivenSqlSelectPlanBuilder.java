...
import org.hibernate.persister.common.spi.MetamodelDrivenSqlSelectPlanBuilder;
...
public abstract class AbstractMetamodelDrivenSqlSelectPlanBuilder
		implements MetamodelDrivenSqlSelectPlanBuilder, LoadPlanBuildingContext {
	 ...
	 */
	protected AbstractMetamodelDrivenSqlSelectPlanBuilder(SessionFactoryImplementor sessionFactory) {
...
//	}
	...
	/**
	 * Simultaneously manages the stack of PropertyPath references as well as the logging MDC value
	 * based on the current PropertyPath node.
	 * <p/>
	 * Due to the recursive calls needed for processing a Navigable graph it is generally important to see
	 * exactly where we are in the graph walking as part of log messages.  We enable this capability via this
	 * MDC hook.
	 */
	public static class PropertyPathStack {
		...
		public void push(PropertyPath path) {
			...
			internalStack.addFirst( path );

			MDC.put( MDC_KEY, path.getFullPath() );
		}
