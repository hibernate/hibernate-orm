...
//					);
	...
	private static final MemberResolver normalMemberResolver = (attributeContext, metadataContext) -> {
		...
		if ( Type.PersistenceType.EMBEDDABLE == persistenceType ) {
			...
			return resolveEntityMember( property, declaringEntity, metadataContext );
		}
		else {
	...
	private static Member resolveMappedSuperclassMember(
		...
		if ( declaringEntity != null ) {
			...
			if ( persistenceType == Type.PersistenceType.ENTITY ) {
				final EntityPersister subDeclaringEntity = getDeclaringEntity( (AbstractIdentifiableType<?>) subType, metadataContext );
				return resolveEntityMember( property, subDeclaringEntity, metadataContext );
			}
