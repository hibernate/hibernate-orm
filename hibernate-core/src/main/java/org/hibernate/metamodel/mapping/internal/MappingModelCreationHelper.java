...
public class MappingModelCreationHelper {
	...
	public static EntityIdentifierMapping buildEncapsulatedCompositeIdentifierMapping(
		...
		final var component = (Component) bootProperty.getValue();
	...
	public static EmbeddedAttributeMapping buildEmbeddedAttributeMapping(
			...
			int fetchableIndex,
			Property bootProperty,
			@Nullable DependantValue dependantValue,
			...
			CascadeStyle cascadeStyle,
			MappingModelCreationProcess creationProcess) {
		final var attributeMetadataAccess = getAttributeMetadata(
				bootProperty,
				attrType,
				propertyAccess,
				cascadeStyle,
				...
				attributeMappingType -> {
					if ( component.isEmbedded() ) {
						return new VirtualEmbeddedAttributeMapping(
								...
								attrName,
								castNonNull( declaringType.getNavigableRole() ).append( attrName ),
								stateArrayPosition,
								fetchableIndex,
								tableExpression,
	...
	private static Component component(Property bootProperty, @Nullable DependantValue dependantValue) {
	...
	public static AttributeMetadata getAttributeMetadata(PropertyAccess propertyAccess) {
		return new SimpleAttributeMetadata( propertyAccess, ImmutableMutabilityPlan.instance(), false, true, false, false, true );
	}
	...
	public static PluralAttributeMapping buildPluralAttributeMapping(
		...
		final var bootValueMapping = (Collection) bootProperty.getValue();

		final var creationContext = creationProcess.getCreationContext();
		final var domainModel = creationContext.getDomainModel();

		final var collectionDescriptor = domainModel.findCollectionDescriptor( bootValueMapping.getRole() );
		...
		final var elementDescriptor = interpretElement(
				bootValueMapping,
				tableExpression,
		...
		switch ( collectionSemantics.getCollectionClassification() ) {
			case ARRAY: {
				...
				indexDescriptor = collectionPart(
						creationProcess,
						(IndexedCollection) bootValueMapping,
						creationContext,
						collectionDescriptor,
						tableExpression
				);
	...
	@Nonnull
	private static CollectionPart collectionPart(
			MappingModelCreationProcess creationProcess,
			IndexedCollection bootValueMapping,
			RuntimeModelCreationContext creationContext,
			CollectionPersister collectionDescriptor,
			String tableExpression) {
		final var index = (BasicValue) bootValueMapping.getIndex();
	...
	private static void interpretPluralAttributeMappingKeyDescriptor(
		...
		if ( mappedByProperty == null || mappedByProperty.isEmpty() ) {
			...
			final var partMappingType =
					(ModelPartContainer)
							attributeMapping.getElementDescriptor().getPartMappingType();
			attributeMappingSubPart =
					partMappingType.findSubPart( mappedByProperty, null );
		}
		...
		final var bootValueMappingKey = bootValueMapping.getKey();
		final var keyType = bootValueMappingKey.getType();
		...
		final var fkTargetPart =
				isReferenceToPrimaryKey
						? collectionDescriptor.getOwnerEntityPersister().getIdentifierMappingForJoin()
						: castNonNull( declaringType.findContainingEntityMapping() ).findSubPart( lhsPropertyName );

		if ( keyType instanceof BasicType ) {
			assert bootValueMappingKey.getColumnSpan() == 1;

			final var simpleFkTargetPart = castNonNull( castNonNull( fkTargetPart ).asBasicValuedModelPart() );
			...
			final var keyDescriptor = new SimpleForeignKeyDescriptor(
					keyDeclaringType,
...
//			creationProcess.registerForeignKey( collectionDescriptor.getAttributeMapping(), keyDescriptor );
			...
			throw new UnsupportedOperationException(
					"Support for " + castNonNull( fkTargetPart ).getClass() + " foreign keys not yet implemented: " + bootValueMapping.getRole()
			);
		}
	}
	...
	static String getPropertyPath(NavigableRole navigableRole) {
		...
		while (navigableRole.getParent() != null) {
			...
			navigableRole = navigableRole.getParent();
		}
	...
	public static boolean interpretToOneKeyDescriptor(
			...
			@Nullable PropertyAccess inversePropertyAccess,
			MappingModelCreationProcess creationProcess) {
		...
		if ( referencedPropertyName != null  ) {
			...
			if ( modelPart == null ) {
				...
				final var embeddedForeignKeyDescriptor = buildEmbeddableForeignKeyDescriptor(
						...
						castNonNull( attributeMapping.findContainingEntityMapping() ),
						null,
						true,
		...
		final var simpleFkTarget = castNonNull( fkTarget ).asBasicValuedModelPart();
		if ( simpleFkTarget != null ) {
			...
			final var foreignKeyDescriptor = new SimpleForeignKeyDescriptor(
					attributeMapping.getDeclaringType(),
					keySelectableMapping,
...
//			creationProcess.registerForeignKey( attributeMapping, embeddedForeignKeyDescriptor );
			...
			throw new UnsupportedOperationException(
					"Support for " + castNonNull( fkTarget ).getClass() + " foreign-keys not yet implemented: " +
							bootProperty.getPersistentClass().getEntityName() + " -> " + bootProperty.getName()
			);
		}
	...
	private static PropertyAccess getDeclaringKeyPropertyAccess(
		...
		if ( inversePropertyAccess == null ) {
			if ( bootValueMapping instanceof OneToOne ) {
				final var identifierMapping = castNonNull( attributeMapping.findContainingEntityMapping() ).getIdentifierMapping();
				...
				if ( identifierMapping instanceof PropertyBasedMapping propertyIdentifierMapping ) {
					return castNonNull( propertyIdentifierMapping.getPropertyAccess() );
				}
			}
			return new ChainedPropertyAccessImpl(
					castNonNull( attributeMapping.getPropertyAccess() ),
					castNonNull( simpleFkTarget.getPropertyAccess() )
			);
		}
		else {
			return new ChainedPropertyAccessImpl(
	...
	private static boolean interpretNestedToOneKeyDescriptor(
		...
		for ( int i = 0; i < propertyPath.length; i++ ) {
			...
			final var modelPart = i == 0
					? referencedEntityDescriptor.findSubPart( path )
					: castNonNull( lastEmbeddableModelPart ).findSubPart( path, null );

			if ( modelPart == null ) {
				return false;
			}
			else if ( modelPart instanceof ToOneAttributeMapping referencedAttributeMapping ) {
				final var foreignKeyDescriptor = referencedAttributeMapping.getForeignKeyDescriptor();
	...
	public static EmbeddedForeignKeyDescriptor buildEmbeddableForeignKeyDescriptor(
			...
			@Nullable String keyTableExpression,
			boolean inverse,
			boolean[] insertable,
			boolean[] updateable,
			MappingModelCreationProcess creationProcess) {
		final var parentSelectablePath = getSelectablePath( keyDeclaringType );
	...
	public static @Nullable SelectablePath getSelectablePath(@Nullable ManagedMappingType type) {
	...
	private static void setReferencedAttributeForeignKeyDescriptor(
		...
		final var declaringEntityMapping = attributeMapping.findContainingEntityMapping();
		final var targetPart = foreignKeyDescriptor.getTargetPart();
		if ( targetPart instanceof EntityIdentifierMapping
				&& targetPart != castNonNull( declaringEntityMapping ).getIdentifierMapping() ) {
			...
			attributeMapping.setForeignKeyDescriptor(
					foreignKeyDescriptor.withTargetPart( castNonNull( declaringEntityMapping ).getIdentifierMapping() )
			);
		}
		else {
			attributeMapping.setForeignKeyDescriptor( foreignKeyDescriptor );
	...
	private static ForeignKeyDescriptor foreignKeyDescriptor(
			ToOneAttributeMapping referencedAttributeMapping,
			EntityPersister referencedEntityDescriptor,
			String referencedPropertyName,
			MappingModelCreationProcess creationProcess) {
		...
		if ( foreignKeyDescriptor == null ) {
			...
			return referencedAttributeMapping.getForeignKeyDescriptor();
		}
		else {
	...
	private static CollectionPart interpretMapKey(
		...
		final var indexedCollection = (IndexedCollection) bootValueMapping;
		final var bootMapKeyDescriptor = indexedCollection.getIndex();
		...
		if ( bootMapKeyDescriptor instanceof Component component ) {
			final var compositeType = component.getType();
			final var mappingType = EmbeddableMappingTypeImpl.from(
					component,
					compositeType,
					component.getColumnInsertability(),
					component.getColumnUpdateability(),
	...
	private static CollectionPart interpretElement(
		...
		if ( element instanceof Component component ) {
			final var compositeType = (CompositeType) collectionDescriptor.getElementType();
			final var mappingType = EmbeddableMappingTypeImpl.from(
					component,
					compositeType,
					component.getColumnInsertability(),
		...
		if ( element instanceof Any anyBootMapping ) {
			return new DiscriminatedCollectionPart(
					...
					creationProcess.getCreationContext().getTypeConfiguration()
							.getJavaTypeRegistry().resolveDescriptor( Object.class ),
					anyBootMapping,
	...
	public static Expression buildColumnReferenceExpression(
		...
		if ( modelPart instanceof EmbeddableValuedModelPart ) {
			...
			final var basicPart = castNonNull( modelPart.asBasicValuedModelPart() );
	...
	public static BasicType<?> resolveAggregateColumnBasicType(
		...
		if ( column instanceof AggregateColumn aggregateColumn ) {
			final var component = aggregateColumn.getComponent();
			final var embeddableRole = navigableRole.append( CollectionPart.Nature.ELEMENT.getName() );
			final var mappingType = EmbeddableMappingTypeImpl.from(
					component,
					component.getType(),
					component.getColumnInsertability(),
					...
					inflightDescriptor -> new EmbeddableValuedModelPart() {
						...
						public SqlTuple toSqlExpression(
							...
							throw new UnsupportedOperationException( "Aggregate type resolution does not support toSqlExpression" );
						}
						...
						public SqlAstJoinType getDefaultSqlAstJoinType(TableGroup parentTableGroup) {
							throw new UnsupportedOperationException( "Aggregate type resolution does not support getDefaultSqlAstJoinType" );
						}
						...
						public TableGroupJoin createTableGroupJoin(
							...
							throw new UnsupportedOperationException( "Aggregate type resolution does not support createTableGroupJoin" );
						}
						...
						public TableGroup createRootTableGroupJoin(
								...
								SqlAstCreationState creationState) {
							throw new UnsupportedOperationException( "Aggregate type resolution does not support createRootTableGroupJoin" );
						}
						...
						public FetchOptions getMappedFetchOptions() {
							throw new UnsupportedOperationException( "Aggregate type resolution does not support getMappedFetchOptions" );
						}
						...
						public Fetch generateFetch(
							...
							throw new UnsupportedOperationException( "Aggregate type resolution does not support generateFetch" );
						}
						...
						public <T> DomainResult<T> createDomainResult(
							...
							throw new UnsupportedOperationException( "Aggregate type resolution does not support createDomainResult" );
						}
			...
			return (BasicType<?>) castNonNull( mappingType.getAggregateMapping() ).getJdbcMapping();
		}
		return null;
	}
	...
	public static ToOneAttributeMapping buildSingularAssociationAttributeMapping(
		...
		if ( bootProperty.getValue() instanceof ToOne value ) {
			final var entityPersister = creationProcess.getEntityPersister( value.getReferencedEntityName() );
			final var attributeMetadata = getAttributeMetadata(
					bootProperty,
					attrType,
					propertyAccess,
					cascadeStyle,
	...
	private static FetchTiming fetchTiming(Property bootProperty, ManagedMappingType declaringType, ToOne value, EntityPersister entityPersister, FetchStyle fetchStyle, AssociationType type, SessionFactoryImplementor sessionFactory) {
		final String role = castNonNull( declaringType.getNavigableRole() ).toString() + "." + bootProperty.getName();
