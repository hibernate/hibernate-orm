...
public class PropertyAccessorServiceInitiator implements StandardServiceInitiator<PropertyAccessorService> {
	...
	public PropertyAccessorService initiateService(
		...
		final AccessorStrategy strategy = AccessorStrategy.interpret(
				getString( ACCESSOR_STRATEGY, configurationValues, AccessorStrategy.BYTE_BUDDY.getConfigValue() )
		);
		return switch ( strategy ) {
			case BYTE_BUDDY -> new ByteBuddyPropertyAccessorService( configurationValues );
			case REFLECTION -> new DelegatingPropertyAccessorService( AccessorFactory.reflection() );
			case REFLECTION_NO_MULTI -> new DelegatingPropertyAccessorService( new ReflectionNoMultiAccessorFactory() );
		};
	}
	...
	private enum AccessorStrategy {
		...
		/**
		 * Uses plain {@code java.lang.reflect} for all accessors, including multi-value.
		 */
		REFLECTION( "reflection" ),

		/**
		 * Uses reflection for individual accessors but disables multi-value accessor generation,
		 * falling back to per-property access.
		 */
		REFLECTION_NO_MULTI( "reflection-no-multi" );
		...
		public static AccessorStrategy interpret(Object strategy) {
			if ( strategy == null ) {
				return AccessorStrategy.BYTE_BUDDY;
			}
