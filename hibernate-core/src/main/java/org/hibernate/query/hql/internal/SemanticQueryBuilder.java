...
public class SemanticQueryBuilder extends HqlParserBaseVisitor implements SqmCreationState {
	...
	@Override
	public Object visitCurrentDate(HqlParser.CurrentDateContext ctx) {
		return getFunctionTemplate("current date")
	...
	@Override
	public SqmExpression visitCurrentTime(HqlParser.CurrentTimeContext ctx) {
		return getFunctionTemplate("current time")
				.makeSqmFunctionExpression(
	...
	@Override
	public SqmExpression visitCurrentDatetime(HqlParser.CurrentDatetimeContext ctx) {
		return getFunctionTemplate("current datetime")
				.makeSqmFunctionExpression(
	...
	public SqmExpression visitCurrentInstantFunction(HqlParser.CurrentInstantFunctionContext ctx) {
		return getFunctionTemplate("current_instant")
				.makeSqmFunctionExpression(
						basicType( Instant.class ),
						creationContext.getQueryEngine(),
						creationContext.getDomainModel().getTypeConfiguration()
	...
	public SqmLiteral visitLiteralExpression(HqlParser.LiteralExpressionContext ctx) {
		...
		else if ( ctx.literal().escapedDateLiteral() != null ) {
			return sqlDateLiteralFrom( ctx.literal().escapedDateLiteral().dateTimeLiteralText().getText() );
		}
		else if ( ctx.literal().escapedTimeLiteral() != null ) {
			return sqlTimeLiteralFrom( ctx.literal().escapedTimeLiteral().dateTimeLiteralText().getText() );
		}
		else {
	...
	public Object visitDateOrTimeField(HqlParser.DateOrTimeFieldContext ctx) {
		NodeBuilder nodeBuilder = creationContext.getNodeBuilder();
		if (ctx.DATE()!=null) {
			return new SqmExtractUnit<>(DATE, basicType( LocalDate.class ), nodeBuilder);
		}
		if (ctx.TIME()!=null) {
			return new SqmExtractUnit<>(TIME, basicType( LocalTime.class ), nodeBuilder);
		}
	...
	private SqmCastTarget<?> interpretCastTarget(HqlParser.CastTargetContext castTargetContext) {
		...
		switch (text.toLowerCase()) {
			case "string": return new SqmCastTarget<>( basicType( String.class ), nodeBuilder );
			case "integer": return new SqmCastTarget<>( basicType( Integer.class ), nodeBuilder );
			case "long": return new SqmCastTarget<>( basicType( Long.class ), nodeBuilder );
			case "float": return new SqmCastTarget<>( basicType( Float.class ), nodeBuilder );
			case "double": return new SqmCastTarget<>( basicType( Double.class ), nodeBuilder );
			case "time": return new SqmCastTarget<>( basicType( Time.class ), nodeBuilder );
			case "date": return new SqmCastTarget<>( basicType( Date.class ), nodeBuilder );
			case "timestamp": return new SqmCastTarget<>( basicType( Timestamp.class ), nodeBuilder );
