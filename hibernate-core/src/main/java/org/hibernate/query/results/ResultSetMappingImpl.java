...
public class ResultSetMappingImpl implements ResultSetMapping {
	...
	private Map<Fetchable, Map<Fetchable, DynamicFetchBuilderLegacy>> legacyFetchBuilders;
	...
	private ResultSetMappingImpl(ResultSetMappingImpl original) {
		...
		if ( !original.isDynamic || original.legacyFetchBuilders == null ) {
			...
			final Map<Fetchable, Map<Fetchable, DynamicFetchBuilderLegacy>> legacyFetchBuilders = new HashMap<>( original.legacyFetchBuilders.size() );
			for ( Map.Entry<Fetchable, Map<Fetchable, DynamicFetchBuilderLegacy>> entry : original.legacyFetchBuilders.entrySet() ) {
	...
	public void visitLegacyFetchBuilders(Consumer<DynamicFetchBuilderLegacy> resultBuilderConsumer) {
		...
		for ( Map.Entry<Fetchable, Map<Fetchable, DynamicFetchBuilderLegacy>> entry : legacyFetchBuilders.entrySet() ) {
	...
	public void addLegacyFetchBuilder(DynamicFetchBuilderLegacy fetchBuilder) {
		...
		if ( existingFetchBuildersByOwner == null ) {
			...
			legacyFetchBuilders.put( fetchBuilder.getFetchable(), fetchBuildersByOwner );
		}
		else {
