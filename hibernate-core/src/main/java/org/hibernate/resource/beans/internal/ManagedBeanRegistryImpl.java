...
public class ManagedBeanRegistryImpl implements ManagedBeanRegistry, BeanContainer.LifecycleOptions, Stoppable {
	...
	@Override
	public <T>ManagedBean<T> getUncachedBootstrapSafeBean(Class<T> beanClass){
		if ( beanContainer == null ) {
			return new FallbackContainedBean<>( beanClass, FallbackBeanInstanceProducer.INSTANCE );
		}
		return new DelayedBeanImpl<>(
				beanClass,
				JpaCompliantLifecycleStrategy.INSTANCE,
				FallbackBeanInstanceProducer.INSTANCE,
				beanContainer
