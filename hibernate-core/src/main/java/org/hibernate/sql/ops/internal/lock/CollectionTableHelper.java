...
/**
 * Helper for dealing with collection-table follow-on locking.
 *
...
public class CollectionTableHelper {
	public static void lockCollectionTable(
		...
		if ( DB_OP_DEBUG_ENABLED ) {
			DB_OP_LOGGER.debugf( "Follow-on locking for collection table `%s` - %s", keyTableName, attributeMapping.getRootPathName() );
		}
