 ...
 */
final class StatsConcurrentHashMap<V> {
	 ...
	 */
	StatsConcurrentHashMap(int capacity, int concurrencyLevel) {
	...
	public String[] keysAsArray() {
		//We don't allocate an array directly, nor use streams, as its a concurrent map which we don't want to lock:
		ArrayList<String> collector = new ArrayList<>( map.size() );
