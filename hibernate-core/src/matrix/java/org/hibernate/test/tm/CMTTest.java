...
public class CMTTest extends BaseCoreFunctionalTestCase {
	...
	public void testConcurrentCachedDirtyQueries() throws Exception {
		...
		// the expected value was 2, which is incorrect IMO.
		...
		assertEquals( 1, sessionFactory().getStatistics().getUpdateTimestampsCachePutCount() );  //was 2

		// the excepted value here was 4, which I think is incorrect.
		...
		// since there is only 1 update action, I don't see where 4 came from.
		assertEquals( 1, sessionFactory().getStatistics().getUpdateTimestampsCacheHitCount() );  //was 4

		TestingJtaBootstrap.INSTANCE.getTransactionManager().resume( tx1 );
		TestingJtaBootstrap.INSTANCE.getTransactionManager().commit();

		// update action's TX committed, so, invalidate is called, put new timestamp into UpdateTimestampsCache
		// the expect value was 3
		assertEquals( sessionFactory().getStatistics().getUpdateTimestampsCachePutCount(), 2 );  //was 3
		// was 5, but no more query cache lookup here, so it should still 1
		assertEquals( sessionFactory().getStatistics().getUpdateTimestampsCacheHitCount(), 1 );  //was 5
		...
		assertEquals( 2, sessionFactory().getStatistics().getUpdateTimestampsCacheHitCount() );  //was 6
		...
		assertEquals( 3, sessionFactory().getStatistics().getQueryCacheMissCount() );
		assertEquals( 3, sessionFactory().getStatistics().getUpdateTimestampsCacheHitCount() );   //was 7
