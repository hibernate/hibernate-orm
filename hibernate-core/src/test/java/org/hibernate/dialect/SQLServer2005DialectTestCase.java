...
public class SQLServer2005DialectTestCase extends TestCase {
	...
	public void testGetLimitString() {
		...
		assertEquals( "with query as (select row_number() over (order by f234, f67 desc) as __hibernate_row_nr__, f1 as f53245 from table849752  group by f1) select * from query where __hibernate_row_nr__ between ? and ? order by __hibernate_row_nr__", sqlDialect.getLimitString(input, 10, 15).toLowerCase() );
	}
}
