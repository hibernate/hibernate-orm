...
public class OneToManyCompositeKeyTest extends EnversEntityManagerFactoryBasedFunctionalTest {
	...
	public void testRevisionCounts() {
		assertThat( getAuditReader().getRevisions( Contract.class, 1 ), contains( 1 ) );
		assertThat( getAuditReader().getRevisions( Design.class, 1 ), contains( 1 ) );
	}

	@DynamicTest
	@Disabled("NYI - UnknownPathException thrown resolving path `contract_id` - ManyToOne inside EmbeddedId")
	public void testOneToManyAssociationAuditQuery() {
		inTransaction(
				entityManager -> {
					...
					assertThat( contract.getDesigns(), CollectionMatchers.hasSize( 1 ) );

					final DesignContract designContract = contract.getDesigns().iterator().next();
					assertThat( designContract.getContract().getId(), equalTo( contract.getId() ) );
					assertThat( designContract.getDesign().getId(), equalTo( design.getId() ) );
				}
		);
	}
