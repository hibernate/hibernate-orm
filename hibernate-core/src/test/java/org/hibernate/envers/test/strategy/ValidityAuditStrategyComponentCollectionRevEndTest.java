...
public class ValidityAuditStrategyComponentCollectionRevEndTest extends EnversEntityManagerFactoryBasedFunctionalTest {
	...
	public void prepareAuditData() {
		// Revision 1
		this.productId = inTransaction( entityManager -> {
	...
	public void testRevision1() {
		...
		assertThat( getItemNamesFromProduct( product ), contains( "bread" ) );
	}

	@DynamicTest
	public void testRevision2() {
		final Product product = getAuditReader().find( Product.class, productId, 2 );
		assertThat( getItemNamesFromProduct( product ), contains( "bread", "bread2" ) );
	}

	@DynamicTest
	public void testRevision3() {
		final Product product = getAuditReader().find( Product.class, productId, 3 );
		assertThat( getItemNamesFromProduct( product ), contains( "bread2" ) );
	}

	private static List<String> getItemNamesFromProduct(Product product) {
		return product.getItems().stream().map( Item::getName ).collect( Collectors.toList() );
	}
