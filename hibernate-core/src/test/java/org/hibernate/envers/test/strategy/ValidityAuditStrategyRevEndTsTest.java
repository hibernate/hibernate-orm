...
public class ValidityAuditStrategyRevEndTsTest extends EnversEntityManagerFactoryBasedFunctionalTest {
	...
	public void prepareAuditData() {
		...
		assertThat( getAuditReader().findRevisions( SequenceIdRevisionEntity.class, revisions ).entrySet(), hasSize( 5 ) );
	}
	...
	private void verifyRevEndTimeStamps(String debugInfo, List<Map<String, Object>> revisionEntities) {
		for ( Map<String, Object> revisionEntity : revisionEntities ) {
			...
			if ( revendTimestamp == null ) {
				...
				if ( isDialect( MySQL5Dialect.class ) && !isDialect( MySQL57Dialect.class, MariaDB53Dialect.class ) ) {
					...
					assertThat( debugInfo, revEnd.getTimestamp() - ( revEnd.getTimestamp() % 1000 ), equalTo( revendTimestamp.getTime() ) );
				}
				else if ( isDialect( SybaseASE15Dialect.class ) ) {
					// Sybase "DATETIME values are accurate to 1/300 second on platforms that support this level of granularity".
					assertThat( debugInfo, revEnd.getTimestamp() / 1000.0, closeTo( revendTimestamp.getTime() / 1000.0, 1.0 / 300.0 ) );
				}
				else {
					assertThat( debugInfo, revEnd.getTimestamp(), equalTo( revendTimestamp.getTime() ) );
				}
			}
		}
	}
	...
	private boolean isDialect(Class<?>... dialectClasses) {
