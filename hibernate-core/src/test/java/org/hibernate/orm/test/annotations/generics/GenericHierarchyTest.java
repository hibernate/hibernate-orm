...
@DomainModel(
		annotatedClasses = {
				GenericHierarchyTest.CommonEntity.class,
				GenericHierarchyTest.SpecificEntity.class
		}
)
@SessionFactory
@JiraKey(value = "HHH-19872")
public class GenericHierarchyTest implements SessionFactoryScopeAware {
	...
	void testGenericHierarchy(Consumer<UpdateContext> updater) {
		scope.inTransaction( session -> {
			...
			criteriaUpdate.where( cb.equal( root.get( GenericHierarchyTest_.SpecificEntity_.id ), base.getId() ) );
	...
	static Stream<Arguments> criteriaUpdateFieldSetters() {
		Consumer<UpdateContext> updateUsingPath = context ->
				context.query.set( context.root.get( GenericHierarchyTest_.SpecificEntity_.relative ), context.relative );
		Consumer<UpdateContext> updateUsingSingularAttribute = context ->
				context.query.set( GenericHierarchyTest_.SpecificEntity_.relative, context.relative );
