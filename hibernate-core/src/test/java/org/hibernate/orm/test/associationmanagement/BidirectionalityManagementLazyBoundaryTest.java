...
class BidirectionalityManagementLazyBoundaryTest {
	@AfterEach
	...
	@Test
	void manyToOneReassignmentDoesNotInitializeLazyInverseCollection(SessionFactoryScope scope) {
		...
		scope.inTransaction( session -> {
			...
			// reassign parent and flush, should issue the update without initializing
			// newParent#children solely for bidirectionality management
	...
	@Test
	void manyToOneReassignmentDoesNotInitializePreviouslyLoadedParentCollection(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
			final Parent oldParent = new Parent( 1 );
			final Parent newParent = new Parent( 2 );
			final Child child = new Child( 1, oldParent );
		...
		scope.inTransaction( session -> {
			...
			assertThat( Hibernate.isInitialized( oldParent.children ) ).isFalse();
			assertThat( Hibernate.isInitialized( newParent.children ) ).isFalse();

			// load of oldParent + load of newParent + load of child
	...
	@Test
	void owningManyToManyCollectionDoesNotInitializeLazyInverseCollections(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
