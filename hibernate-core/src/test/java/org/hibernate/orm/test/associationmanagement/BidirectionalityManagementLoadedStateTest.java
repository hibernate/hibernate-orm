...
class BidirectionalityManagementLoadedStateTest {
	@AfterEach
	...
	@Test
	void manyToOneReassignmentSynchronizesLoadedInverseCollections(SessionFactoryScope scope) {
		...
		scope.inTransaction( session -> {
			...
			// Initialize the two inverse collections so automatic management can update them.
	...
	@Test
	void owningOneToOneReassignmentSynchronizesInverseReferences(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
	...
	@Test
	void owningManyToManyCollectionSynchronizesLoadedInverseCollections(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
