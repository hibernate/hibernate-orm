...
@ServiceRegistry(settings = @Setting( name = AvailableSettings.DIALECT_NATIVE_PARAM_MARKERS, value = "false" ))
	...
	public void testBatchInitializeChildCollection(SessionFactoryScope scope){
		...
		scope.inTransaction(session -> {
			...
			if ( scope.getSessionFactory().getJdbcServices().getDialect().useArrayForMultiValuedParameters() ) {
				...
				assertThat( sqlCollector.getSqlQueries().get( 1 ) ).containsOnlyOnce( "in (?,?,?,?,?)" );
			}
		} );
	}
