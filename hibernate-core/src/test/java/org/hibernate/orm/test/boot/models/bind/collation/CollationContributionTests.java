	...
	void testCollationContribution(ServiceRegistryScope scope) {
		checkDomainModel(
				(context) -> {
					...
					final var nameProperty = entityBinding.getProperty( "name" );
					final BasicValue nameValue = (BasicValue) nameProperty.getValue();
					assertThat( ( (Column) nameValue.getColumn() ).getCollation() ).isEqualTo( "ucs_basic" );
					final var nameHandoff = context.getBindingState().getCollationPropertyHandoff( nameProperty );
					assertThat( nameHandoff ).isNotNull();
					assertThat( nameHandoff.contribution().contribution() ).isSameAs( nameContribution.contribution() );
					assertThat( nameHandoff.property() ).isSameAs( nameProperty );
					assertThat( context.getBindingState().getCollationPropertyHandoffs( nameContribution ) )
							.containsExactly( nameHandoff );

					final Component details = (Component) entityBinding.getProperty( "details" ).getValue();
					final var descriptionProperty = details.getProperty( "description" );
					final BasicValue descriptionValue = (BasicValue) descriptionProperty.getValue();
					assertThat( ( (Column) descriptionValue.getColumn() ).getCollation() ).isEqualTo( "en_US" );
					final var descriptionHandoff = context.getBindingState()
							.getCollationPropertyHandoff( descriptionProperty );
					assertThat( descriptionHandoff ).isNotNull();
					assertThat( descriptionHandoff.contribution().contribution() )
							.isSameAs( nestedContribution.contribution() );
					assertThat( descriptionHandoff.property() ).isSameAs( descriptionProperty );
					assertThat( context.getBindingState().getCollationPropertyHandoffs( nestedContribution ) )
							.containsExactly( descriptionHandoff );
				},
				scope.getRegistry(),
