	...
	void testNaturalIdContribution(ServiceRegistryScope scope) {
		checkDomainModel(
				(context) -> {
					...
					final var handoff = context.getBindingState().getNaturalIdPropertyHandoff( property );
					assertThat( handoff ).isNotNull();
					assertThat( handoff.contribution().contribution() ).isSameAs( contribution.contribution() );
					assertThat( handoff.property() ).isSameAs( property );
					assertThat( context.getBindingState().getNaturalIdPropertyHandoffs( contribution ) )
