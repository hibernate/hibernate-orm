	...
	void testSimpleSecondaryTest(ServiceRegistryScope scope) {
		checkDomainModel(
				(context) -> {
					...
					assertThat( context.getBindingState().getTableByBinding( join.getTable() ).binding() )
							.isSameAs( join.getTable() );
					assertThat( context.getBindingState().getSecondaryTable( join.getTable() ).binding() )
							.isSameAs( join.getTable() );
