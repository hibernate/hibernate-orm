	...
	void testSimpleTenancy(ServiceRegistryScope scope) {
		checkDomainModel(
				(context) -> {
					...
					final var handoff = context.getBindingState().getTenantIdPropertyHandoff( tenantProperty );
					assertThat( handoff ).isNotNull();
					assertThat( handoff.binding().binding() ).isSameAs( tenantIdBinding.binding() );
					assertThat( handoff.rootClass() ).isSameAs( entityBinding );
					assertThat( handoff.property() ).isSameAs( tenantProperty );
					assertThat( context.getBindingState().getTenantIdPropertyHandoff( (org.hibernate.mapping.RootClass) entityBinding ) )
							.isSameAs( handoff );
					assertThat( context.getBindingState().getTenantIdPropertyHandoff( tenantIdBinding ) )
							.isSameAs( handoff );
