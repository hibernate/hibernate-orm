 ...
 */
public class OneToManyNonJoinTableMapTest extends SessionFactoryBasedFunctionalTest {
	@Entity(name = "Owner")
	...
	public void testOperations() {
		...
		sessionFactoryScope().inTransaction(
				session -> {
					...
					final Owner owner = new Owner();
					owner.getCars().put( this.carId1, car );
					session.save( owner );
		...
		sessionFactoryScope().inTransaction(
				session -> {
					...
					final Owner owner = new Owner();
					owner.getCars().put( this.carId1, car1 );
					owner.getCars().put( this.carId2, car2 );
					session.save( owner );
		...
		// Update ownerId2, add carId1 to empty collection
		// Removes carId1 from ownerId1
		...
		sessionFactoryScope().inTransaction(
				session -> {
					final Owner owner = session.find( Owner.class, this.ownerId2 );
					owner.getCars().remove( this.carId1 );
					session.update( owner );
				}
		);
		...
		sessionFactoryScope().inTransaction(
				session -> {
					...
					final HashMap<Integer, Car> collection = new HashMap<>();
					collection.put( car1.getId(), car1 );
					collection.put( car2.getId(), car2 );
