...
@DomainModel(annotatedClasses = EmbeddableAsElementCollectionUpdateTest.Person.class)
@SessionFactory
@JiraKey(value = "HHH-16297")
public class EmbeddableAsElementCollectionUpdateTest {
	...
	public void removeElementAndAddNewOne(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
			...
			addresses.add( new Address( "via Pierce", null ) );
			assertThat( addresses )
					.containsExactlyInAnyOrder(
							new Address( "via Pierce", null ),
							new Address( "via Cartesio", "Milano" )
					);
		} );
		scope.inTransaction( session -> {
			...
			addresses.remove( new Address( "via Pierce", null ) );
		} );
