	...
	public void tearDown(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					...
					List<EntityA> entityAs = session.createQuery( "select a from EntityA a", EntityA.class ).list();
					entityAs.forEach(
							entityA -> {
								ComponentA componentA = entityA.getComponentA();
								if ( componentA != null ) {
									componentA.setEntityA( null );
									session.flush();
								}
