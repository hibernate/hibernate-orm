...
@DomainModel(
		annotatedClasses = {
				NestedEmbeddableCurrentTimeStampInsertEventTimeTest.TestEntity.class
		}
)
@SessionFactory
@JiraKey("HHH-18756")
public class NestedEmbeddableCurrentTimeStampInsertEventTimeTest {
	...
	public void testCurrentTimeStamp(SessionFactoryScope scope) {
		...
		scope.inTransaction(
				session -> {
					...
					testEntity.getAnEmbeddable().setTimestamp( new Date() );
					testEntity.getAnotherEmbeddable().setAnotherTimestamp( new Date() );
		...
		scope.inTransaction(
				session -> {
					...
					anotherEmbeddable.setAnotherTimestamp( new Date( 1971, 1, 1 ) );
				}
		);
		...
		scope.inTransaction( session -> {
			...
			assertThat( anotherEmbeddable ).isNotNull();
			assertThat( anotherEmbeddable.getAnotherString() ).isEqualTo( anotherValue );
			assertThat( anotherEmbeddable.getAnotherTimestamp() ).isNotNull();
		} );
	}
