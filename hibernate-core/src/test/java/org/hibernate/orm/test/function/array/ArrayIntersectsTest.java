...
@RequiresDialectFeature(feature = DialectFeatureChecks.SupportsArrayIntersects.class)
...
//			cb.collectionIntersectsNullable( Collections.<Integer>emptyList(), cb.literal( "" ) );
	...
	public void testArrayParameter(SessionFactoryScope scope) {
		scope.inSession( em -> {
			List<EntityWithArrays> results = em.createQuery( "from EntityWithArrays e where array_intersects(e.theArray, :array)", EntityWithArrays.class )
