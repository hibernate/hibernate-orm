...
@BootstrapServiceRegistry(
		javaServices = @BootstrapServiceRegistry.JavaService(role = FunctionContributor.class, impl = TemporalTypeTest.MyFunctionContributor.class)
)
...
public class TemporalTypeTest {
	...
	@Test
	@Jira("https://hibernate.atlassian.net/browse/HHH-20021")
	public void testTemporalTypeCalendar(SessionFactoryScope scope) {
		...
		TypeConfiguration typeConfiguration = scope.getSessionFactory().getTypeConfiguration();
		...
		typeConfiguration.getBasicTypeForJavaType( GregorianCalendar.class );
		scope.inTransaction(
				entityManager -> {
