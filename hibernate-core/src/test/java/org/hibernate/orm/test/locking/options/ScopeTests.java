...
//			Helper.checkSql( sqlCollector.getSqlQueries().get( 0 ), session.getDialect(), Helper.Table.BOOK_GENRES );
	...
	void testLock(SessionFactoryScope factoryScope) {
		...
		factoryScope.inTransaction( (session) -> {
			...
			final PessimisticLockStyle lockStyle = session
					.getDialect()
					.getLockingSupport()
					.getMetadata()
					.getPessimisticLockStyle();
			if ( lockStyle == PessimisticLockStyle.CLAUSE ) {
				final String expectedClause = session.getDialect().getWriteLockString( BOOKS.getTableAlias(), Timeouts.WAIT_FOREVER );
				assertThat( sql ).endsWith( expectedClause );
			}
			else if ( lockStyle == PessimisticLockStyle.TABLE_HINT ) {
				// t-sql variant...
				final String booksTableReference = buildHintedTableReference( session );
	...
	private static String buildHintedTableReference(SessionImplementor session) {
		final String lockHint;
		if ( session.getDialect() instanceof SQLServerDialect ) {
			lockHint = SQLServerSqlAstTranslator.determineLockHint( PESSIMISTIC_WRITE, Timeouts.WAIT_FOREVER_MILLI );
		}
		else if ( session.getDialect() instanceof SybaseDialect ) {
			lockHint = SybaseASESqlAstTranslator.determineLockHint( PESSIMISTIC_WRITE, Timeouts.WAIT_FOREVER_MILLI );
		}
		else {
			throw new RuntimeException( "Unexpected Dialect : " + session.getDialect().getClass().getName() );
		}
		return BOOKS.getTableName() + " " + BOOKS.getTableAlias() + lockHint;
	}
