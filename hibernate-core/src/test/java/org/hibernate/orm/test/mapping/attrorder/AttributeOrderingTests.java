...
public class AttributeOrderingTests {
	...
	public void verifyRuntimeEntityMapping(EntityMappingType entityMappingType) {
		...
		assertThat( attributeMappings.getAttributeMapping( 0 ).getAttributeName(), is( "assignment" ) );
		assertThat( entityMappingType.getEntityPersister().getPropertyNames()[ 0 ], is( "assignment" ) );

		assertThat( attributeMappings.getAttributeMapping( 1 ).getAttributeName(), is( "name" ) );
		assertThat( entityMappingType.getEntityPersister().getPropertyNames()[ 1 ], is( "name" ) );

		final EmbeddedAttributeMapping theComponentAttrMapping = (EmbeddedAttributeMapping) attributeMappings.getAttributeMapping( 2 );
		...
		assertThat( embeddableAttributeMappings.getAttributeMapping( 0 ).getAttributeName(), is( "nestedAnything" ) );
		assertThat( embeddableAttributeMappings.getAttributeMapping( 1 ).getAttributeName(), is( "nestedName" ) );

		assertThat( attributeMappings.getAttributeMapping( 3 ).getAttributeName(), is( "theComponents" ) );
		assertThat( entityMappingType.getEntityPersister().getPropertyNames()[ 3 ], is( "theComponents" ) );

		assertThat( attributeMappings.getAttributeMapping( 4 ).getAttributeName(), is( "userCode" ) );
