...
public class ZonedDateTimeMappingTests {
	...
	public void verifyMappings(SessionFactoryScope scope) {
		...
		final Matcher<Integer> expectedJdbcType;
		if ( sf.getSessionFactoryOptions().isDirectJavaTimeJdbcAccessEnabled( ZonedDateTime.class ) ) {
			expectedJdbcType = equalTo( SqlTypes.ZONED_DATE_TIME );
		}
		else if ( sf.getSessionFactoryOptions().isDirectJavaTimeJdbcAccessEnabled( OffsetDateTime.class ) ) {
			expectedJdbcType = equalTo( SqlTypes.OFFSET_DATE_TIME );
		}
		else {
			expectedJdbcType = isOneOf( SqlTypes.TIMESTAMP, SqlTypes.TIMESTAMP_WITH_TIMEZONE );
		}
