	...
	public void setUp(SessionFactoryScope scope) {
		...
		scope.inTransaction( session -> {
			session.createNativeMutationQuery(
					"insert into otm_join_table(container_id, basic_id) values (2,1)"
			).executeUpdate();
			session.createNativeMutationQuery(
					"insert into otm_join_table(container_id, basic_id) values (2,2)"
			).executeUpdate();
			session.createNativeMutationQuery(
			...
			session.createNativeMutationQuery( "insert into TargetEntity(id, container_id) values (98,2)" )
					.executeUpdate();
			session.createNativeMutationQuery( "insert into TargetEntity(id, container_id) values (99,2)" )
					.executeUpdate();
		} );
	}
