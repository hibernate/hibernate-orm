...
public class AttributeConverterAndFunctionTest {
	...
	public void setup(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> session.persist( new EntityWithEnum( 1L, State.SOMESTATE ) )
		);
	}
	...
	public void tearDown(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> session.createMutationQuery( "delete from EntityWithEnum" ).executeUpdate()
		);
	}

	@Test
	@JiraKey( "HHH-18474" )
	@FailureExpected(reason = "Converter ignored when using function on enum field")
