...
public abstract class AbstractCompositeIdAndNaturalIdTest {
		...
		scope.inTransaction(
				(session) -> {
					...
					Account account = new Account(
							new AccountId( 1 ), NATURAL_ID_VALUE );
	...
	public void testNaturalIdCriteria(SessionFactoryScope scope) {
		scope.inTransaction(
				(session) -> {
					final Account account = session.bySimpleNaturalId(
							Account.class ).load( NATURAL_ID_VALUE );
	...
	public void testNaturalIdApi(SessionFactoryScope scope) {
		...
		scope.inTransaction(
				(session) -> {
					final Account account = session.byNaturalId(
							Account.class ).using( "shortCode", NATURAL_ID_VALUE ).load();
					assertThat( account, notNullValue() );
				}
		);
	}
