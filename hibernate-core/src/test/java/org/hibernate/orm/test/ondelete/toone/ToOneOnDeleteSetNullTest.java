...
@SessionFactory
@SkipForDialect( dialectClass = SpannerPostgreSQLDialect.class, reason = "Spanner doesn't support ON DELETE SET DEFAULT")
public class ToOneOnDeleteSetNullTest {
	...
	public void testManyToOne(SessionFactoryScope scope) {
		...
		scope.inTransaction( session -> {
			assertNotNull( session.find(Child.class, 1L).parent );
			assertNotNull( session.find(Child.class, 2L).parent );
			assertNotNull( session.find(GrandChild.class, 2L).parent );
			assertNotNull( session.find(GrandChild.class, 3L).parent );
		} );
		...
		scope.inTransaction( session -> {
			assertNull( session.find(Child.class, 1L).parent );
			assertNull( session.find(Child.class, 2L).parent );
			assertNotNull( session.find(GrandChild.class, 2L).parent );
			assertNotNull( session.find(GrandChild.class, 3L).parent );
