	...
	public void currentTimestampTests(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					...
					session.createQuery( "select e from EntityOfBasics e where e.theTimestamp = current_timestamp" )
							.list();
					session.createQuery( "select e from EntityOfBasics e where e.theTimestamp = current_timestamp()" )
							.list();

					session.createQuery(
									"select e from EntityOfBasics e where current_timestamp between e.theTimestamp and e.theTimestamp" )
							.list();
					session.createQuery(
									"select e from EntityOfBasics e where current_timestamp() between e.theTimestamp and e.theTimestamp" )
							.list();
	...
	public void currentDateTests(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					...
					session.createQuery(
									"select e from EntityOfBasics e where current_date between e.theDate and e.theDate" )
							.list();
					session.createQuery(
									"select e from EntityOfBasics e where current_date() between e.theDate and e.theDate" )
							.list();
	...
	public void currentInstantTests(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					...
					session.createQuery( "select e from EntityOfBasics e where e.theInstant = current_instant()" )
							.list();

					session.createQuery(
									"select e from EntityOfBasics e where current_instant between e.theInstant and e.theInstant" )
							.list();
					session.createQuery(
									"select e from EntityOfBasics e where current_instant() between e.theInstant and e.theInstant" )
							.list();
	...
	public void localDateTimeTests(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					...
					session.createQuery( "select e from EntityOfBasics e where e.theTimestamp = local_datetime" )
							.list();
					session.createQuery( "select e from EntityOfBasics e where e.theTimestamp = local_datetime()" )
							.list();

					session.createQuery(
									"select e from EntityOfBasics e where local_datetime between e.theTimestamp and e.theTimestamp" )
							.list();
					session.createQuery(
									"select e from EntityOfBasics e where local_datetime() between e.theTimestamp and e.theTimestamp" )
							.list();
	...
	public void localDateTests(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					...
					session.createQuery(
							"select e from EntityOfBasics e where local_date between e.theDate and e.theDate" ).list();
					session.createQuery(
									"select e from EntityOfBasics e where local_date() between e.theDate and e.theDate" )
							.list();
	...
	public void localTimeTests(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					...
					session.createQuery(
									"select e from EntityOfBasics e where local_time between e.theLocalTime and e.theLocalTime" )
							.list();
					session.createQuery(
									"select e from EntityOfBasics e where local_time() between e.theLocalTime and e.theLocalTime" )
							.list();
...
//							.list();
					session.createQuery(
									"select coalesce(nullif(e.gender,org.hibernate.testing.orm.domain.gambit.EntityOfBasics$Gender.FEMALE), e.gender) from EntityOfBasics e" )
							.list();
					session.createQuery( "select coalesce(nullif(e.gender,?1), e.gender) from EntityOfBasics e" )
							.setParameter( 1, FEMALE )
							.list();
					session.createQuery(
									"select ifnull(e.gender, org.hibernate.testing.orm.domain.gambit.EntityOfBasics$Gender.MALE) from EntityOfBasics e" )
							.list();
				}
		);
	}
	...
	public void testTrigFunctions(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					session.createQuery(
									"select sin(e.theDouble), cos(e.theDouble), tan(e.theDouble), asin(e.theDouble), acos(e.theDouble), atan(e.theDouble) from EntityOfBasics e" )
							.list();
	...
	public void testMathFunctions(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					session.createQuery(
									"select abs(e.theInt), sign(e.theInt), mod(e.theInt, 2) from EntityOfBasics e" )
							.list();
					session.createQuery(
									"select abs(e.theDouble), sign(e.theDouble), sqrt(e.theDouble) from EntityOfBasics e" )
							.list();
	...
	public void testSubstrInsideConcat(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					session.createQuery(
									"select concat('111', concat('222222', '1')) from EntityOfBasics s where s.id = :id" )
							.setParameter( "id", 1 )
							.list();
				}
		);
		...
		scope.inTransaction(
				session -> {
					session.createQuery( "select function('timestampadd',month,2,current date) from EntityOfBasics e" )
							.list();
					session.createQuery(
									"select function('timestampdiff',hour,e.theTimestamp,current timestamp) from EntityOfBasics e" )
							.list();

					session.createQuery( "select timestampadd(month,2,local date) from EntityOfBasics e" )
							.list();
					session.createQuery(
									"select timestampdiff(hour,e.theTimestamp,local datetime) from EntityOfBasics e" )
							.list();
				}
		);
	}
	...
	public void testLengthFunction(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					session.createQuery(
									"select length(e.theString) from EntityOfBasics e where length(e.theString) > 1" )
							.list();
				}
		);
	}
	...
	public void testCastFunction(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					session.createQuery(
									"select cast(e.theDate as string), cast(e.theTime as string), cast(e.theTimestamp as string) from EntityOfBasics e" )
							.list();
					session.createQuery(
									"select cast(e.id as string), cast(e.theInt as string), cast(e.theDouble as string) from EntityOfBasics e" )
							.list();
				}
		);
	}
	...
	public void testStrFunction(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					session.createQuery(
									"select str(e.theDate), str(e.theTime), str(e.theTimestamp) from EntityOfBasics e" )
							.list();
	...
	public void testIntervalDiffExpressions(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					...
					session.createQuery(
									"select (e.theTimestamp - e.theTimestamp + 4 day) by second from EntityOfBasics e" )
							.list();
					session.createQuery(
									"select (e.theTimestamp - (e.theTimestamp + 4 day)) by second from EntityOfBasics e" )
							.list();
					session.createQuery(
									"select (e.theTimestamp + 4 day - e.theTimestamp) by second from EntityOfBasics e" )
							.list();
					session.createQuery(
									"select (e.theTimestamp + 4 day - 2 hour - e.theTimestamp) by second from EntityOfBasics e" )
							.list();
					session.createQuery(
									"select (e.theTimestamp - e.theTimestamp + 4 day + 2 hour) by second from EntityOfBasics e" )
							.list();
					session.createQuery(
									"select (e.theTimestamp - (e.theTimestamp + 4 day + 2 hour)) by second from EntityOfBasics e" )
							.list();
					session.createQuery(
									"select (e.theTimestamp + (4 day - 1 week) - e.theTimestamp) by second from EntityOfBasics e" )
							.list();
					session.createQuery(
									"select (e.theTimestamp - e.theTimestamp + (4 day + 2 hour)) by second from EntityOfBasics e" )
							.list();
...
//							.list();
	...
	public void testExtractFunction(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					...
					session.createQuery(
									"select extract(time from e.theTimestamp), extract(date from e.theTimestamp) from EntityOfBasics e" )
							.list();
					session.createQuery(
									"select extract(time from local_datetime), extract(date from local_datetime) from EntityOfBasics e" )
							.list();
	...
	public void testExtractFunctionTimeZone(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					assertThat(
							session.createQuery(
											"select extract(offset hour from e.theZonedDateTime) from EntityOfBasics e" )
									.getResultList()
									.get( 0 ),
					...
					assertThat(
							session.createQuery(
											"select extract(offset minute from e.theZonedDateTime) from EntityOfBasics e" )
									.getResultList()
									.get( 0 ),
							anyOf( nullValue(), instanceOf( Integer.class ) )
					);
	...
	@RequiresDialectFeature(feature = DialectFeatureChecks.SupportsFormat.class,
			comment = "We extract the offset with a format function")
...
//	@FailureExpected
	...
	public void testLeastGreatestFunctions(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					session.createQuery(
									"select least(1, 2, e.theInt, -1), greatest(1, e.theInt, 2, -1) from EntityOfBasics e" )
							.list();
					session.createQuery(
									"select least(0.0, e.theDouble), greatest(0.0, e.theDouble, 2.0) from EntityOfBasics e" )
							.list();
				}
		);
	}
	...
	public void testAggregateFunctions(SessionFactoryScope scope) {
		scope.inTransaction(
				session -> {
					session.createQuery(
									"select avg(e.theDouble), avg(abs(e.theDouble)), min(e.theDouble), max(e.theDouble), sum(e.theDouble), sum(e.theInt) from EntityOfBasics e" )
							.list();
	...
	@RequiresDialectFeature(feature = DialectFeatureChecks.SupportsFormat.class)
	@SkipForDialect(dialectClass = CockroachDialect.class,
			reason = "unknown signature: experimental_strftime(time, string)")
	// could cast the first argument to timestamp to workaround this
	...
	@Test
	@SkipForDialect(dialectClass = CockroachDialect.class, reason = "unknown signature: log(int, int)")
	// could cast an argument to double to workaround this
