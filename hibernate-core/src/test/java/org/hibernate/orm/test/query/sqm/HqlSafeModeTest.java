...
@SessionFactory
@DomainModel (annotatedClasses = {HqlSafeModeTest.TestEntity.class})
@ServiceRegistry (settings = @Setting( name = QuerySettings.SAFE_MODE_ENABLED, value = "true"))
public class HqlSafeModeTest {
	...
	public void testHqlFunctionWithUnknownFunction(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
			...
			assertTrue( ex.getMessage().contains( "Function [regexp_substr] is not allowed in safe mode" ) );
		} );
	}
	...
	@Test
	public void testHqlFunctionColumnFunction(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
			Exception ex = assertThrows( IllegalArgumentException.class,
					() -> session.createQuery( "select function('column', e.name) from TestEntity e", String.class )
							.getResultList() );
	...
	@Test
	public void testHqlFunctionSqlFunction(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
			Exception ex = assertThrows( IllegalArgumentException.class,
					() -> session.createQuery( "select function('sql', 'SELECT 1') from TestEntity e", String.class )
							.getResultList() );
	...
	@Test
	public void testHqlFunctionSafeFunction(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
			session.createQuery( "select function('upper', e.name) from TestEntity e", String.class ).getResultList();
		} );
	}
	...
	public void testCriteriaUnknownFunction(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
			...
			JpaPath<String> name = from.get( "name" );
	...
	@Test
	public void testCriteriaBuilderFunctionSqlFunction(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
			var criteriaBuilder = session.getCriteriaBuilder();
			JpaCriteriaQuery<String> query = criteriaBuilder.createQuery( String.class );
			JpaRoot<TestEntity> from = query.from( TestEntity.class );
	...
	public void testCriteriaBuilderSqlFunction(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
			...
			Exception ex = assertThrows( SemanticException.class,
					() -> {
						query.select( criteriaBuilder.sql( "SELECT 1", String.class, name ) );
						session.createQuery( query ).getResultList();
					} );
	...
	@Test
	public void testCriteriaBuilderFunctionSafeFunction(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
			var criteriaBuilder = session.getCriteriaBuilder();
			JpaCriteriaQuery<String> query = criteriaBuilder.createQuery( String.class );
			JpaRoot<TestEntity> from = query.from( TestEntity.class );
			...
			query.select( criteriaBuilder.function( "upper", String.class, name ) );
		} );
	}
