...
public class RefreshPessimisticLockPlanCacheTest {
	...
	public void cascadePlanUnderPessimisticLockDoesNotPoisonTheRegularCache(SessionFactoryScope scope) {
		...
		scope.inTransaction( session -> {
			...
			// note that a resfresh currently fetch even unloaded lazy to-one relation
			//  and this is a bug similar to HHH-20628. There are currently no JIRA ticket for it though.
			//  Once this is fixed, it will be needed to manually initialize the relation before calling refresh
