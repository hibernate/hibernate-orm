...
@DomainModel(annotatedClasses = {
		AuditSingleTableExcludedPropertyTest.Base.class,
		AuditSingleTableExcludedPropertyTest.Middle.class,
		AuditSingleTableExcludedPropertyTest.Leaf.class
})
@ServiceRegistry(settings = @Setting(name = StateManagementSettings.CHANGESET_ID_SUPPLIER,
		value = "org.hibernate.temporal.audit.inheritance.exclusion.AuditSingleTableExcludedPropertyTest$TxIdSupplier"))
@Jira("https://hibernate.atlassian.net/browse/HHH-20857")
class AuditSingleTableExcludedPropertyTest {
	...
	void testAuditRead(SessionFactoryScope scope) {
		try (var session = scope.getSessionFactory().withStatelessOptions()
			...
			var results = session.createSelectionQuery(
					"from SingleTableExcludedBase order by id",
					Base.class
			).getResultList();
			assertThat( results ).hasSize( 3 );

			var base = (Base) results.get( 0 );
	...
	void testAuditReadWithTypePredicate(SessionFactoryScope scope) {
		try (var session = scope.getSessionFactory().withStatelessOptions()
				.atChangeset( AuditLog.ALL_CHANGESETS ).openStatelessSession()) {
			var results = session.createSelectionQuery(
					"from SingleTableExcludedBase b where type(b) = SingleTableExcludedLeaf order by id",
					Base.class
			).getResultList();
			assertThat( results ).hasSize( 1 );
			var leaf = (Leaf) results.get( 0 );
			assertThat( leaf.id ).isEqualTo( 3L );
			assertThat( leaf.baseIncluded ).isEqualTo( "leaf" );
			assertThat( leaf.middleIncluded ).isEqualTo( "leaf-middle" );
			assertThat( leaf.leafIncluded ).isEqualTo( "leaf" );
			assertThat( leaf.baseExcluded ).isNull();
	...
	@Entity(name = "SingleTableExcludedBase")
	@Audited
	...
	@Entity(name = "SingleTableExcludedMiddle")
	...
	@Entity(name = "SingleTableExcludedLeaf")
