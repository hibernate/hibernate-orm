 ...
 */
package org.hibernate.test.annotations.converter;
...
public class ConverterTest extends BaseEntityManagerFunctionalTestCase {
	...
	@Test
	public void testJPQLUpperAttributeValueBindParameter() throws Exception {
		doInJPA( this::entityManagerFactory, entityManager -> {
				Photo photo = entityManager.createQuery(
				"select p " +
				"from Photo p " +
				"where upper(caption) = upper(:caption) ", Photo.class )
			.setParameter( "caption", new Caption("Nicolae Grigorescu"))
			.getSingleResult();
