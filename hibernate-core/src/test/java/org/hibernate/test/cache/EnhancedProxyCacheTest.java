...
public class EnhancedProxyCacheTest extends BaseCoreFunctionalTestCase {
	...
	public void testPreferenceFor2LCOverUninitializedProxy() throws Exception {
		...
		inSession( s -> {
			...
			//Check that in this case we didn't initialize an enhanced proxy.
			final PersistentAttributeInterceptable interceptable = (PersistentAttributeInterceptable) continent;
			final LazyAttributeLoadingInterceptor interceptor = (LazyAttributeLoadingInterceptor) interceptable.$$_hibernate_getInterceptor();

			//And properties are already initialized, as there is no reason to defer this:
			assertTrue( interceptor.isAttributeLoaded( "code" ) );
