...
public class FetchTest extends BaseNonConfigCoreFunctionalTestCase {
	...
	public void assertOnlyOneSelect(Criteria criteria) {
		...
		try {
			...
			for ( Cat cat : cats ) {
				Assert.assertEquals( 3, cat.kittens.size() );

				for ( Kitten kitten : cat.kittens ) {
					Assert.assertNotNull( kitten.cat );
				}
			}

			Assert.assertEquals( "too many statements generated", 1, sessionFactory().getStatistics().getPrepareStatementCount() );
		}
		finally {
