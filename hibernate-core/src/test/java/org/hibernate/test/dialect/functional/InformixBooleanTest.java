...
/**
 * Tests the correctness of the parameter hibernate.dialect.informix.use_legacy_boolean_type which controls the mapping of
 * boolean types to be either SMALLINT (parameter is set to true) or BOOLEAN (default behavior or parameter is set to
 * false)
 * 
 * @author Jonathan Bregler
 * @author VladoKuruc
 */
@RequiresDialect(value = { InformixDialect.class })
public class InformixBooleanTest extends BaseCoreFunctionalTestCase {
		...
		doInHibernate( this::sessionFactory, session -> {
			session.doWork( connection -> {
				try ( PreparedStatement ps = connection
						.prepareStatement( "CREATE TABLE " + ENTITY_NAME + " (key INTEGER, bool BOOLEAN, PRIMARY KEY (key))" ) ) {
					ps.execute();
				}

				try ( PreparedStatement ps = connection
						.prepareStatement( "CREATE TABLE " + LEGACY_ENTITY_NAME + " (key INTEGER, bool SMALLINT, PRIMARY KEY (key))" ) ) {
					ps.execute();
				}
			} );
		} );
	...
	public void testBooleanType() throws Exception {
		rebuildSessionFactory( configuration -> {
			configuration.setProperty( "hibernate.dialect.informix.use_legacy_boolean_type", Boolean.FALSE.toString() );
		} );
	...
	public void testLegacyBooleanTypeLegacyBehavior() throws Exception {
		rebuildSessionFactory( configuration -> {
			configuration.setProperty( "hibernate.dialect.informix.use_legacy_boolean_type", Boolean.TRUE.toString() );
		} );
