...
public class AlterTableQuoteDefaultSchemaTest extends BaseCoreFunctionalTestCase {
		...
		try {
			doInHibernate( this::sessionFactory, session -> {
				session.createNativeQuery(
						"DROP TABLE \"default-schema\".\"my_entity\"" )
						.executeUpdate();
			} );
		}
	...
	public void testDefaultSchema() throws IOException {
		...
		try {
			...
			Pattern fileContentPattern = Pattern
					.compile( "create table \"default\\-schema\"\\.\"my_entity\"" );
		...
		try {
			String fileContent = new String( Files.readAllBytes( output.toPath() ) );
			Pattern fileContentPattern = Pattern
					.compile( "alter table \"default\\-schema\"\\.\"my_entity\"" );
			Matcher fileContentMatcher = fileContentPattern.matcher( fileContent.toLowerCase() );
			assertThat( fileContentMatcher.find(), is( true ) );
		}
		catch (IOException e) {
