...
public class EhcacheTransactionalDataRegion extends EhcacheDataRegion implements TransactionalDataRegion {
	...
	/**
	 * Construct an transactional Hibernate cache region around the given Ehcache
	 * instance which is further backed by REDIS for Invalidation.
	 */
	EhcacheTransactionalDataRegion(EhcacheAccessStrategyFactory accessStrategyFactory, Ehcache cache,
		...
		super(accessStrategyFactory, cache, properties, redisson, batchOptions, localCachedMapOptions);
	...
	public final Object get(Object key) {
		try {
			DistributedCacheLockWrapper response = validateCacheEntryBasedOnTimeStamp(key);
			if (response == null) {
	...
	/**
	 * Map the given value to the given key, replacing any existing mapping for this
	 * key
	 *
	 * @param key
	 *            The cache key
	...
	public final void putLock(Object key, Object value) throws CacheException {
		try {
			final Element element = new Element(key, value);
			getCache().put(element);
			putLockInDistributedCache(key, (Lock) value);
		} catch (IllegalArgumentException e) {
			throw new CacheException(e);
		} catch (IllegalStateException e) {
			throw new CacheException(e);
	...
	public final void putFromLoad(Object key, Object value) throws CacheException {
		try {
			final Element element = new Element(key, value);
			getCache().put(element);
			updateTimeStampInLocal(key);
		} catch (IllegalArgumentException e) {
			throw new CacheException(e);
		} catch (IllegalStateException e) {
			throw new CacheException(e);
	...
	/**
	 * Returns <code>true</code> if the locks used by the locking methods of this
	 * region are the independent of the cache.
	 * <p/>
	 * Independent locks are not locked by the cache when the cache is accessed
	 * directly. This means that for an independent lock lock holds taken through a
	 * region method will not block direct access to the cache via other means.
	 *
