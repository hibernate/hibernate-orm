...
import org.hibernate.redis.ehcache.regions.EhcacheTransactionalDataRegion;
...
public abstract class AbstractReadWriteEhcacheAccessStrategy<T extends EhcacheTransactionalDataRegion>
	...
	/**
	 * Returns <code>null</code> if the item is not readable. Locked items are not
	 * readable, nor are items created afterQuery the start of this transaction.
	 *
	...
	/**
	 * Returns <code>false</code> and fails to put the value if there is an existing
	 * un-writeable item mapped to this key.
	 *
	 * @see RegionAccessStrategy#putFromLoad(SharedSessionContractImplementor,
	 *      Object, Object, long, Object, boolean)
	 * @see RegionAccessStrategy#putFromLoad(SharedSessionContractImplementor,
	 *      Object, Object, long, Object, boolean)
	 */
	@Override
	public final boolean putFromLoad(SharedSessionContractImplementor session, Object key, Object value,
		...
		try {
			...
			if (writeable) {
				if (region().containsInDistributedCache(key)) {
					region().putFromLoad(key, new Item(value, version, region().nextTimestamp()));
				} else {
	 ...
	 *
	 * @see RegionAccessStrategy#lockItem(SharedSessionContractImplementor, Object,
	 *      Object)
	 * @see RegionAccessStrategy#lockItem(SharedSessionContractImplementor, Object,
	 *      Object)
	 */
	public final SoftLock lockItem(SharedSessionContractImplementor session, Object key, Object version)
		...
		try {
			...
			final Lock lock = (item == null) ? new Lock(timeout, uuid, nextLockId(), version)
					: item.lock(timeout, uuid, nextLockId());
			region().putLock(key, lock);
			return lock;
		} finally {
			region().writeUnlock(key);
		}
	 ...
	 *
	 * @see RegionAccessStrategy#unlockItem(SharedSessionContractImplementor,
	 *      Object, SoftLock)
	 * @see RegionAccessStrategy#unlockItem(SharedSessionContractImplementor,
	 *      Object, SoftLock)
	 */
	...
	protected void handleLockExpiry(Object key, Lockable lock) {
		...
		newLock.unlock(ts);
		region().putLock(key, newLock);
	}

	/**
	 * Read lock the entry for the given key if internal cache locks will not
	 * provide correct exclusion.
	 */
	...
	/**
	 * Read unlock the entry for the given key if internal cache locks will not
	 * provide correct exclusion.
	 */
	...
	public static interface Lockable {
		...
		/**
		 * Returns <code>true</code> if the enclosed value can be read by a transaction
		 * started at the given time.
		 */
		public boolean isReadable(long txTimestamp);

		/**
		 * Returns <code>true</code> if the enclosed value can be replaced with one of
		 * the given version by a transaction started at the given time.
		 */
		...
		/**
		 * Returns <code>true</code> if the given lock can be unlocked using the given
		 * SoftLock instance as a handle.
		 */
		public boolean isUnlockable(SoftLock lock);

		/**
		 * Locks this entry, stamping it with the UUID and lockId given, with the lock
		 * timeout occuring at the specified time. The returned Lock object can be used
		 * to unlock the entry in the future.
		 */
	...
	public static final class Item implements Serializable, Lockable {
		...
		/**
		 * Creates an unlocked item wrapping the given value with a version and creation
		 * timestamp.
		 */
		...
		public String toString() {
			return "Item{" + "value=" + value + ", version=" + version + ", timestamp=" + timestamp + '}';
		}
	}
	...
	public static final class Lock implements Serializable, Lockable, SoftLock {
		...
		public boolean isWriteable(long txTimestamp, Object newVersion, Comparator versionComparator) {
			...
			if (version != null) {
				LOG.error("Versioningnot allowed for Persister");
			}
			return txTimestamp > unlockTimestamp;
		}
		
		public boolean isWriteable(long txTimestamp) {
			return (txTimestamp > timeout) ? true : false;
		}
