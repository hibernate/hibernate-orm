...
public class CascadeDetachedExecutableTestTask extends AbstractExecutable {
	...
	protected void prepared() {
		assertIsEnhancedForLazyLoading( Book.class );
		assertIsEnhancedForLazyLoading( Author.class );
	}

	private void assertIsEnhancedForLazyLoading(Class<?> clazz) {
		EntityPersister ep = getEntityManagerFactory().getSessionFactory().getEntityPersister( clazz.getName() );
