...
public class SimpleStatelessSessionTests {
	...
	void simpleSession(SessionFactoryScope factoryScope) {
		...
		factoryScope.inTransaction( (session) -> {
			final Long count = session.createQuery( String.format(
							"select count(*) from %s_AUD",
							SimpleStatelessSessionTests.Person.class.getName() ),
					Long.class
			).getSingleResult();
	...
	@Test
	@Jira( "https://hibernate.atlassian.net/browse/HHH-19737" )
