...
public class NonTxInvalidationInterceptor extends BaseRpcInterceptor implements JmxStatisticsExposer {
	...
	private void invalidateAcrossCluster(boolean synchronous, Object[] keys, InvocationContext ctx) throws Throwable {
		...
		if (lockOwner == null) {
			log.debug("Won't send invalidation (transaction probably rolled back)");
			return;
		}
		BeginInvalidationCommand beginInvalidationCommand = commandInitializer.buildBeginInvalidationCommand(
				InfinispanCollections.<Flag>emptySet(), keys, lockOwner);
		if (log.isDebugEnabled()) {
			log.debug("Cache [" + rpcManager.getAddress() + "] replicating " + beginInvalidationCommand);
		}

		rpcManager.invokeRemotely(null, beginInvalidationCommand, rpcManager.getDefaultRpcOptions(synchronous));
	}
