...
public class IsolatedClassLoaderTest extends DualNodeTestCase {
    ...
    protected void cleanupTest() throws Exception {
        try {
            if (localQueryCache != null && localQueryListener != null) localQueryCache.removeListener(localQueryListener);
            if (remoteQueryCache != null && remoteQueryListener != null) remoteQueryCache.removeListener(remoteQueryListener);
        } finally {
            super.cleanupTest();
        }
    }
    ...
    /**
     * Simply confirms that the test fixture's classloader isolation setup is functioning as expected.
     *
     * @throws Exception
     */
    ...
    protected void queryTest( boolean useNamedRegion ) throws Exception {
        ...
        // LOG.info("First query (get count for branch + " + branch + " ) on node0 done, contents of local query cache are: " +
        // TestingUtil.printCache(localQueryCache));
