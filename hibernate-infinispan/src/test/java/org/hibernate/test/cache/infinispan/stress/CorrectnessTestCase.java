...
public abstract class CorrectnessTestCase {
   ...
   public static class Transactional extends CorrectnessTestCase {
      @Override
   ...
   @Ignore // as long-running test, we'll execute it only by hand
   public static class ReadOnly extends CorrectnessTestCase {
      @Override
      protected Operation getOperation() {
         ThreadLocalRandom random = ThreadLocalRandom.current();
         Operation operation;
   ...
   @Ignore // as long-running test, we'll execute it only by hand
   public static class NonTransactional extends Transactional {
      @Override
      protected void applySettings(StandardServiceRegistryBuilder ssrb) {
         ssrb.applySetting(Environment.CACHE_REGION_FACTORY, ForceNonTxInfinispanRegionFactory.class.getName());
      }
   ...
   public void beforeClass() {
      ...
      transactionManager = TestingJtaPlatformImpl.transactionManager();
   }
   ...
   public static class ForceNonTxInfinispanRegionFactory extends TestInfinispanRegionFactory {
      ...
      protected AdvancedCache getCache(String regionName, String typeKey, Properties properties, CacheDataDescription metadata) {
         if (typeKey.equals("entity")) {
            typeKey = "immutable-entity";
         }
         return super.getCache(regionName, typeKey, properties, metadata);
      }
   }
                  ...
                  try {
                     ...
                     if (e instanceof RollbackException) {
                        ...
                        if (cause instanceof StaleObjectStateException
                           ...
                           continue;
                        } else if (hasCause(e, org.infinispan.util.concurrent.TimeoutException.class)) {
                           // FIXME: HHH-7898
                           continue;
                        }
   ...
   protected void checkForEmptyPendingPuts() throws Exception {
      ...
      List<Map.Entry> delayed = new LinkedList<>();
      ...
      while (System.currentTimeMillis() < deadline) {
         for (Iterator<Map.Entry> iterator = delayed.iterator(); iterator.hasNext(); ) {
            Map.Entry entry = iterator.next();
            java.util.Collection invalidators = (java.util.Collection) getInvalidators.invoke(entry.getValue());
            if (invalidators == null || invalidators.isEmpty()) {
               iterator.remove();
   ...
   private abstract class Operation {
      ...
      protected void withSession(Consumer<Session> consumer) throws Exception {
         ...
         TransactionManager tm = transactionManager(node);
         Session s = null;
         tm.begin();
         try {
            s = sessionFactory(node).openSession();
      ...
      protected void withRandomFamily(BiConsumer<Session, Family> consumer, Ref<String> familyNameUpdate, Ref<Set<String>> familyMembersUpdate, LockMode lockMode) throws Exception {
         ...
         log.tracef("Started %s(%d) at %d", getClass().getSimpleName(), id, before);
   ...
   private class InsertFamily extends Operation {
      ...
      public void run() throws Exception {
         ...
         try {
            ...
            LogType type = failure ? LogType.WRITE_FAILURE : LogType.WRITE;
   ...
   private class UpdateFamily extends Operation {
      ...
      public void run() throws Exception {
         ...
         withRandomFamily((s, f) -> {
            f.setName(newName);
            s.persist(f);
         }, Ref.of(newName), Ref.empty(), LockMode.OPTIMISTIC_FORCE_INCREMENT);
      }
   }
   ...
   private PutFromLoadValidator getPutFromLoadValidator(SessionFactoryImplementor sfi, String regionName) throws NoSuchFieldException, IllegalAccessException {
      ...
      Field validatorField = delegate.getClass().getDeclaredField("putValidator");
      validatorField.setAccessible(true);
      return (PutFromLoadValidator) validatorField.get(delegate);
   }

   protected TransactionManager transactionManager(int node) {
