...
[source,bash]
----
./gradlew :hibernate-tck-runner:test -Pdb=postgresql -Pruntck=true
----

[NOTE]
====
Make sure to include the `postgresql` database parameter!
====
...
[source,bash]
----
./gradlew :hibernate-tck-runner:test -Pdb=postgresql -Pruntck=true -DenableMavenLocalRepo=true
----
...
* Edit the newly created run configuration and add the `-Pdb=postgresql` at the very end of the run command:
