...
[source,bash]
----
./gradlew :hibernate-tck-runner:test -Pdb=pgsql -Pruntck=true
----

[NOTE]
====
Make sure to include the `pgsql` database parameter!
====
...
[source,bash]
----
./gradlew :hibernate-tck-runner:test -Pdb=pgsql -Pruntck=true -DenableMavenLocalRepo=true
----
...
* Edit the newly created run configuration and add the `-Pdb=pgsql` at the very end of the run command:
