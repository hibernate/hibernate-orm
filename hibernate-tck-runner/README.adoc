...
[source,bash]
----
./gradlew :hibernate-tck-runner:test -Pdb=pgsql_ci -Pruntck=true
----
...
[source,bash]
----
./gradlew :hibernate-tck-runner:test -Pdb=pgsql_ci -Pruntck=true -DenableMavenLocalRepo=true -Pgradle.libs.versions.persistenceTck=4.0.0-SNAPSHOT
----
...
The `-Pgradle.libs.versions.persistenceTck=4.0.0-SNAPSHOT` overrides the version of the TCK used to run the tests.
Specify the version that you've built and want to test against.
