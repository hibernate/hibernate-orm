...
public final class ValidationDiagnostic {
	static final Comparator<ValidationDiagnostic> ORDERING = Comparator
			.comparing( (ValidationDiagnostic diagnostic) -> diagnostic.rule.getId() )
	...
	static ValidationDiagnostic fromEvidence(ValidationEvidence.Item item) {
		return new ValidationDiagnostic(
				item.getRule(),
				item.getSourceElementId(),
				item.getTargetElementId(),
				item.getSourceCategory(),
				item.getTargetCategory(),
				item.getEdgeKind(),
				item.getRoles(),
				item.getPath().isEmpty() ? List.of( item.getSourceElementId(), item.getTargetElementId() ) : item.getPath(),
				item.getMessage()
		);
	}
