...
public final class ValidationResult {
	...
	static ValidationResult complete(
		...
		for ( ValidationAllowlist.Entry entry : allowlist.getEntries() ) {
			if ( entry.getCause().getDomain() == domain && !usedEntries.contains( entry ) ) {
	...
	static ValidationResult focused(List<ValidationDiagnostic> diagnostics) {
		diagnostics.sort( ValidationDiagnostic.ORDERING );
		return new ValidationResult( diagnostics, Collections.emptyList() );
	}
	...
	public boolean hasFailures() {
		...
		for ( ValidationDiagnostic diagnostic : diagnostics ) {
			if ( diagnostic.getSeverity() == ValidationCause.Severity.ERROR
