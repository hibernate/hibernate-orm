...
`UserType` is still supported, and is specified using the new `@CustomType` annotation.
...
6.0 adds some new ways to influence collection mapping and makes some changes
to an existing way.
...
6.0 adds the `@CollectionClassificationType` annotation to allow specifying the semantics to apply to a
plural attribute mapping based on discrete (enum) set of classifications.

See https://docs.jboss.org/hibernate/orm/6.0/userguide/html_single/Hibernate_User_Guide.html#collection-classification-ann
for details of using `@CollectionClassificationType`


==== `@CollectionSemantics`

6.0 adds the `@CollectionSemantics` annotation to allow specifying a custom `CollectionSemantics` implementation
to use for a plural attribute mapping.

It also adds `@CollectionSemanticsRegistration` annotation to auto-apply a custom semantic to all collections
determined to use the specified classification.

See https://docs.jboss.org/hibernate/orm/6.0/userguide/html_single/Hibernate_User_Guide.html#collection-semantics-ann
for details of using `@CollectionSemantics`
...
// todo (6.0) - Query parameter binding overloads accepting `Type`, `AllowableParameterType`
