...
For a sequence id generation strategy Hibernate 6.0 does not create anymore a single sequence named `hibernate_sequence` but it creates one sequence for each entity id.

When using `@GeneratedValue` without specifying a  `generator` value the default increment size also changed from 1 to 50.

In order to maintain backward compatibility it is possible to add the following setting to the configuration:

```
hibernate.id.prefer_sequence_per_entity = hibernate_sequence

hibernate.id.default_increment_size = 1
```

In alternative the code using a `@GeneratedValue` with a `GenerationType.SEQUENCE` or `GenerationType.AUTO` strategy with no `generator` value or with the `generator` value no matching any generator name (in this last case only if the configuration has the `PREFER_GENERATOR_NAME_AS_DEFAULT_SEQUENCE_NAME` set to `false`  ), needs to be modified as follows:
```
@Id
@GeneratedValue
```
...
```
@Id
@GeneratedValue(stratefy = "GenerationType.AUTO")
```
...
```
To avoid repeating the `@SequenceGenerator` for each entity a `@org.hibernate.annotations.GenericGenerator` can be defined in a `package-info.java` file
```
...
In case the mapping is not using annotations but an `orm.xml` file then you can define a
```
<sequence-generator
...
```
 <id name=...>
    <generated-value strategy="AUTO" generator="hibernate_sequence_generator"/>
</id
```
and
```
 <id ...>
    <generated-value strategy="SEQUENCE" generator="hibernate_sequence_generator"/>
</id
```

For `hbm.xml` mapping all the
```
<id ...>
...
```
have to be changed respectively into
```
<id name="id">
    <generator class="hilo">
        <param name="native">hibernate_sequence</param>
