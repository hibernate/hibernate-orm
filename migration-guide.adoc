...
The `@Target` annotation has been replaced with the new `@TargetEmbeddable`. This new annotation is only allowed on elements that are designated as `@Embedded`, and its purpose is to indicate the `@Embeddable` class that is the implementation target of the embedded instance.
