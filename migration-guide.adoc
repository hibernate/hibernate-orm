...
In order to improve performances during schema creation and validation 5.1.4 introduced a new default strategy
called `grouped` to retrieve JDBC Metadata.

The grouped became the new default strategy and it may require the `default_schema` and/or `default_catalog` to be provided.

An alternative to providing the default schema/catalog is to set `hibernate.hbm2ddl.jdbc_metadata_extraction_strategy`
to `individually`.
