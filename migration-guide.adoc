...
The annotation processor now propagates `@DenyAll`, `@PermitAll`, and `@RolesAllowed` (`jakarta.annotation.security`) from repository interfaces to generated implementations. Previously, only `@InterceptorBinding` annotations were propagated.
