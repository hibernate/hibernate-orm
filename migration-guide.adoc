...
These changes require that code using a `@GeneratedValue` with a `GenerationType.SEQUENCE` or `GenerationType.AUTO` strategy with no `generator` value needs to be modified as follows:
```
@Id
@GeneratedValue
```
