...
`Expression`s contribute to this by exposing a `Selectable`, which in turn can be used to
build `Return` references which model the individual values in the overall domain query result.
The specific `Return` sub-types include:
	* `ReturnScalar` - scalar query result; e.g. a function call in the HQL or reference to a String-valued attribute
	* `ReturnComposite` - composite/embeddable valued query result
	* `ReturnEntity` - an entity valued query return
	* `ReturnDynamicInstantiation` - a "constructor" query result.  only used through SQM -> SQL AST producer
	* `ReturnCollection` - specialized query result used when loading plural attribute entries.

Generally speaking an `Expression` is its own `Selectable`.  Examples where that is not the case include
`NavigableReferenceExpression`s.

Ultimately this `SelectClause` need to be converted into a SQL SELECT statement as well as
"readers" to read back values from the JDBC `ResultSet`.
...
`SqlSelectAstToJdbcSelectConverter` is responsibile to render the SQL AST into a SQL query String.  It
does this by walking the nodes of the `SelectStatement` using the visitation pattern.  This is where the
collection of `SqlSelection` references occurs.
...
Overall, this `SqlSelectAstToJdbcSelectConverter` has 2 main responsibilities:
	* Rendering the SQL String
	* Building "readers"
...
One of the functions performed by `SqlSelectAstToJdbcSelectConverter` is to render the SQL AST into a SQL query String.  It
does this by walking the nodes of the `SelectStatement` using the visitation pattern.  This is where the
collection of `SqlSelection` references as described above occurs.
...
Again, this will be discussed more in the section describing processing results.
