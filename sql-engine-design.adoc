...
Consumption of an SQL AST tree involves:
	...
	* generating JDBC "operation" - `org.hibernate.sql.ast.tree.spi.Statement` todo (6.0) : spi package?
	* executing these JDBC operation - `org.hibernate.sql.ast.consume.spi.JdbcSelectExecutor` and other still to
		be defined for executing DELETEs, etc
	* (possibly) processing ResultSet(s) into domain results - `org.hibernate.sql.exec.results`
