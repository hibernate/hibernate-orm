...
== Schema Annotation Generation

The plugin contributes a `generateSchemaAnnotations` task which connects to a JDBC database, reads table
and column metadata, and generates annotation types for the configured Java package.

[source,groovy]
----
tasks.named("generateSchemaAnnotations") {
    hibernateProperties = "hibernate.properties" // optional, defaults to hibernate.properties
    revengFile = "hibernate.reveng.xml" // optional
    packageName = "org.example.schema"
}
----

The generated sources are written to `build/generated/sources/schemaAnnotations` by default. The task
reads JDBC connection settings from `hibernate.properties` in the main resources source set. Direct
task properties (`jdbcDriver`, `jdbcUrl`, `username`, `password`, `catalogName`, and `schemaName`)
override values read from `hibernate.properties`.

[source,properties]
----
hibernate.connection.driver_class=org.h2.Driver
hibernate.connection.url=jdbc:h2:./database/test
hibernate.connection.username=sa
hibernate.connection.password=
hibernate.default_schema=PUBLIC
----

The JDBC driver must be available from the project's runtime classpath.

When specified, `revengFile` is resolved from the main resources source set. The schema annotation
generator supports schema selection, table filters, table exclusions, column exclusions, and
user-defined foreign keys from the reverse-engineering file.
