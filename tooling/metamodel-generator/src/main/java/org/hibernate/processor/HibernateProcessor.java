...
@SupportedAnnotationTypes({
		// standard for JPA 2
		Constants.ENTITY, Constants.MAPPED_SUPERCLASS, Constants.EMBEDDABLE,
		// standard for JPA 3.2
		Constants.NAMED_QUERY, Constants.NAMED_NATIVE_QUERY, Constants.NAMED_ENTITY_GRAPH, Constants.SQL_RESULT_SET_MAPPING,
		// extra for Hibernate
		Constants.HIB_FETCH_PROFILE, Constants.HIB_FILTER_DEF, Constants.HIB_NAMED_QUERY, Constants.HIB_NAMED_NATIVE_QUERY,
		// Hibernate query methods
		Constants.HQL, Constants.SQL, Constants.FIND,
		// Jakarta Data repositories
		Constants.JD_REPOSITORY // do not need to list any other Jakarta Data annotations here
})
@SupportedOptions({
		DEBUG_OPTION,
...
public class HibernateProcessor extends AbstractProcessor {
		...
		for ( Element element : roundEnvironment.getRootElements() ) {
			try {
				if ( isEntityOrEmbeddable( element ) ) {
					...
					final AnnotationMirror repository = TypeUtils.getAnnotationMirror( element, Constants.JD_REPOSITORY );
					if ( repository != null ) {
						final String provider = (String) TypeUtils.getAnnotationValue( repository, "provider" );
						...
						for ( Element member : typeElement.getEnclosedElements() ) {
							if ( TypeUtils.hasAnnotation( member, Constants.HQL, Constants.SQL, Constants.FIND ) ) {
								context.logMessage( Diagnostic.Kind.OTHER, "Processing annotated class '" + element + "'" );
								final AnnotationMetaEntity metaEntity =
										AnnotationMetaEntity.create( typeElement, context, false, false );
								context.addMetaAuxiliary( metaEntity.getQualifiedName(), metaEntity );
	...
	private void handleRootElementAnnotationMirrors(final Element element) {
		if ( TypeUtils.isClassOrRecordType( element ) ) {
			if ( TypeUtils.hasAnnotation( element, Constants.ENTITY, Constants.MAPPED_SUPERCLASS, Constants.EMBEDDABLE ) ) {
				final TypeElement typeElement = (TypeElement) element;
				...
				if ( alreadyExistingMetaEntity != null && alreadyExistingMetaEntity.isMetaComplete() ) {
					...
					final boolean requiresLazyMemberInitialization
							= TypeUtils.hasAnnotation( element, Constants.EMBEDDABLE, Constants.MAPPED_SUPERCLASS );
					final AnnotationMetaEntity metaEntity =
...
//						}
	...
	private @Nullable Metamodel tryGettingExistingEntityFromContext(TypeElement typeElement, String qualifiedName) {
		if ( TypeUtils.hasAnnotation( typeElement, Constants.ENTITY, Constants.MAPPED_SUPERCLASS ) ) {
			return context.getMetaEntity( qualifiedName );
		}
		else if ( TypeUtils.hasAnnotation( typeElement, Constants.EMBEDDABLE ) ) {
	...
	private void addMetamodelToContext(TypeElement typeElement, AnnotationMetaEntity entity) {
		...
		if ( TypeUtils.hasAnnotation( typeElement, Constants.ENTITY ) ) {
			context.addMetaEntity( key, entity );
		}
		else if ( TypeUtils.hasAnnotation( typeElement, Constants.MAPPED_SUPERCLASS ) ) {
			context.addMetaEntity( key, entity );
		}
		else if ( TypeUtils.hasAnnotation( typeElement, Constants.EMBEDDABLE ) ) {
