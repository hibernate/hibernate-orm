...
import org.hibernate.processor.annotation.CriteriaFinderMethod.OrderBy;
...
public class AnnotationMetaEntity extends AnnotationMeta {
	...
	public @Nullable String getSupertypeName() {
		return TypeUtils.findMappedSuperClass( this, context );
	}
	...
	protected final void init() {
		...
		entityAccessTypeInfo = NullnessUtil.castNonNull( context.getAccessTypeInfo( getQualifiedName() ) );
		...
		for ( ExecutableElement method: methodsOfClass ) {
			...
			else if ( TypeUtils.containsAnnotation( method, Constants.HQL, Constants.SQL, Constants.JD_QUERY, Constants.FIND, Constants.JD_FIND ) ) {
				queryMethods.add( method );
			}
			else if ( TypeUtils.containsAnnotation( method, Constants.JD_INSERT, Constants.JD_UPDATE, Constants.JD_DELETE, Constants.JD_SAVE ) ) {
				lifecycleMethods.add( method );
			}
		}

		jakartaDataRepository = TypeUtils.hasAnnotation( element, Constants.JD_REPOSITORY );
		findSessionGetter( element );
		if ( !repository && jakartaDataRepository) {
			repository = true;
			sessionType = Constants.HIB_STATELESS_SESSION;
	...
	private @Nullable String dataStore() {
		final AnnotationMirror repo = TypeUtils.getAnnotationMirror( element, Constants.JD_REPOSITORY );
		if ( repo != null ) {
			final String dataStore = (String) TypeUtils.getAnnotationValue( repo, "dataStore" );
	...
	private void findSessionGetter(TypeElement type) {
		if ( !TypeUtils.hasAnnotation( type, Constants.ENTITY )
				&& !TypeUtils.hasAnnotation( type, Constants.MAPPED_SUPERCLASS )
				&& !TypeUtils.hasAnnotation( type, Constants.EMBEDDABLE ) ) {
	...
	private boolean isPersistent(Element memberOfClass, AccessType membersKind) {
		return ( entityAccessTypeInfo.getAccessType() == membersKind
					|| TypeUtils.determineAnnotationSpecifiedAccessType( memberOfClass ) != null )
			&& !TypeUtils.containsAnnotation( memberOfClass, Constants.TRANSIENT )
	...
	private void addLifecycleMethods(List<ExecutableElement> queryMethods) {
		for ( ExecutableElement method : queryMethods) {
			if ( method.getModifiers().contains(Modifier.ABSTRACT) ) {
				if ( TypeUtils.hasAnnotation( method, Constants.JD_INSERT, Constants.JD_UPDATE, Constants.JD_DELETE, Constants.JD_SAVE ) ) {
	...
	private void addQueryMethod(ExecutableElement method) {
		...
		if ( kind == TypeKind.VOID ||  kind == TypeKind.ARRAY || kind.isPrimitive() ) {
			...
			switch ( typeArguments.size() ) {
				case 0:
					if ( TypeUtils.containsAnnotation( declaredType.asElement(), Constants.ENTITY ) ) {
	...
	private void addQueryMethod(
		...
		final AnnotationMirror hql = TypeUtils.getAnnotationMirror( method, Constants.HQL );
		if ( hql != null ) {
			addQueryMethod( method, returnType, containerType, hql, false );
		}
		final AnnotationMirror sql = TypeUtils.getAnnotationMirror( method, Constants.SQL );
		if ( sql != null ) {
			addQueryMethod( method, returnType, containerType, sql, true );
		}
		final AnnotationMirror jdql = TypeUtils.getAnnotationMirror( method, Constants.JD_QUERY );
		if ( jdql != null ) {
			addQueryMethod( method, returnType, containerType, jdql, false );
		}
		if ( TypeUtils.hasAnnotation( method, Constants.FIND, Constants.JD_FIND ) ) {
	...
	private static String lifecycleOperation(ExecutableElement method) {
		if ( TypeUtils.hasAnnotation(method, Constants.JD_INSERT) ) {
			return "insert";
		}
		else if ( TypeUtils.hasAnnotation(method, Constants.JD_UPDATE) ) {
			return "update";
		}
		else if ( TypeUtils.hasAnnotation(method, Constants.JD_DELETE) ) {
			return "delete";
		}
		else if ( TypeUtils.hasAnnotation(method, Constants.JD_SAVE) ) {
			return "upsert";
		}
		else {
	...
	private void addFinderMethod(
		...
		if ( returnType == null || returnType.getKind() != TypeKind.DECLARED ) {
			...
			if ( !TypeUtils.containsAnnotation( entity, Constants.ENTITY ) ) {
				context.message( method,
				...
				if ( containerType != null ) {
					...
					for ( VariableElement parameter : method.getParameters() ) {
						...
						if ( AbstractQueryMethod.isPageParam(type) ) {
							context.message( parameter, "pagination would have no effect", Diagnostic.Kind.ERROR);
						}
						else if ( AbstractQueryMethod.isOrderParam(type) ) {
	...
	private void createCriteriaFinder(
		...
		for ( VariableElement parameter : method.getParameters() ) {
			if ( isFinderParameterMappingToAttribute( parameter ) ) {
				...
				final TypeMirror parameterType = parameter.asType();
				if ( AbstractQueryMethod.isOrderParam( parameterType.toString() ) ) {
	...
	private List<OrderBy> orderByList(ExecutableElement method, TypeElement entityType) {
		final AnnotationMirror orderByList =
				TypeUtils.getAnnotationMirror( method, "jakarta.data.repository.OrderBy.List" );
		if ( orderByList != null ) {
			final List<OrderBy> result = new ArrayList<>();
			@SuppressWarnings("unchecked")
			final List<AnnotationValue> list = (List<AnnotationValue>)
					NullnessUtil.castNonNull( TypeUtils.getAnnotationValue( orderByList, "value" ) );
			for ( AnnotationValue element : list ) {
				result.add( orderByExpression( NullnessUtil.castNonNull( (AnnotationMirror) element.getValue() ), entityType, method ) );
			}
			return result;
		}
		final AnnotationMirror orderBy =
				TypeUtils.getAnnotationMirror( method, "jakarta.data.repository.OrderBy" );
		if ( orderBy != null ) {
	...
	private OrderBy orderByExpression(AnnotationMirror orderBy, TypeElement entityType, ExecutableElement method) {
		final String fieldName = (String) NullnessUtil.castNonNull( TypeUtils.getAnnotationValue(orderBy, "value") );
		final Boolean descendingOrNull = (Boolean) TypeUtils.getAnnotationValue(orderBy, "descending");
		final Boolean ignoreCaseOrNull = (Boolean) TypeUtils.getAnnotationValue(orderBy, "ignoreCase");
			...
			case DECLARED:
				...
				if ( parameterTypeName.startsWith( List.class.getName() ) ) {
					for (TypeMirror arg : type.getTypeArguments()) {
						...
						|| parameterTypeName.startsWith( Constants.JD_SORT )
						|| parameterTypeName.startsWith( Constants.JD_ORDER )) {
					for ( TypeMirror arg : type.getTypeArguments() ) {
						switch ( arg.getKind() ) {
							case WILDCARD:
	...
	private static boolean isFinderParameterMappingToAttribute(VariableElement param) {
		...
		return !isSessionParameter( type )
			&& !AbstractQueryMethod.isPageParam( type )
			&& !AbstractQueryMethod.isOrderParam( type );
	}
	...
	private static String getSessionVariableName(String sessionType) {
		switch (sessionType) {
			case Constants.HIB_SESSION:
			case Constants.HIB_STATELESS_SESSION:
			case Constants.MUTINY_SESSION:
				return "session";
			default:
	...
	private static List<String> enabledFetchProfiles(ExecutableElement method) {
		final AnnotationMirror findAnnotation = TypeUtils.getAnnotationMirror( method, Constants.FIND );
		if ( findAnnotation == null ) {
			return emptyList();
		}
		else {
			final Object enabledFetchProfiles =
					TypeUtils.getAnnotationValue( findAnnotation, "enabledFetchProfiles" );
	...
	private int countNaturalIdFields(TypeElement entity) {
		...
		for ( Element member : entity.getEnclosedElements() ) {
			if ( TypeUtils.containsAnnotation( member, Constants.NATURAL_ID ) ) {
				count ++;
			}
		}
		return count;
	...
	private @Nullable FieldType validateFinderParameter(TypeElement entityType, VariableElement param) {
		...
		if ( member != null) {
			...
			if ( TypeUtils.containsAnnotation( member, Constants.ID, Constants.EMBEDDED_ID ) ) {
				return FieldType.ID;
			}
			else if ( TypeUtils.containsAnnotation( member, Constants.NATURAL_ID ) ) {
		...
		final AnnotationMirror idClass = TypeUtils.getAnnotationMirror( entityType, Constants.ID_CLASS );
		if ( idClass != null ) {
			final Object value = TypeUtils.getAnnotationValue( idClass, "value" );
	...
	private AccessType getAccessType(TypeElement entity) {
		...
		TypeUtils.determineAccessTypeForHierarchy(entity, context );
		return NullnessUtil.castNonNull( context.getAccessTypeInfo( entityClassName ) ).getAccessType();
	}
	...
	private void addQueryMethod(
			...
			boolean isNative) {
		final AnnotationValue value = TypeUtils.getAnnotationValueRef( mirror, "value" );
		if ( value != null ) {
	...
	private boolean checkReturnedEntity(EntityDomainType<?> model, TypeMirror returnType) {
		if ( returnType.getKind() == TypeKind.DECLARED ) {
			final DeclaredType declaredType = (DeclaredType) returnType;
			final TypeElement typeElement = (TypeElement) declaredType.asElement();
			final AnnotationMirror mirror = TypeUtils.getAnnotationMirror(typeElement, Constants.ENTITY );
			if ( mirror != null ) {
				final Object value = TypeUtils.getAnnotationValue( mirror, "name" );
	...
	private static String parameterName(VariableElement parameter) {
		final AnnotationMirror by = TypeUtils.getAnnotationMirror( parameter, "jakarta.data.repository.By" );
		final AnnotationMirror param = TypeUtils.getAnnotationMirror( parameter, "jakarta.data.repository.Param" );
		if ( by != null ) {
			final String name = (String) NullnessUtil.castNonNull(TypeUtils.getAnnotationValue(by, "value"));
			...
			final String name = (String) NullnessUtil.castNonNull(TypeUtils.getAnnotationValue(param, "value"));
			if ( name.contains("<error>") ) {
				throw new ProcessLaterException();
			}
			return name;
	...
	private static boolean isNullable(Element member) {
		...
		for ( AnnotationMirror mirror : member.getAnnotationMirrors() ) {
			...
			if ( name.contentEquals(Constants.BASIC)
				...
				if ( FALSE.equals( TypeUtils.getAnnotationValue(mirror, "optional") ) ) {
					nullable = false;
				}
			}
		}
