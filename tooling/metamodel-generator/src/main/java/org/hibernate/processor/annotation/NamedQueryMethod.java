...
class NamedQueryMethod implements MetaAttribute {
	...
	private String fieldName() {
		return "QUERY_" + StringUtil.nameToFieldName(name);
	}
	...
	private @Nullable TypeElement entityType(String entityName) {
		TypeElement symbol =
				ProcessorSessionFactory.findEntityByUnqualifiedName( entityName,
		...
		for ( ModuleElement module : annotationMeta.getContext().getElementUtils().getAllModuleElements() ) {
			symbol = ProcessorSessionFactory.findEntityByUnqualifiedName( entityName, module );
			if ( symbol != null ) {
				return symbol;
			}
		}
