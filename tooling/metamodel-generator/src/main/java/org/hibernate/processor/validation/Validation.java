...
//			e.printStackTrace();
	...
	private static SemanticQueryBuilder<?> createSemanticQueryBuilder(
		...
		if ( returnType != null && returnType.getKind() == TypeKind.DECLARED ) {
			...
			return ProcessorSessionFactory.isEntity( typeElement )
					? new SemanticQueryBuilder<>( typeName, shortName, ProcessorSessionFactory.getEntityName(typeElement), CREATION_OPTIONS, factory, hql )
