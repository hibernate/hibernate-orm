...
The new method `createExistsQuery()` of `JpaCriteriaQuery` is similar to the older `createCountQuery()`, but it transforms a criteria query into a query that returns `true` if the original query has at least one result.
The resulting SQL uses an `exists()` quantifier.