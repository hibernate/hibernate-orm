...
[[bulk-select]]
== Bulk Select (Subselect) Fetching

The following major enhancements were made to _bulk select fetching_ (what we previously called "subselect" fetching).
...
- `FetchMethod` was introduced as a JPA 4 `FetchOption` enumerating the basic fetching methods `SELECT`, `JOIN`, and `BULK_SELECT`. To request bulk select fetching for an `AttributeNode` of an `EntityGraph`, call `node.addOption(FetchMethod.BULK_SELECT)`.
+
