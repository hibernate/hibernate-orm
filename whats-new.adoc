...
Support for Jackson 3 JSON and XML ``FormatMapper``s has been added, alongside the existing Jackson 2 support.
Note that if you have both versions of Jackson in your classpath, Hibernate will default to the older one (Jackson 2).
This behavior can be overridden by specifying the `hibernate.type.json_format_mapper` and `hibernate.type.xml_format_mapper` configuration properties.