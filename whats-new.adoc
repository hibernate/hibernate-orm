...
[[bidir-mgmt]]
== Post-Flush Bidirectionality Management

Hibernate can now optionally repair bidirectional associations in the managed in-memory object graph after a successful flush. When enabled with `hibernate.bidirectionality_management=true`, Hibernate treats the owning side as authoritative and synchronizes inverse-side references and collections for managed entities, without scheduling additional SQL or making inverse-side-only changes persistent in the same flush.

Handling of lazy inverse state is controlled by `hibernate.bidirectionality_management.lazy_policy`, with `skip`, `warn` (default), and `initialize` options.
